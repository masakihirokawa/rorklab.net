...
冒頭で触れた「作り直し」を私が避けられるようになったのは、画面を足す前にディレクトリ構造を一度紙に書き出す習慣をつけてからでした。タブ・スタック・モーダルの3つのパターンを使い分け、認証ガードとディープリンクを必要に応じて重ねていく。この順序を守るだけで、後からの手戻りはかなり減らせます。まずはシンプルなタブナビゲーションから始めて、構造が見えてきた段階で複雑な画面遷移を足していくのがおすすめです。