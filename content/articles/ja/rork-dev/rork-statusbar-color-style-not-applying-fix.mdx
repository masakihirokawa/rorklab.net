...
author: "Rork Lab"
description: "Rorkで生成したアプリのStatusBarがiOSとAndroidで意図通りに表示されない問題を、5つのケースで切り分けます。アイコンが背景に溶ける、Androidだけ背景色が効かない、画面遷移で点滅する、モーダルで色が戻るといった症状の対処とデバッグの順番を示します。"
...
スプラッシュ画面から本編に切り替わる瞬間の挙動が気になる場合は、[Rork のスプラッシュ画面が固まる・白フラッシュする問題の解決法](/articles/rork-dev/rork-splash-screen-stuck-white-flash-fix) もあわせて確認すると、起動時のちらつきまで一気に潰せます。