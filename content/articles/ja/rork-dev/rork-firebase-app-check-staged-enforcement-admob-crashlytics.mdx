...
ステージ 1 — `appCheckMode = "enforce"` だが、Remote Config の `appCheckRolloutPercent = 10` で 10% のユーザのみ enforce 対象とする。クライアント側で自身のユーザ ID を sha256 して下 2 桁を取り、`< 10` のユーザだけ Crashlytics と Realtime Database を実際に App Check 越しで使う実装にします。
