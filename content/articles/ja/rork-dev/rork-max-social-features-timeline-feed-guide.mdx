...
author: "Rork Lab"
description: "Rork Max + Supabase でフォロー/タイムライン/いいね・コメント/リアルタイム同期/プッシュ通知を本番品質で組み上げる手順を、累計5,000万DLの運用経験からまとめたガイドです。"
tags: ["rork-max", "social-app", "timeline", "follow", "realtime", "supabase", "feed"]
highlights:
  - "累計5,000万DLの運用で見えた、ソーシャル機能を「壊さずに伸ばす」設計判断（フォロー外混入・フォロワーキャッシュ・LWWで十分な領域）"
  - "Supabase Realtime が1万接続超で詰まる前に切り替える段階的フォールバック戦略と、リトライ・無効通知の抑制コード"
  - "プッシュ通知の「効く通知」だけを残すセグメンテーション基準（許可率・OPEN率の閾値と、誤送信時の収束手順）"
---

廣川政樹です。2013年から個人で iOS / Android アプリを運営しており、累計ダウンロードは5,000万を超えました。壁紙系・癒し系・引き寄せ系といった、いわゆる「静かな」ユーティリティを中心に育ててきましたが、ここ数年は同じユーザーが「他の人の投稿を見たい」「お気に入りを誰かと共有したい」と言ってくれる場面が増えてきました。
...
ただし、これらの巨大サービスを真似しようとすると、ほぼ確実に「全部入りで動きはするが、誰のフィードも汚い」という状態に陥ります。私が壁紙アプリで実際に経験したのも同じパターンでした。データベース設計・タイムラインの並び・通知の出し方、それぞれで「やらない判断」を最初に決めるのが、結果として一番ユーザーに親切な作りになります。
...
ここまでで「機能としてのソーシャルアプリ」は揃いました。ここからは、Supabase / FCM の公式ドキュメントには書かれていない、累計5,000万DLの運用を通じて私が拾った6つの判断材料を共有します。すべて、過去に失敗してから引き返した経験ベースの内容です。
...
ソーシャルアプリ機能を本番品質で組み上げるには、データベース・タイムライン・リアルタイム・通知のそれぞれで「**過剰に作らない判断**」が必要です。このガイドの実装パターンは、5,000万DLの運用で「やってよかったこと」と「やらなくてよかったこと」の両方を反映した、最小構成です。
...
[Designing Data-Intensive Applications（マーティン・クレップマン）](https://www.amazon.co.jp/dp/4873118700/?tag=pinocchio-22)
