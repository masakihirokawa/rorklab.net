...
highlights:
  ...
  - "4本のアプリを同時に新解像度へ寄せるときの定数同期スクリプトと、実機とシミュレーターの確認分担をそのまま流用できる"
---
...
個人開発で並行して面倒を見ているのは4本です——Beautiful HD Wallpapers、Ukiyo-e Wallpapers、Relaxing Healing、Law of Attraction Everyday。新機種が出るたびに解像度の確認が入るのは織り込み済みでしたが、2026年は iPhone Air という新しいフォームファクターが加わり、確認すべき組み合わせが一段増えました。

厄介なのは、崩れ方が派手ではないことです。クラッシュしてくれれば気づけます。数ポイントのはみ出しは、指摘されるまで静かに残ります。
...
実機を持っていない機種は、TestFlight のベータテスターに確認を頼むのが現実的です。Beautiful HD Wallpapers では日本語圏以外の利用も多く、英語・韓国語・スペイン語のユーザーから「この機種で余白がおかしい」という報告が先に届くことがあります。手元にない機種の目は、外から借りるしかありません。依頼するときは「どの画面を、どの向きで見てほしいか」を1〜2行に絞って伝えると、返ってくる情報の精度が上がります。

## 症状から原因へ — 崩れたときに見る順番

新機種で「なんとなくずれている」と感じたとき、闇雲にスタイルを触ると別の機種を壊します。私は次の順で切り分けています。

1. **上端または下端だけがずれている** → Safe Area の値を固定値で書いている箇所を疑います。`paddingTop: 44` のようなハードコードが残っていれば、`useSafeAreaInsets()` の戻り値に置き換えるのが確実な対処です。Dynamic Island 世代の実機で確認するまで解決とみなさないのが安全です。
2. **グリッドの列数や余白だけがおかしい** → 幅のブレークポイント判定が原因です。`width > 400` のような単一しきい値は iPhone Air（420pt）を Pro Max 側に分類してしまうため、機種カテゴリを定数化して回避します。しきい値を増やすより、`DeviceConfig.ts` に列数そのものを持たせるほうが後々崩れません。
3. **画像だけが見切れる・余白が出る** → アスペクト比の問題であり、レイアウトの問題ではありません。`resizeMode` を変えるとどちらかの機種で必ず破綻するため、フルスクリーン表示では「どの比率でも成立する構図」を素材側の要件にするのが根本的な解決です。

この3つで説明がつかない場合だけ、個別画面のスタイルを読みにいきます。順番を決めておくと、原因の切り分けにかかる時間が読めるようになります。
...
複数のアプリを並行して運用していると、何度でも同じ結論に戻ってきます。鍵は「変更の影響範囲をできるだけ小さくすること」。今回の解像度定数の一元管理も、その考え方の延長線上にあります。新機種のたびに触る箇所が1ファイルで済むなら、次の対応にかかる時間はほぼ読めるようになります。

## 毎年の解像度対応を「短くする」ための考え方

新機種対応をひととおり終えたあとに残るのは、達成感よりも「来年もまた同じことをやる」という感覚です。管理すべき解像度は、標準・Plus・Pro・Pro Max・Air と増え続けていて、減る見込みはありません。

ここで問うべきなのは「作業をなくせるか」ではなく「1回あたりを何分にできるか」だと考えています。解像度対応そのものは避けられません。避けられるのは、対応のたびにコードベース全体を探し回る時間のほうです。

判断の目安として、私は次の3点を見ています。新機種を1機種追加するとき、変更するファイルは何個になるか。変更後に目視確認が必要な画面は何画面か。その確認をシミュレーターだけで終えられるか。この3つの数字が小さいほど、次回の作業は短くなります。今回の対応前は「4本のアプリ × 十数ファイル」でしたが、`DeviceConfig.ts` に集約したあとは1ファイルの定数追加と、フルスクリーン系の3画面の目視だけで済むようになりました。

Rork でアプリを作ると、最初のバージョンは驚くほど早く形になります。そのぶん、長く運営するアプリでは「地味だが確実に必要な作業」をどれだけ短くできるかが差になります。書いた本人が「どこを変えれば何が変わるか」を即答できる構造にしておくこと。これが、翌年の自分への一番実用的な引き継ぎだと感じています。
...
繰り返し実感しているのは、「今の作業が楽になる構造」より「次の作業が楽になる構造」を選ぶほうが割に合う、ということです。解像度定数の一元管理は、作る当初にひと手間かかります。その手間は、新機種が出るたびに少しずつ返ってきます。アプリを長く運営するというのは、こうした地味なメンテナンスを毎年積み重ねることでもあります。その1回を軽くしておくことが、作りたいものに向ける時間を守ることにつながります。
