...
まず、CarPlay シーンから発火するイベントには必ず経路の属性を付けます。`didConnect` で接続フラグを立て、切断で戻す。これだけで、同じ「再生開始」イベントを車内と車外に切り分けられます。
...
3 つ目を分けている理由は、運転中の不具合が利用者に与える印象が、通常のクラッシュとは比べものにならないためです。画面を見られない状況で音が止まり、操作もできない。この体験は、そのままアンインストールにつながります。
