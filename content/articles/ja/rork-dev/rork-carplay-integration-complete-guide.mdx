---
title: "Rork アプリで CarPlay 対応を実装する — 音楽・ナビ・メッセージを車載で動かす設計と App Store 審査の勝ち方"
...
author: "Rork Lab"
description: "Rork で作ったアプリを CarPlay に対応させるまでの道筋を、エンタイトルメント申請の書き方から CPTemplate API の選び方、UIScene 設定、審査で見られた箇所まで、自分が詰まった順に書き残します。音楽・ナビ・メッセージのカテゴリ別の設計も添えました。"
...
CarPlay に対応した車種は、レンタカーを借りるたびに増えていると感じます。いっぽうで、実際にヘッドユニットの App Library を開いてみると、そこに並ぶサードパーティ製のアプリは数えるほどしかありません。実装の難しさが理由だと思い込んでいたのですが、周りの個人開発者と話すうちに、多くの方が「CarPlay は大手アプリだけのもの」と初めから見送っているのだと気づきました。
...
運転中の利用者は、通知に気を取られて別のアプリへ移ることがほとんどありません。再生を始めたら、目的地に着くまで開いたままになります。私が音源アプリの計測を見返していて意外だったのは、車内経由のセッションが長くなったことそのものよりも、その長さが日ごとにほとんどぶれないことでした。通勤という決まった時間に結びついているからかもしれません。
...
このコードで補っておきたい点が 2 つあります。

1 つは `synchronize()` を呼んでいないことです。手動の同期は以前から非推奨とされており、書き込みは適切なタイミングで反映されます。呼んでも害はないものの、「呼んだから確実に届いた」と思い込む材料になるだけで、実際の不整合はプロセス間の読み取り時点のほうにあります。

もう 1 つは、Swift のクラスを JavaScript 側から見えるようにする登録です。ここを忘れると `NativeModules.SharedStorage` が `undefined` になり、App Group 側ではなく JS 側で落ちます。ブリッジのヘッダに 1 行を置いてください。
...
また、CPListItem の画像にも上限があります。ここで私が最初に書いたのは、調べた数値をそのまま定数にしたコードでした。結果は芳しくありませんでした。上限はアプリのカテゴリによって変わるため、ある車では出ていた画像が別の車では出ない、という再現しづらい報告に追われることになったのです。

Apple は上限そのものを [`CPListItem.maximumImageSize`](https://developer.apple.com/documentation/carplay/cplistitem/maximumimagesize) として公開しております。ポイント単位の解像度非依存の値で、実行時のカテゴリに応じた値が返ります。数値を持つのではなく、問い合わせるほうが安全です。

```swift
extension UIImage {
    /// CarPlay のリスト用に、実行時の上限へ収めた画像を返す
...
上限を超えた画像は表示されないだけで、例外もログも出ません。気づきにくい落とし穴です。

もう一つ、静かに壊れるのがリストの件数です。`CPListTemplate` には表示できる項目数とセクション数の上限があり、超えた分は例外もログもなく単に切り捨てられます。上限値は OS のバージョンで変わりうるため、数値をコードに直書きせず、実行時に API から読んで丸める書き方をしております。

```swift
extension CPListTemplate {
    /// 実行時の上限値でセクションを丸めてから生成する
...
`maximumItemCount` はテンプレート全体での合計です。セクションごとに上限があると思い込んで実装すると、後ろのセクションだけが車内で消えるという再現しづらい不具合になります。私はここで一度、実機でしか起きない「最後の 1 カテゴリだけ表示されない」現象に丸一日を使いました。

もう一点、シミュレータで見落としがちなのが描画速度です。CarPlay ウィンドウは母艦の Mac の性能で動くため、リスト構築に多少の重さがあっても気づけません。実際のヘッドユニットは処理性能が控えめで、体感が明確に落ちます。私はテンプレート生成を 16ms 以内に収めることを目安にし、画像の読み込みは必ず非同期にしたうえで、プレースホルダを先に返す構成にしております。

切断時の挙動も、シミュレータで確かめられます。Xcode の CarPlay ウィンドウを閉じると `sceneDidDisconnect` が呼ばれるため、再生中に閉じてから開き直す往復を、リリース前に必ず数回通しておくと安心です。審査でもこの流れを見られることがあります。
...
2 つ目は「CarPlay Simulator では動くが、実機で動かない」問題です。私の場合、原因はエンタイトルメントの入り方でした。開発用のプロビジョニングプロファイルには CarPlay capability が入っているのに、配布用のプロファイルを再生成しておらず、アーカイブしたビルドだけがエンタイトルメントなしで出ていたのです。ビルド後の `.app` に対して `codesign -d --entitlements :- YourApp.app` を実行し、カテゴリに応じたキーが実際に埋まっているかを目で確かめる習慣にしております。シミュレータは署名を見ないため、ここだけはどうしても抜け落ちます。
...
5 つ目は「App Store 審査で『CarPlay のカテゴリ適合性が不十分』とリジェクトされる」問題です。審査官はエンタイトルメント申請時よりさらに厳しくカテゴリ適合性を見ます。TestFlight で Apple 社員が実際に車内でテストするケースもあるため、運転中の実用性を動画で示すなどの追加資料を用意しておくと通りやすくなります。
...
リリース後は、CarPlay 対応アプリ特有のメトリクス（CarPlay 経由のセッション時間、連続再生時間、運転中のクラッシュ率）を App Store Connect で別途追跡すると改善ポイントが見えてきます。詳細な設計論は [Rork で本格的な iOS アプリを構築する実践ガイド](/articles/app-dev/rork-ios-app-production-guide) や [Rork Max で App Store 審査を一発通過するノウハウ集](/articles/rork-business/rork-max-app-store-review-guide) も参考にしてください。

## リリース後の 30 日 — CarPlay を「別のアプリ」として計測する

CarPlay 対応は、出した瞬間がゴールではありません。むしろ最初のひと月で計測の土台を作れるかどうかが、その後の判断の精度を決めます。iPhone 側と同じ指標をまとめて見てしまうと、CarPlay の効果はほぼ確実に埋もれます。

### 経路を分けてイベントを送る

まず、CarPlay シーンから発火するイベントには必ず経路の属性を付けます。`didConnect` で接続フラグを立て、切断で元に戻します。これだけで、同じ「再生開始」イベントを車内と車外に切り分けられます。
...
送信側では `surface` を全イベントの共通パラメータに載せます。後から分けようとすると過去分が復元できないため、ここは公開前に済ませておくのが確実です。

### 最初のひと月で見る 3 つの数字

見る指標は絞ったほうが判断できます。私は次の 3 つに限定しております。

1. CarPlay 経由の 1 セッションあたり連続利用時間。車内は中断が入りにくいため、iPhone 側より明確に長くなるはずです。ここが伸びていなければ、階層が深すぎて運転中に操作できていない可能性が高いです。
2. CarPlay を一度でも使った利用者の 7 日継続率と、使っていない利用者の 7 日継続率の差。在籍期間を揃えて比べます。CarPlay を投資として説明できるかどうかは、ほぼこの差で決まります。
3. CarPlay シーンで発生したクラッシュの件数。全体のクラッシュ率に混ぜず、単独で追います。

### 車内のクラッシュは重みが違う

3 つ目を分けている理由は、運転中の不具合が利用者に与える印象が、通常のクラッシュとは比べものにならないためです。画面を見られない状況で音が止まり、操作もできません。この体験は、そのままアンインストールにつながります。

私は CarPlay 由来のクラッシュだけは件数にかかわらず最優先で潰す運用にしております。頻度で優先度を決める通常のルールを、ここだけは適用しません。

## 最初の一歩 — カテゴリを 1 つ選んで、申請文を書いてみる

私は長いあいだ、CarPlay 対応を実装の問題だと思っておりました。いま思えば、詰まっていたのはいつも実装より前の部分でした。どのカテゴリで出すのかが決まらないまま手を動かしていたので、テンプレートの選び方も、審査で聞かれる観点も、そのたびに揺れていたのです。

ですから最初の一歩は、コードではなく [Apple の CarPlay 開発者ページ](https://developer.apple.com/carplay/) を開くことをおすすめします。カテゴリ一覧から自分のアプリにいちばん近いものを 1 つだけ選び、「運転中の必然性」「安全性への配慮」「カテゴリ適合性」の 3 点を、500 字ほどで書いてみてください。書けたなら、実装は待っていてくれます。書けなかったなら、足りないのはコードではなく、運転中にしか生まれない価値のほうだと思います。

**申請文が一段落で書けない機能は、まだ車内へ出す段階ではありません。** この線引きだけは、実装が先に動いてしまった日でも崩さないようにしております。

最後までお読みくださり、ありがとうございました。車内という文脈は、私にとってもまだ手探りの領域です。気づいたことがあれば、また書き残してまいります。