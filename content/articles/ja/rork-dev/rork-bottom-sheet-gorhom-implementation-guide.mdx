...
私自身、自分のアプリに「2段階で開くシート」を入れたかったのですが、Rork が出してくれた `Modal` ベースのコードでは慣性スクロールが効かず、Android で背景が真っ黒に光る問題に半日悩みました。結論としては [@gorhom/bottom-sheet](https://github.com/gorhom/react-native-bottom-sheet) を後から差し込むのが一番素直で、Rork の出力をほとんど壊さずに導入できます。ここではその手順とつまずきやすいポイントをまとめておきます。
