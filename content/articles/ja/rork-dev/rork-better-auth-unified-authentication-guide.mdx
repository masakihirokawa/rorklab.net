---
title: "Rork × Better Auth で Web/モバイル統合認証を実装する完全ガイド — Cookie・Deep Link・パスキーまで"
...
この記事では、Rork で作ったモバイルアプリ（React Native + Expo）と Next.js の管理画面を、ひとつの Better Auth サーバーに統合する手順を、私が本番で踏んだ落とし穴と一緒にまとめます。コード例はすべて、執筆時点の Better Auth 1.2 系で動作確認済みです。
...
技術書籍で TypeScript ファーストの認証設計をさらに学びたい方には、[達人が教える Web パフォーマンスチューニング 〜ISUCON から学ぶ高速化の実践〜（藤原俊一郎ほか）](https://www.amazon.co.jp/dp/B09NTLXJTV/?tag=pinocchio-22) の 4 章がセッション設計の前提知識として参考になります。Better Auth 自体ではなく「セッションをどう設計するか」の判断軸が学べる一冊です。
