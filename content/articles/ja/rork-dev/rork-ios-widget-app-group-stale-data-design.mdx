...
highlights:
  - "ウィジェットが親アプリのデータを読めず詰まっていた人が、App Groupの発行からタイムライン更新までを今日動く形で手に入れられる"
  - "UserDefaults共有コンテナとFile system共有の使い分けを、自分のアプリの構造に合わせて判断できるようになる"
  - "起動直後にウィジェットだけ古い情報を出す現象を、設計レベルで再発させない構造を組み立てられる"
---

壁紙アプリの月次アップデートを配信したあと、ホーム画面ウィジェットだけが「先月のデイリー壁紙」を表示し続けたことがありました。アプリを開けば最新画像は更新されているのに、ウィジェットは数時間どころか丸一日前のサムネイルを出している。エンドユーザーから「古いままなんですが」とフィードバックが届いて、その晩は寝るどころではなくなりました。
