...
author: "Rork Lab"
description: "Rork Max で iOS Widget・ロック画面ウィジェット・Dynamic Island 対応のライブアクティビティを実装した記録。Timeline の焼き込み設計、APNs のヘッダーと優先度、push-to-start、stale-date の扱いまで実運用の判断を残します。"
...
highlights:
  ...
  - "APNs のヘッダー・優先度5と10の使い分け・push-to-start・stale-date まで含めて、サーバー主導の更新を設計できる"
...
個人開発で運用している壁紙アプリに「今日の一枚」を映すウィジェットを載せた夜、私が最初に面食らったのは、更新のされなさでした。`getTimeline` で数分おきに新しい画像へ差し替えるつもりでいたのですが、iOS は省電力のために再読み込みをかなり間引きます。翌朝ホーム画面を見ると、前日の絵柄がそのまま残っておりました。

そこで方針を変えました。一日分のエントリをあらかじめ `Timeline` に積んでおき、深夜0時に一度だけ全体を組み直す設計です。リクエストの回数は最小限に収まり、それでいてユーザーから見れば毎朝きちんと絵柄が変わります。この「先に焼き込む」考え方にたどり着けたのは、実機のホーム画面を何日も眺めたあとでした。いま思えば、Widget を本体アプリの縮小版として捉えていたことが遠回りの原因だったのかもしれません。

**Widget は「呼ばれたときに作る画面」ではなく、「先に用意しておく紙芝居」です。** この見方に切り替えてから、更新まわりで手が止まる時間がはっきり減りました。

Rork Max はネイティブ Swift コードを生成できる AI アシスト開発環境です。WidgetKit・ActivityKit の実装もプロンプトドリブンで進められますが、SwiftUI の仕組みと Widget 固有の制約を押さえていないと、出力されたコードに誤りが混ざっていても気づけません——そして Widget の誤りは、クラッシュではなく「何も表示されない空白」として現れるぶん、なおさら見つけにくいのです。以降の各セクションでは、実装の原理と、実運用でつまずいた箇所の手触りを並べてお伝えします。
...
ロック画面ウィジェットは **モノクロ表示**（システムが色を管理）である点に、先に気を配っておきたいところです。`@Environment(\.widgetRenderingMode)` で現在の描画モードを取得し、フルカラー・ビブラント・アクセントカラーを適切に使い分けることができます。
...
`Info.plist` に `NSSupportsLiveActivities` キーが `YES` で追加されているかを、まず確認します。ここで一つ訂正しておきたいことがあります。私は長らく「ライブアクティビティは実機でしか確認できない」と思い込んでおりました。実際にはシミュレーターでも表示できますし、ActivityKit のプッシュ通知の検証も、Apple T2 セキュリティチップまたは Apple シリコンを積んだ Mac で macOS 13 以降であれば、シミュレーター上で行えます。この条件は [Starting and updating Live Activities with ActivityKit push notifications](https://developer.apple.com/documentation/activitykit/starting-and-updating-live-activities-with-activitykit-push-notifications) に明記されています。実機を1台占有せずに済むと分かってから、検証のテンポがずいぶん変わりました。

**「Compact/Minimal が正しく表示されない」問題。**
Dynamic Island の Compact 表示は iPhone 14 Pro 以降の機種でしか確認できません。Xcode のシミュレーターで「iPhone 15 Pro」以降を選んで確かめます。
...
OS のバッテリー最適化により、更新間隔は保証されません。低電力モード時や特定の条件下では更新が大きく遅れることがあります。確実に届けたい更新がある場合は、次の第8節で扱う APNs 経由の更新に寄せるほうが安全です。
...
func startActivityWithPushToken(orderId: String) throws {
    let activity = try Activity<DeliveryAttributes>.request(
        attributes: DeliveryAttributes(orderId: orderId, storeName: "カフェ", customerName: "受取人"),
        content: .init(state: .init(status: .preparing, estimatedMinutes: 30, currentLocation: "準備中"),
                       staleDate: Date().addingTimeInterval(15 * 60)),
        pushType: .token  // Push更新を有効化
    )

    // pushTokenUpdates は終端しない非同期シーケンスです。
    // 呼び出し元で await すると、この関数から戻ってこなくなります。
    Task {
        for await data in activity.pushTokenUpdates {
            let token = data.map { String(format: "%02x", $0) }.joined()
            // 新しいトークンを受け取ったら、サーバー側で古いトークンを無効化します
...
`pushTokenUpdates` を `await` の直列に置いてしまう書き方は、私も一度やりました。ライブアクティビティ自体は開始されるのに、呼び出し元の処理がその先へ進まず、「開始ボタンを押すと画面が固まる」という形で表面化します。トークンはこのシーケンスから何度でも届きますので、監視は独立した `Task` に逃がし、届いたら古いトークンをサーバー側で無効化する——この2点はセットで考えておくと安全です。
...
ペイロードの形だけ整えても、ヘッダーが揃っていなければ APNs は受け付けてくれません。ここでつまずくと原因が見えにくいので、必要なヘッダーを先に並べておきます。

<table>
<thead>
<tr><th>ヘッダー</th><th>指定する値</th><th>補足</th></tr>
</thead>
<tbody>
<tr><td><code>apns-push-type</code></td><td><code>liveactivity</code></td><td>通常のリモート通知の値とは別物です</td></tr>
<tr><td><code>apns-topic</code></td><td><code>&lt;バンドルID&gt;.push-type.liveactivity</code></td><td>バンドルIDだけでは通りません</td></tr>
<tr><td><code>apns-priority</code></td><td><code>5</code> または <code>10</code></td><td>省略時は <code>10</code> 扱いになります</td></tr>
<tr><td><code>apns-channel-id</code></td><td>チャンネルID</td><td>ブロードキャストで更新・終了する場合のみ</td></tr>
</tbody>
</table>

**更新頻度は「10 を使わない設計」から考えます。** ライブアクティビティ向けのプッシュには時間あたりの予算があり、`apns-priority: 10` はこの予算を消費します。予算を超えるとスロットリングの対象になり、届いてほしい場面ほど届かなくなります。一方 `apns-priority: 5` は予算を消費しません。配達アプリであれば、1分ごとの位置更新は `5`、「配達員が到着しました」だけ `10` という具合に、混ぜて使うのが素直です。

スポーツの実況のように、そもそも予算に収まらない用途もあります。その場合は `Info.plist` に `NSSupportsLiveActivitiesFrequentUpdates` を `YES` で追加すると、高頻度の更新が許されます。ただしこれは利用者が「設定」アプリからオフにできる項目です。アプリ側は `ActivityAuthorizationInfo().frequentPushesEnabled` で現在の値を読み、`frequentPushEnablementUpdates` で変化を監視して、オフになっていればサーバーへ伝えて送信頻度を落とす——この往復まで含めて初めて設計が閉じます。

### アプリを開かせずにライブアクティビティを開始する

iOS 17.2 以降は、ユーザーがアプリを起動しなくてもサーバー側からライブアクティビティを開始できます。`Activity<DeliveryAttributes>.pushToStartTokenUpdates` を購読して push-to-start トークンを受け取り、サーバーへ預けておく形です。開始のためにアプリ内で一度ライブアクティビティを走らせておく必要はありません。

```swift
// 起動時に一度だけ購読しておく（開始用トークン・iOS 17.2 以降）
...
開始用のペイロードは更新用と似ていますが、`event` が `start` になり、`attributes-type` と `attributes`、そして `alert` が必須になります。`alert` が必須なのは、身に覚えのない表示が突然ロック画面に現れる事態を避けるためです。
...
```json
{
  "aps": {
    ...
    "attributes": { "orderId": "A-1042", "storeName": "カフェ", "customerName": "受取人" },
    "content-state": { "status": "preparing", "estimatedMinutes": 30, "currentLocation": "準備中" },
    "input-push-token": 1,
    "alert": { "title": "注文を受け付けました", "body": "準備を始めています" }
  }
}
```

`input-push-token: 1` は iOS 18 以降で効くキーで、開始と同時に更新用のプッシュトークンを受け取る指定です。チャンネル経由で更新したい場合は代わりに `input-push-channel` にチャンネルIDを渡します。なお、ブロードキャスト（チャンネル）でライブアクティビティを**開始**することはできません。開始は push-to-start トークン、以降の更新はチャンネル、という組み合わせで考えます。

### 届かなかった時間をどう見せるか

私がいちばん時間をかけたのは、更新が成功する経路ではなく、届かなかったときの見え方でした。地下やトンネルで通信が切れれば、ライブアクティビティは古い値を表示し続けます。ここで効くのが `stale-date` です。ペイロードに未来の時刻を入れておくと、その時刻を過ぎた時点で `activityState` が `.stale` へ変わります。アプリ側は `activityStateUpdates` を監視しておき、Widget 側では「最終更新から時間が経っています」と正直に出すようにしています。古い数字を出し続けるより、こちらのほうが信頼を損ないません。

終了後の後片付けにも既定値があります。ライブアクティビティを終了しても、ロック画面には最大4時間そのまま残ります。すぐ消したい場合は `dismissal-date` に過去の時刻を、少しだけ残したい場合は4時間以内の時刻を指定します。

複数のライブアクティビティが同時に走る場合は `relevance-score` を添えます。指定がなければ最初に開始したものが Dynamic Island に出るため、後から始まったより重要な進行が埋もれます。重要な更新に `100`、そうでないものに `50` といった相対値を割り振り、どの値を誰に渡したかを自分の側でも控えておくと、順序の入れ替えに困りません。

**サーバー更新の設計は、通信が成功した前提を捨てるところから始まります。** ヘッダー・優先度・staleDate の3つを最初に決めてしまえば、あとの実装は素直に進みます。
...
ローカライズで見落としがちなのは、文字列リソースの置き場所です。Widget Extension は本体アプリとは別バンドルとして扱われるため、`Localizable.strings` を本体側だけに置いても参照されません。Extension のターゲットにも追加しておきます。
...
**`getTimeline` には重い処理を置かないようにしています。** 並べ替え・絞り込み・整形は、本体アプリ側で済ませて共有ストレージに書き出しておきます。Provider の仕事は読み出しだけに留めるのが基本です。

**画像はネットワークから取らないようにしています。** `getTimeline` の中で通信を始めると、完了を待たずに Extension が打ち切られます。本体アプリで先に取得し、App Group のコンテナに保存したものを読むだけにします。

**エントリはまとめて焼き込んでおきます。** 予定表や日替わりコンテンツのように未来の値が分かっているものは、先の分まで一度に積んでおきます。OS が Extension を起こす回数そのものを減らせます。
...
**ビューは素朴に保ちます。** ウィジェットはスナップショットとして描かれます。込み入ったアニメーションや深い入れ子は描画時間を押し上げるだけで、見返りがありません。iOS 17 の `Button(intent:)` を除けば、状態を持たない静的な描画として設計するのが正解です。
...
作り始めるなら、いきなり Dynamic Island を目指さないことをおすすめします。共有ストレージから値をひとつ読むだけの Small ウィジェットを先に通し、データが確かに流れることを確認してから、ロック画面、そしてライブアクティビティへと積み上げていきます。この順番なら、うまくいかないときに疑う場所がいつも一箇所に絞られます。
