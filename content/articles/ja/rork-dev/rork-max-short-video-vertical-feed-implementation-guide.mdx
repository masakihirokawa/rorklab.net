...
highlights:
  - "Rork Maxに頼んで最初に出力された動画コードが使い物にならなかった理由と、段階的なプロンプト設計で本番品質に持ち込むまでの試行錯誤を公開する"
  - "カメラ撮影・Supabase Storageへのバックグラウンドアップロード・FlatList縦型スクロールの3つの実装を、そのままコピーして動くコード付きで解説する"
  - "累計5,000万DLのアプリ開発経験から導いた、動画フィードでユーザーが離脱しない3つのUX設計パターンを具体的な実装と紐付けて説明する"
---
...
この記事では、そうした問題にぶつかりながら実際に動くショート動画フィードを完成させた経験から、Rork Maxへの効果的なプロンプト戦略と、そのまま使える実装パターンをまとめます。
