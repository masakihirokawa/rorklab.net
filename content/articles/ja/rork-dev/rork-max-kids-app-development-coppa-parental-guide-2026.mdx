---
title: "Rork Max で子ども向けアプリを作る — App Store Kids カテゴリ審査・COPPA対応・ペアレンタルゲートの完全実装ガイド"
...
highlights:
  - "App Store Kids カテゴリ審査でリジェクトされ続けていた人が、ペアレンタルゲート・外部リンク禁止・第三者SDK制限の実装チェックリストを手に入れ、審査を通過できるようになる"
  - "COPPA の法的要件を React Native / Expo の動作確認済みコードとして今日から実装できる（年齢確認・保護者同意フロー・データ収集制限）"
  - "子ども向けアプリで実際に収益化するための、Ask to Buy 対応と保護者向け課金 UI 設計、COPPA 準拠 AdMob 設定を習得できる"
---
...
保留中の Ask to Buy がある場合のUI設計では、**子どもに失望させない**ことが重要です。「購入できませんでした」ではなく「お父さんお母さんに確認をお願いしました。承認されたら使えるようになります」という表現が体験として自然です。
...
対処法: すべての外部リンク・課金導線・他アプリ誘導に本記事のペアレンタルゲートを実装する。審査前に「外部へのリンクがある場所を全て列挙する」レビューを必ず実施してください。一つ見落とすだけでリジェクトされます。
...
COPPA 準拠の実装パターンについては、[実践 React Native アプリ開発（ソシム）](https://www.amazon.co.jp/dp/4802614268/?tag=pinocchio-22)が子ども向けアプリに限らず、プライバシー設計全般の参考になります。設計の考え方をさらに深めたい方にはおすすめです。
