---
title: "Rork アプリで広告アトリビューションを正確に測る実装 — AppsFlyer・Adjust・SKAdNetwork 4.0・ATT 統合"
...
---
私が iOS 14.5 のアップデート後に最初に痛感したのは、「広告を打っても、どのキャンペーンが効いているのか以前のように見えない」という事実でした。前まではざっくり眺めていれば成果が分かっていたのに、ある日を境に管理画面のインストール数が半分になり、CPA が二倍に膨らんだように見えました。実際にはユーザーが減ったわけではなく、IDFA を取得できないことで計測がズレているだけだったのですが、原因に辿り着くまでに数週間を消費しました。
...
ここで、私が最初に書いて長らく気づけなかった実装を先にお見せします。読み飛ばしたくなるくらい素直なコードですが、この形には二つの穴が空いています。

```typescript
// ❌ 私が最初に書いた版。動くように見えて、二か所で計測を失います
...
一つ目は、`cumulativeScore` がプロセス内の変数だという点です。アプリを一度落として翌日に開き直すと 0 に戻りますので、そこから積み上げ直した値は前回の到達点より低くなります。SKAN は単調増加でしか値を受け取らないため、この低い値は黙って捨てられます。エラーも警告も出ません。オンボーディング完了までは正しく、課金トライアルの 32 点だけが永遠に届かない——という歪み方をします。

二つ目は、`fine` しか更新していない点です。SKAN 4.0 では `fine` を運べるのは最初のポストバック（インストール後 2 日まで）だけで、2 回目と 3 回目は `coarse` しか運べません。`coarse` を一度も設定していなければ、後続のポストバックには `coarse: none` が入って返ってきます。ダッシュボードで「window 1 以降が空っぽ」に見える原因の多くはここです。`coarse` の設計を熱心に語っておきながら、コードが SKAN 3.0 のままだった、というのが当時の私でした。

**設計は 4.0、実装は 3.0——このズレは、レポートが空になるまで誰も教えてくれません。**
...
// fine の到達点から coarse を決めます。しきい値は自分の LTV 分布に合わせて調整してください
function toCoarse(fine: number): CoarseValue {
  if (fine >= 32) return 'high';   // 課金トライアル到達
  if (fine >= 8) return 'medium';  // 初回コンテンツ閲覧以降
  return 'low';
}
...
export async function reportEvent(event: Event, payload?: Record<string, unknown>): Promise<void> {
  ...
  // 2. 累積スコアはストレージから読み直す（プロセス内変数だと再起動で 0 に戻ります）
  ...
  // 3. fine と coarse を同時に更新する。トライアル到達時は window を閉じて即座に送出させる
...
`updateSkanConversionValue` の中身は、iOS 16.1 以降の `SKAdNetwork.updatePostbackConversionValue(_:coarseValue:lockWindow:)` を呼ぶだけの薄いブリッジです。Expo Modules API で 30 行ほど書けば済みますので、MMP の SDK が対応するのを待つ必要はありません。

```swift
// modules/skan/ios/SkanModule.swift — Expo Modules API の薄いブリッジ
...
public class SkanModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("update") { (fine: Int, coarse: String, lockWindow: Bool, promise: Promise) in
      // iOS 16.1 未満は coarse の概念そのものが無いため fine だけを送ります
...
一点だけ線引きが必要です。AppsFlyer の SKAN Conversion Studio のようにサーバ側で Conversion Value を決める仕組みを有効にしている場合、アプリ側からの自前更新と二重に走らせてはいけません。どちらが最後に書いたかで値が決まってしまい、原因の切り分けが難しくなります。**Conversion Value を書く主体は、アプリかサーバのどちらか一方に必ず寄せます。** 私は自前で持つ側を選んでいますが、媒体を増やすほどサーバ側に寄せる利点が効いてきますので、規模が変わったら見直すつもりでおります。

`lockWindow` を `true` にすると、その window の残り時間を待たずにポストバックが送出されます。トライアル開始のように「これ以上スコアが動かない」と分かる地点でだけ立てるのが要点で、闇雲に立てると後続イベントの計測機会を捨てることになります。

期待する出力: AppsFlyer のダッシュボードで `Cohort Report → SKAN Postbacks` を開くと、postback window 0 に `fine` の分布が、window 1 と 2 に `coarse` の low / medium / high の分布が現れます。window 1 以降が `none` で埋まっているうちは、まだ `coarse` が届いていない証拠です。
...
ディープリンクの基礎については[Rork Max でのディープリンク・ユニバーサルリンク実装](/articles/rork-dev/rork-max-deep-linking-universal-links)で図解しているので、Universal Links の `apple-app-site-association` ファイル設置でつまずいている方はそちらも参照してください。
...
7. **累積スコアをメモリだけで持っている**: 前述のとおり、アプリを再起動すると 0 に戻り、以降の更新が単調増加の制約で捨てられます。手元で再現するなら、オンボーディング完了まで進めてからアプリを完全終了し、再起動して課金トライアルまで進めてください。ポストバックの `fine` が 4 のまま止まっていれば、この穴に落ちています。
...
- **TikTok Ads**: 認知獲得に強く、`fine` の低スコア帯（起動・オンボーディング完了）でターゲティング最適化が効きやすい媒体です。クリエイティブの賞味期限が短く 2〜3 週間で枯れますので、`coarse` まで見て高 LTV のクリエイティブだけを残す運用が向いています。
- **Meta Ads**: 高 LTV ユーザーの獲得に強く、`coarse` の high レンジで配信最適化を回すと CPI は上がるものの ROAS は伸びる傾向があります。Advantage+ App Campaigns と SKAN 4.0 の組み合わせは相性が良いと感じています。
- **Apple Search Ads**: 検索意図が明確なユーザーが入ってきますので、課金率と継続率がもっとも高くなります。Apple Search Ads は SKAN ポストバックを直接受け取りますので、AppsFlyer 経由でも独自の Search Ads Attribution API を併用すると精度が上がります。
...
```typescript
// src/lib/attribution-adjust.ts — react-native-adjust v5 系
...
export function initAdjust(appToken: string) {
  ...
  // v4 では Adjust.create(config) でした。v5 で initSdk に変わっています
...
ここで一度つまずいたのが SDK のメジャーバージョンでした。react-native-adjust は v5 で初期化のエントリポイントが変わり、v4 までの `Adjust.create(config)` は `Adjust.initSdk(config)` になっています。古い記事やサンプルをそのまま写すと、`Adjust.create is not a function` で起動直後に落ちますので、まず `package.json` の系列を確かめてください。あわせて、セッション単位のコールバックパラメータも `addSessionCallbackParameter` から `addGlobalCallbackParameter` へ改名されています（イベント単位の `addCallbackParameter` はそのままです）。移行の詳細は [Adjust 公式の SDK v5 移行ガイド](https://dev.adjust.com/en/sdk/migration/react-native/v4-to-v5/) に一覧があります。

私の経験上、AppsFlyer から Adjust への移行時に最も詰まりやすいのは「ATT 同意フローを再実装する手間」です。両 SDK ともネイティブの ATT API をラップしてはいますが、初期化順序の最適解が微妙に違います。Adjust の場合は `Adjust.initSdk` を ATT 確定前に呼んでも内部で待機してくれるので、起動時の体感速度が AppsFlyer より一段速く感じられます。
...
// リスナーはリニューアルでも発火します。同じイベントを二度送らないための門番です
...
export function bindRevenueCatToSKAN() {
  Purchases.addCustomerInfoUpdateListener((info: CustomerInfo) => {
    ...
    // periodType は 'TRIAL' / 'NORMAL' / 'INTRO' を返します。大文字小文字の取り違えに注意
...
先ほどの版では、トライアルと本課金の分岐が両方とも `trial_started` を送っていました。コメントには「本課金は最大値にクランプする」と書きながら、実際には同じイベントが飛んでいたわけです。SKAN の単調増加の制約に守られて例外もエラーも出ないため、レポートの `high` がいつまでも増えないという形でしか現れません。分岐を書いたら、両方の枝を声に出して読み上げる——これは私が自分に課している確認です。

もう一点、`customerInfo` は購読の更新（リニューアル）でも発火します。毎回送っても単調増加の制約で無視されるだけなので実害は小さいのですが、ダッシュボードのイベント数だけが膨らみ、本当の初回課金がどこだったのか読めなくなります。上のように送信済みフラグを永続化しておくと、後からログを追うときに助かります。`paid_started` を `SCORE_MAP` に追加するのを忘れないでください。
