...
先に、移行に関わる話をひとつ。SDK 54 で `expo-file-system` のデフォルトエクスポートが新しいオブジェクト指向 API に差し替わり、これまでの `documentDirectory` や `createDownloadResumable` は `expo-file-system/legacy` へ移されました（[Expo Blog: Expo File System gets a major upgrade in SDK 54](https://expo.dev/blog/expo-file-system)）。
