---
title: "Rork でポッドキャストアプリを作る完全実装ガイド：バックグラウンド再生・RSS 取得・オフラインダウンロードを本番レベルで動かす"
...
highlights:
  - "expo-av のバックグラウンドモードで詰まっていた開発者が、iOS/Android 両方で確実に動く実装パターンを今日手に入れられる"
  - "RSS フィード解析からオフラインダウンロード・ロック画面対応まで、コピー&ペーストで動く完全なコード例を習得できる"
  - "本番リリース済みポッドキャストアプリと同等の機能を Rork だけで再現し、App Store・Google Play 審査を通過できる"
---
...
壁紙アプリや健康系アプリを長年作ってきた経験のなかで、バックグラウンド再生まわりの設定ミスが原因で TestFlight のフィードバックに「画面を消すと止まる」という報告を受けたことがあります。原因は些細な設定漏れでした。この記事では、同じ轍を踏まないための実装パターンを、動作するコードとともにお伝えします。
...
音声アプリ開発をさらに深く学びたい方には、[「詳解 Swift」（著: 荻原 剛志）](https://www.amazon.co.jp/dp/4797395079/?tag=pinocchio-22)が参考になります。AVFoundation の章では iOS ネイティブレイヤーでの音声制御の仕組みが詳しく解説されており、expo-av の動作原理を理解する助けになります。
