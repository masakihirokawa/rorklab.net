...
EAS Update（Expoの無線更新基盤）はとても優秀ですが、デフォルトの使い方は「ブランチにpublishしたら、そのチャンネルを見ている端末が次回起動時に取得する」というシンプルなものです。これは小規模なうちは十分でも、ユーザー数が増えると「いきなり全員」がリスクそのものになります。この記事では、その即時全量配信を、5%→25%→100%の段階配信に作り替え、さらに壊れたバンドルを掴んだ端末を自動で前バージョンへ戻すところまでを、実際に運用しているコードで共有します。
...
## 5%→25%→100% を rollout で実装する

EAS Updateには `--rollout-percentage` があり、同一チャンネル内で「新ブランチを何%の端末に見せるか」を制御できます。私の運用では、新バンドルをまず5%に出し、クラッシュ率とadoption率を見ながら段階的に引き上げます。
...
私が昇格の前に必ず見る数字は二つです。ひとつは**adoption率**（その新バンドルを実際に取得・適用した端末の割合）。配信から24時間で、ロールアウト対象の70%以上が新バンドルを掴んでいなければ、配信経路かバンドルサイズに問題があると見ます。もうひとつは**新旧のクラッシュ率の差**で、新バンドル側のクラッシュ率が旧バンドルより20%以上悪化していたら昇格しません。5,000万DL規模だと5%でも数十万台あり、統計的に十分な母数が24時間で揃うため、この粒度で判断できます。逆にユーザー数が少ないアプリなら、5%ではサンプルが足りないので、最初の段は20%程度から始めることを勧めます。
...
この仕組みの肝は、`markBootHealthy()` を呼ぶ場所を「アプリが実用的に動き始めた地点」に置くことです。`App` のマウント直後ではなく、ナビゲーションの `onReady` や最初の画面のデータ取得成功後に置くことで、「JSは起動したがレンダリングで即死する」タイプのバグも捕捉できます。公式ドキュメントは `expo-updates` のAPI自体はよく説明していますが、この「正常起動の定義をどこに置くか」は運用で決めるしかない部分で、私は実害が出てからここを描画完了地点まで深くしました。

## 配信後72時間の監視テーブル

段階配信は「出して終わり」ではなく、各段で次の判断をするための監視がセットです。私が実際に使っている閾値を共有します。

- 配信〜6時間: 新バンドルのクラッシュ率を最優先で監視します。旧バンドル比で +20% を超えたら即、チャンネルの向き先を旧ブランチへ戻します（昇格はしません）。
- 6〜24時間: adoption率を確認します。ロールアウト対象の70%未満なら、バンドルサイズ（差分が大きすぎないか）と配信CDNを疑います。
