...
author: "Rork Lab"
description: "最低サポート OS の引き上げを利用率で判断すると、毎年同じ結論になって先送りが続きます。OS 分岐に注記を付けて棚卸しし、iOS と Android の引き上げ先ごとに消せる分岐を数えるスクリプトと、引き上げ後に見る指標をまとめました。"
tags: ["Rork", "Expo", "React Native", "保守設計", "iOS", "長期運用"]
highlights:
  - "OS 分岐を supportMatrix.ts と @drop-when 注記に寄せ、iOS と Android の引き上げ先ごとに「消せる分岐が何箇所か」を数える棚卸しスクリプト全文"
...
それなのに、結論だけが毎年同じでした。「まだ切れないな」と打ち込んで、シートを閉じておりました。
...
それならば、その数を直接数えればよいのだと思い至りました。利用率の代わりに、そちらを判断の材料にしています。そう考えて作ったのが、これから紹介する仕組みです。
...
処理そのものは軽いものです。1,000 ファイル・注記 2,000 箇所のツリーを用意して 3 回続けて計測したところ、実行時間は 0.08 秒、0.09 秒、0.09 秒でした。プリコミットフックに入れても体感には出ません。

## 「消し忘れ」を CI で落とす

このスクリプトで実際にいちばん効いたのは、引き上げ先の集計ではなく、消し忘れの検出のほうでした。

下限を 16.0 に上げたのに、`ios>=15.0` の注記が付いた分岐がコードに残っています。これは単なる死んだコードです。読む人の時間を奪い、改修のたびに「こちらの経路も確認しなければ」と手を止めさせます。
...
注記のない野良の分岐は、警告として出すだけで失敗にはしていません。ここを失敗にすると、既存コードを一気に注記だらけにする作業が発生して、導入そのものが止まります。まず新しく書く分岐から規約に乗せて、古いものは触ったときに直します。この順番でないと続きませんでした。
...
## Android の下限も、同じ台帳に載せる

ここまでの仕組みを半年ほど回したあとで、恥ずかしい抜けに気づきました。

`supportMatrix.ts` には最初から `android: 31` と書いてありました。それなのに sweep は `ios` の行しか読んでいません。つまり Android の下限だけは、私の頭の中と、Google Play Console の画面と、`build.gradle` に相当するビルド設定という三箇所に散らばったまま放置されていたのです。

iOS 側で「二重管理をやめる」と決めたその同じ記事の中で、もう片方の OS を数えていませんでした。**片方だけ棚卸しをする仕組みは、棚卸しをしていない側の負債を見えなくします。**

直し方は難しくありません。注記の書式にプラットフォームを含めて、下限の読み取りとバケツを 2 つに分けるだけです。

```js
// 変更点1: 注記と呼び出しの正規表現にプラットフォームを含める
const TAG  = /@drop-when\s+(ios|android)>=\s*([0-9]+(?:\.[0-9]+){0,2})(?:\s*--\s*(.*))?/;
const CALL = /atLeast(IOS|Android)\(\s*['"]?([0-9]+(?:\.[0-9]+){0,2})['"]?\s*\)/;

// 変更点2: 下限を両方読む（Android は API レベルなので整数）
function readMin(path) {
  const src = readFileSync(path, 'utf8');
  const mi = src.match(/ios\s*:\s*['"]([0-9][0-9.]*)['"]/);
  const ma = src.match(/android\s*:\s*['"]?([0-9]+)['"]?/);
  if (!mi) throw new Error(`${path} に ios: "x.y" の定義が見つかりません`);
  if (!ma) throw new Error(`${path} に android: NN の定義が見つかりません`);
...
// …注記を拾うところで plat = t[1] を見て、buckets[plat] へ振り分けます
```

`toNum` は `"16.4"` を `160400` のような整数に畳む関数なので、`31` のような API レベルもそのまま通ります。Android のために別の比較器を用意する必要はありません。

注記の側は、こう書きます。

```tsx
// @drop-when android>=33 -- 通知権限の明示要求は API 33 から
...
【iOS】現在の下限: 16.0
  注記つき: 4 箇所 / 注記なし: 1 箇所
    iOS 16.4   まで上げる ->   1 箇所が消せる
    iOS 17.0   まで上げる ->   2 箇所が消せる
    iOS 17.2   まで上げる ->   3 箇所が消せる
    iOS 18.0   まで上げる ->   4 箇所が消せる
    ⚠ 消し忘れ src/screens/Settings.tsx:1  ios>=15.0  すでに下限を超えている（消し忘れ）
    ⚠ 注記なし src/screens/Legacy.tsx:1  17.2

【Android API】現在の下限: 31
  注記つき: 1 箇所 / 注記なし: 1 箇所
    Android API 33     まで上げる ->   1 箇所が消せる
    ⚠ 消し忘れ src/screens/Settings.tsx:5  android>=30  すでに下限（31）を下回る
    ⚠ 注記なし src/screens/Legacy.tsx:2  34
```

終了コードは、どちらかの OS に消し忘れが残っていれば 1 になるようにしています。片方だけ緑にしても意味がないからです。

数え始めてすぐに分かったのは、二つの OS で負債の形が違うということでした。iOS 側の分岐は描画や共有シートといった見た目まわりに集まり、Android 側は権限とバックグラウンド実行に偏ります。Android は下限を 1 つ上げるだけで権限まわりの分岐がまとめて消えることがあり、消せる箇所の数だけで比べると、費用対効果は Android のほうが高く出る場面がありました。

もっとも、Google Play の対象 API レベル要件は毎年更新されますので、Android では「上げるかどうか」を考える余地が iOS よりさらに小さいのが実情です。ここは決められないものとして受け入れ、期日から逆算して分岐を消す順番だけを自分で決めるようにしております。
...
最低サポートを上げると、旧 OS の端末では App Store からアプリが消える——私は長いあいだ、そう理解しておりました。
...
これに気づいてから、判断の置き場所を変えました。SDK のリリースノートで下限の変更を見つけたら、その値を `supportMatrix.ts` の候補として控えておきます。そのうえで sweep の出力と突き合わせて、「その下限まで上がると何箇所が消えるか」を先に眺めておきます。SDK 更新の当日に慌てて分岐を整理する、という状況がなくなりました。
...
嬉しくなって記録しかけて、手が止まりました。コードは何も良くなっていません。消えたのは分岐であって、不具合ではありませんでした。数字が動いたのは、いちばんクラッシュしやすかった旧世代の端末が母集団から抜けたからです。

これを「引き上げの成果」として記録すると、翌年の判断が歪みます。「去年は上げたらクラッシュ率が改善した」という記憶だけが残り、実際には改善していないのに、次も同じ期待で判断してしまいます。
...
### 注記の「直後3行」という窓が、呼び出しを隠すことがある

sweep は、注記の行から数えて 3 行先までを「この注記が守っている範囲」とみなしています。注記のすぐ下に `if` と本体が続く、という書き方を前提にした割り切りです。

この割り切りが裏目に出る書き方があります。注記つきの分岐を 2 つ 3 つと続けて並べたあと、そのすぐ下に注記なしの `atLeastIOS(...)` を書いた場合です。最後の注記の窓がその行まで届いてしまい、野良の呼び出しが「注記済み」として見逃されます。サンプルツリーで実際に踏みました。呼び出しを別の行へ 1 行ずらしただけで、警告が出るようになりました。

窓を広げれば直るというものでもありません。広げるほど、無関係な呼び出しまで注記済みとして飲み込みます。私は窓を 3 行のままにして、次の項の ESLint 側で網を張ることにしました。**片方の検出器の穴は、もう片方の検出器で塞ぎます。** 一つのスクリプトを完璧にしようとしないほうが、結果として漏れが減りました。

### `Platform.Version` の直接参照が抜け道になる

`atLeastIOS` を用意しても、既存コードや急ぎの修正では `Platform.Version` を直接読む書き方が復活します。sweep はそれを拾えませんので、気づかないまま棚卸しの網から漏れていきます。
...
収益の一部がそこにぶら下がっているなら、置き土産のビルドには SDK を最新に上げてから凍結させておくべきです。私は一度これを見落として、凍結側の収益が数か月かけてしぼんでいくのを、あとから気づいて悔しい思いをしました。
...
1. sweep を走らせ、候補となる下限ごとに消せる分岐の数を確認します
2. Expo SDK のリリースノートで、次に要求される下限を確認します
3. App Store Connect で、切り捨て対象の OS バージョンのセッション比率と、そこから発生している売上を確認します（利用率は「決める材料」ではなく「覚悟する材料」として見ます）
4. 引き上げの1つ前のリリースで、置き土産となるビルドの既知不具合を潰しておきます
5. サーバー側 API が、古いクライアントからのリクエストを弾かないことを確認します
6. `supportMatrix.ts` の値を更新し、CI が赤くなることを確かめたうえで、一覧に出た分岐を消します
7. 段階公開の枠に載せて出します
