...
highlights:
  - "毎回のリリースで手動ビルド・手動アーカイブに30分以上かかっていた人が、プルリクをマージするだけでTestFlightに自動配布されるワークフローを今日構築できる"
  - "XCTestユニットテストとUIテストをXcode Cloudで動かし、Rork Maxが生成したコードの品質をリリース前に自動検証する仕組みを手に入れられる"
  - "App Storeへの提出も含めて自動化し、月次リリースを『feature/ ブランチのマージ』だけで完結させる体制を作れる"
---
...
この記事では、Rork Max プロジェクトを Xcode Cloud に接続し、本番に耐えるCI/CDパイプラインを一から構築する手順を解説します。
