...
ここからは、Rork で生成した React Native アプリから Notion を CMS として活用する実装を、個人開発 5 本分の経験から得た運用知識ごと書き留めます。中途半端なサンプルではなく、本番で回り続ける形を目指しましょう。
...
## `databases.query` が動かなくなる日 — API バージョン 2025-09-03 と data source

ここまでのコードは `notion.databases.query()` を使っています。実はこの書き方には期限があります。Notion API のバージョン `2025-09-03` で、**データベースとデータソースという2つの概念が分離された**からです。

何が怖いかというと、壊れるきっかけが自分の手元にないことです。

執筆担当の方が Notion の画面でデータベースに2つ目のデータソースを追加する——ただそれだけで、こちらは何もデプロイしていないのに、古いバージョンで動いているアプリの記事一覧が落ちます。公式のアップグレードガイドは、この状況で失敗する操作を明示しています。

<table>
<tr><th>操作</th><th>2つ目のデータソース追加後の挙動（旧バージョン）</th></tr>
<tr><td>そのデータベースを親としたページ作成</td><td><strong>失敗</strong></td></tr>
<tr><td>データベースの読み取り・書き込み・クエリ</td><td><strong>失敗</strong></td></tr>
<tr><td>そのデータベースを指すリレーションプロパティの書き込み</td><td><strong>失敗</strong></td></tr>
</table>

記事配信アプリで言えば、2番目の行がそのまま「一覧が空になる」を意味します。しかもサーバーのログには `validation_error` が出るだけで、原因が Notion 側の UI 操作だとは気づきにくいのです。

### 移行の骨子は「ID をもう1段掘る」こと

やることは単純です。これまで `database_id` ひとつで叩いていたところを、まず **データベース配下の `data_source_id` を取得**し、以降はそちらを使います。

```ts
// server/src/notion/dataSource.ts
import { Client } from "@notionhq/client";

// 2025-09-03 を明示的に指定する。指定しないと旧バージョンで動く
const notion = new Client({
  auth: NOTION_TOKEN,
  notionVersion: "2025-09-03",
});

// GET /v1/databases/:database_id が data_sources の一覧を返すようになった
...
`2025-09-03` では、`GET /v1/databases/:database_id` の役割が「スキーマを返す」から「配下のデータソース一覧（`id` と `name`）を返す」へ置き換わりました。プロパティ定義を取りたいときは、データソース側の取得 API を使います。

クエリ本体は名前空間が `/v1/data_sources` へ移ります。SDK を v5 に上げれば、専用メソッドで書けます。

```ts
// SDK v5（@notionhq/client 5.x）以降
...
SDK をまだ上げられない事情がある場合は、`notion.request()` に `method: "post"` と `path: "data_sources/<データソースID>/query"` を渡す形で、先にバージョンだけ移行しておくこともできます。ページ作成も同様で、`parent` は `{ type: "database_id", database_id }` から `{ type: "data_source_id", data_source_id }` へ変わります。

### レート制限を考えると、data_source_id はキャッシュ必須です

ここが実装上いちばん見落とされるところなのだと思います。

素直に書くと、記事一覧のリクエストが来るたびに「データベース取得 → データソース取得」の2往復が発生します。前述のとおり Notion の実効上限は 3 req/秒。往復が倍になるということは、耐えられる同時アクセスが半分になるということです。

`data_source_id` は、Notion の画面でデータソースを足したり消したりしない限り変わりません。つまり**長めに持って構わない値**です。

```ts
// Workers なら KV、単一プロセスならモジュールスコープの Map で十分
const DS_CACHE_TTL_SEC = 60 * 60 * 24; // 24時間
...
そのうえで、クエリが `validation_error` で落ちたときだけキャッシュを捨てて引き直す — これで「データソース構成を変えた直後の一度だけ余分に叩く」形に収まります。毎回2往復するより、はるかに素直です。

### 忘れやすい2箇所

検索 API とウェブフックも、同じ改訂の影響を受けます。

- **Search API**: `filter.value` に渡す値が `"database"` から `"data_source"` へ変わります。記事検索を Notion の Search に任せている場合、ここを直さないと結果が空になります。
- **ウェブフック**: `database.content_updated` は `data_source.content_updated` へ、`database.schema_updated` は `data_source.schema_updated` へ改名されました。「記事を更新したらキャッシュをパージする」仕組みをウェブフックで組んでいるなら、購読設定の API バージョンを上げる前に、**新旧どちらのペイロードも受けられる状態**にしてから切り替えてください。

バージョンを上げる作業自体は半日で終わります。厄介なのは、上げないまま放置したときの壊れ方が「ある日いきなり、自分の操作と無関係に」だという点です。運用に入れる前の今日のうちに、`notionVersion` を明示して data source 経由に寄せておくことをおすすめします。
...
**案 A: 毎回 Notion API を叩き直して最新の URL を取ります。** 読者がアプリを開いた瞬間にサーバー経由で Notion を再クエリし、その時点の有効な URL を返す方法です。シンプルですが、レート制限の圧迫と応答時間の延びが懸念です。
...
押さえどころは3つあります。**最上位の子ブロックをページネーションすること**（1回のレスポンスは最大100件まで）、**深さを制限すること**（想定外のネストでレート制限を使い切らないため）、そして**ツリーを保持すること**（トグルをトグルとして描画するには階層構造が必要）です。入れ子を失ったフラットな配列を返してしまうと、レンダラでグルーピングが復元できません。
...
**落とし穴 1: トークンがアプリのバンドルに混入します。** Rork で生成されるコードで `.env` を参照する箇所に `NOTION_TOKEN` を書き、クライアント側で使おうとする実装をよく見ます。これは絶対に避けてください。Notion のトークンは**サーバー側の秘匿情報**であり、バンドルに含まれた瞬間に誰でも抽出できます。クライアントからは常に自分のバックエンド経由で Notion を叩く設計を徹底しましょう。

**落とし穴 2: データベース ID をハードコードして、別ワークスペースに切り替えられません。** 開発用・本番用の Notion データベースを分けるとき、DB ID をコードに直書きしていると入れ替えが辛くなります。環境変数にする、またはサーバーのシークレットとして持つのが安全です。

**落とし穴 3: ブロックの `has_children` を無視して子ブロックを取り損ねます。** Notion のブロックは入れ子になれます（例: トグル内のリスト、引用の中の段落）。最上位のブロック配列だけを描画する実装だと、入れ子要素が全部消えます。`has_children: true` のブロックは `notion.blocks.children.list` で子を取り、再帰的に描画する必要があります。画像中心のアプリなら見逃しがちなので、描画ロジックを書く前に「自分のコンテンツにトグルや引用はあるか？」を棚卸ししてください。

## 本番運用で効いてくる3つの小技

最後に、派手ではないけれど本番で効く工夫を3つ挙げておきます。
...
**工夫 2: Notion 編集時の更新をすぐアプリに反映したいなら、短い TTL + 手動パージを組み合わせます。** 記事を直してすぐ確認したい時のために、管理者だけが叩ける `?refresh=1` パラメータをサーバーに用意し、エッジキャッシュをスキップして最新を取るルートを作っておくと、執筆作業のストレスが大きく減ります。

**工夫 3: 監視は「404 になった画像 URL の件数」を見ます。** 画像 URL の1時間失効が悪さをしていないか、本番で最も先に気づけるのがこの指標です。画像プロキシで 404 を検知したらログに記録し、日次で集計して閾値を超えたらアラートを出す運用が実用的です。
...
一覧エンドポイント・詳細エンドポイント・画像プロキシの3つが揃えば、あとは画面側の UI に肉付けするだけで読み物アプリの骨格が完成します。ペイウォール実装や課金連携に進みたくなったら、[Rork × Cloudflare Durable Objects リアルタイムバックエンド実装ガイド](/articles/rork-dev/rork-cloudflare-durable-objects-realtime-backend-guide) で紹介しているキャッシュ層との合わせ技が効いてきます。また、サーバー側のデータ取得と整合させたクライアントキャッシュを組みたい場合は [署名付きURLに変えたらキャッシュが効かなくなった — expo-image のキーを URL から切り離す](/articles/rork-dev/rork-expo-image-signed-url-stable-cache-key-design) の構成とそのまま統合できます。

Notion の設計を作り込むのは、実際に1画面が動いてからで間に合います。プロパティ名も、キャッシュの TTL も、走らせてみて初めて「ここが足りない」と分かるものでした。

書いたものが数秒後に自分のアプリで読める — その手触りが、次の記事を書く気力を連れてきます。お読みいただきありがとうございました。
