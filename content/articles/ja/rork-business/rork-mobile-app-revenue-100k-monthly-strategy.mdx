...
みっつめは「広告フォーマットの組み合わせ最適化」。私の壁紙アプリ（『[綺麗な壁紙](https://apps.apple.com/jp/app/id706533906)』など）での最適解は「ホーム画面下部にバナー（常時）」「画像詳細表示で 5 回に 1 回インタースティシャル」「壁紙設定時にリワード（任意）」でした。リワード広告の eCPM は通常のバナーの 10〜30 倍になることが多く、これを「設定時の任意操作」と紐付けることで UX を損なわずに高 eCPM を獲得できました。
