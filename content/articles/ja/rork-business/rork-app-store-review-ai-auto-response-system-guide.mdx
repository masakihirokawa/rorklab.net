...
2014年から個人開発で iOS / Android アプリを運営していますが、その中で「レビュー返信」だけは、どれだけ自動化を試みても最後まで残り続ける手作業でした。テンプレ返信を貼ると一気に評価が冷えるし、無視すると新規ユーザーへの印象が悪化します。そこで、App Store Connect API・Google Play Developer API・Gemini API を組み合わせ、AI が下書きし、自分は承認ボタンだけ押す半自動パイプラインを組みました。以下、その全体像と、Rork で作ったモバイル管理ダッシュボードと Cloudflare Workers バックエンドのコードを順に共有していきます。
...
きっかけは、返金を求めるレビューに対して Gemini が丁寧で自然な返信を生成し、confidence が 0.9 を超えていた回でした。日本語としては非の打ちどころがありません。ただ、返金の可否は App Store / Google Play 側の管轄で、開発者が公開の返信欄で見通しを述べてよい話ではない。confidence は「文章としての妥当さ」を測る指標であって、「その話題に踏み込んでよいか」は測ってくれません。ここを混同していました。
...
判定は素朴なキーワードマッチで十分でした。ここで再び LLM に判断させると、止めたいケースを取りこぼしたときに気づけません。曖昧なら止まる側に倒す。それだけの層です。
...
運用面でもうひとつ。App Store Connect の API キーには有効期限がなく、失効させるまで有効です。便利なのですが、Cloudflare Workers の Secret に置いたまま 1 年以上放置すると、どのキーがどの Worker で使われているのか追えなくなります。私は `ASC_KEY_REVIEW_YYYYMM` という命名で 6 ヶ月ごとに発行し直し、旧キーは切り替え完了を確認してから App Store Connect 側で失効させています。この棚卸しの手順を決める前に一度、使用中のキーを失効させてしまい、レビュー取得が 3 日止まったことがありました。
