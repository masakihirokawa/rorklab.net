---
title: "App Store のレビュー返信を1日32分から8分にした半自動パイプライン — App Store Connect API × Gemini × Cloudflare Workers の実装記録"
...
author: "Rork Lab"
description: "レビュー返信を AI が下書きし開発者が承認する半自動パイプラインの実装記録です。confidence しきい値 0.85 の根拠、JWT クロックスキュー対策、Crashlytics 突合による返信分岐を3ヶ月分の実測値とともに共有します。"
tags: ["App Store Connect API", "Google Play API", "Gemini API", "レビュー管理", "Cloudflare Workers", "自動化", "Rork Max"]
highlights:
  - "月間 1,200〜1,800 件の新規レビューを捌くために組んだ、AI が下書き・人間が承認する半自動返信パイプラインの全体像と実装コード"
...
1. [Google Cloud Console](https://console.cloud.google.com) でプロジェクトを作成、または既存のプロジェクトを選択します
2. 「API とサービス」→「ライブラリ」から **Google Play Android Developer API** を有効化します
3. 「IAM と管理」→「サービスアカウント」で新しいサービスアカウントを作成します
4. 作成したアカウントのキーを JSON 形式でダウンロードします
5. Google Play Console の「設定」→「API アクセス」でそのサービスアカウントに権限を付与します
...
バックエンド側のルーティングと JWT 認証をもう少し体系的に組みたい場合は、[Rork × Hono.js × Cloudflare Workers で本番レベル REST API を構築する](/articles/rork-dev/rork-hono-cloudflare-workers-rest-api-guide)がそのまま土台になります。
...
few-shot プロンプトに私が過去に書いた良返信を 3 件埋め込み、`response_template: "apology → concrete_action → gratitude"` を `responseSchema` で強制すると、Flash 系の軽量モデルでもトーンが安定します。temperature を 0.4 に下げるのも有効でした。デフォルトの 1.0 では「致命的な不便をおかけし誠に申し訳ございません」のような過剰謝罪が混じることがあります。
...
## confidence しきい値を 0.95 から 0.85 まで下げた6週間の実測

罠5で「0.95 から週次で 0.05 ずつ下げる」と書きましたが、実際に下げていった6週間の数字を残しておきます。対象は日本語レビューのみ、1週間あたりの新規レビューは 280〜340 件でした。「承認待ちの週末残」は、金曜の夜時点で承認も拒否もしていない `pending:` キーの件数です。

<table>
  <thead>
    <tr><th>週</th><th>しきい値</th><th>自動送信率</th><th>誤送信</th><th>承認待ちの週末残</th></tr>
  </thead>
  <tbody>
    <tr><td>1週目</td><td>0.95</td><td>18%</td><td>0件</td><td>61件</td></tr>
    <tr><td>2週目</td><td>0.95</td><td>21%</td><td>0件</td><td>54件</td></tr>
    <tr><td>3週目</td><td>0.90</td><td>29%</td><td>0件</td><td>38件</td></tr>
    <tr><td>4週目</td><td>0.85</td><td>41%</td><td>0件</td><td>19件</td></tr>
    <tr><td>5週目</td><td>0.85</td><td>44%</td><td>0件</td><td>16件</td></tr>
    <tr><td>6週目</td><td>0.80</td><td>58%</td><td>2件</td><td>9件</td></tr>
  </tbody>
</table>

この表から読み取れることが3つあります。

第一に、0.90 から 0.85 に下げた区間で自動送信率が 12 ポイント伸びるのに対し、誤送信は 0 件のままです。ここが実質的な「無料の伸びしろ」でした。逆に言えば、0.90 で止めていた2週間は、安全のためではなく単に自分の承認作業を増やしていただけということになります。

第二に、0.80 に下げた週で初めて誤送信が2件出ています。内訳は「返信文が実在しないバージョン番号に触れたもの」と「英語レビューに対して不自然に丁寧すぎる日本語で返したもの」でした。どちらもレビュー本文が15文字未満です。短文レビューは Gemini が文脈を掴めないまま高い confidence を返してくることがあるため、しきい値とは別に「本文20文字未満は無条件で手動キューへ」というガードを入れました。
...
第三に、承認待ちの滞留件数は 0.95 の週で 61 件まで膨らみ、私が承認するより速く溜まっていました。しきい値を厳しくすると安全になるのではなく、単に人間側がボトルネックになるだけだった、というのが実際に走らせて分かったことです。滞留した返信は鮮度が落ちるので、遅れて送るくらいなら最初から送らないほうがまだ誠実でした。

戻すときの判断基準も先に決めておくと迷いません。私は「直近2週間で誤送信が1件でも出たら 0.05 上げる」というルールにしています。1件目を許容してしまうと、次に気づくのは評価が落ちてからになります。

## confidence を見る前に止める — 自動送信させない3領域

しきい値の話をすると、「では 0.85 を超えたものは全部出していいのか」と聞かれます。私の運用ではそうしていません。confidence を評価するより前に、レビューの内容そのもので自動送信を打ち切る層を置いています。

きっかけは、返金を求めるレビューに対して Gemini が丁寧で自然な返信を生成し、confidence が 0.9 を超えていた回でした。日本語としては非の打ちどころがありません。ただ、返金の可否は App Store / Google Play 側の管轄で、開発者が公開の返信欄で見通しを述べてよい話ではありません。confidence は「文章としての妥当さ」を測る指標であって、「その話題に踏み込んでよいか」は測ってくれません。ここを混同していました。

止めている領域は3つです。

<table>
  <thead>
    <tr><th>領域</th><th>止める理由</th><th>手動で返すときの原則</th></tr>
  </thead>
  <tbody>
    <tr><td>返金・課金トラブル</td><td>可否の決定権がストア側にあり、開発者の返信が約束と読まれる</td><td>状況を受け止め、ストアの返金窓口へ導線を示すだけに留める</td></tr>
    <tr><td>個人情報・プライバシーへの申し立て</td><td>削除依頼や開示請求に軽率な返答をすると記録として残る</td><td>公開返信では詳細に触れず、サポート窓口へ移す</td></tr>
    <tr><td>健康・安全・未成年に関わる記述</td><td>アプリの用途を超えた助言と読まれる余地がある</td><td>断定を避け、必要なら専門機関に触れる。急ぐより正確さを取る</td></tr>
  </tbody>
</table>

判定は素朴なキーワードマッチで十分でした。ここで再び LLM に判断させると、止めたいケースを取りこぼしたときに気づけません。曖昧であれば、止まる側へ倒します。それだけの層です。

```typescript
// src/ai/hardBlock.ts
// confidence 判定より前に実行する。曖昧なものは必ず手動側へ倒す。

type BlockReason = "refund" | "privacy" | "safety" | null;

const BLOCK_PATTERNS: Record<Exclude<BlockReason, null>, RegExp> = {
  refund: /返金|払い戻し|返して|課金され|二重請求|refund|charged twice|money back/i,
  privacy: /個人情報|削除して|プライバシー|開示|GDPR|データを消/i,
  safety: /子供|子ども|未成年|自傷|病気|薬|診断|child|minor|medical|diagnos/i,
};
...
// 呼び出し側: confidence を見るのは、このゲートを通過したあとだけ
...
この層に引っかかるレビューは、全体から見ればごく少数です。それでも月末に `blockedBy` 付きで溜まったものを読み返すと、自動で返していたら面倒になっていたであろう文面が並んでいます。しきい値を 0.05 刻みで詰める作業より、この 20 行を先に書くほうが効きました。

しきい値は「どれだけ楽をするか」の調整弁で、ハードブロックは「どこから先は自分の言葉で書くか」の宣言です。前者だけを回していると、楽になった実感と引き換えに、返信の責任範囲があいまいになっていきます。

## 返信を書く言語の判定を、手元で測り直しました

上の「7つの罠」で、`locale` を信じずに本文の文字種比率で返信言語を決めている、と書きました。この記事を読み返していて、その 30% という数字を最初に置いたとき、私はほとんど計測をしていなかったことを思い出しました。感覚で決め、事故が減ったので、そのままにしていたのです。

気になって、手元で測り直しました。実際に届きうる書き方をしたレビュー本文を 11 件用意し、二つの数え方を並べています。

- 素朴版: 本文の全文字に対する、ひらがな・カタカナ・漢字の比率
- 語単位版: `Intl.Segmenter` で語に切り、日本語文字を含む語の比率

<table>
  <thead>
    <tr><th>レビュー本文の型</th><th>素朴版</th><th>語単位版</th><th>返したい言語</th></tr>
  </thead>
  <tbody>
    <tr><td>日本語のみ</td><td>0.91</td><td>1.00</td><td>日本語</td></tr>
    <tr><td>日本語＋製品名（iCloud、Wi-Fi）</td><td>0.63</td><td>0.82</td><td>日本語</td></tr>
    <tr><td>日本語＋短いエラー名</td><td>0.19</td><td>0.70</td><td>日本語</td></tr>
    <tr><td>日本語＋長いクラッシュログ</td><td>0.05</td><td>0.21</td><td>日本語</td></tr>
    <tr><td>日本語＋スタックトレース</td><td>0.05</td><td>0.20</td><td>日本語</td></tr>
    <tr><td>英語＋日本語のひと言</td><td>0.27</td><td>0.50</td><td>英語</td></tr>
    <tr><td>英語のみ</td><td>0.00</td><td>0.00</td><td>英語</td></tr>
  </tbody>
</table>

**ログを貼ってくれた日本語のレビューほど、素朴な文字数比率では英語だと判定されます。** 「開くと落ちます。Fatal Exception: NSInvalidArgumentException と出ます。」という程度の本文でも素朴版は 0.19 で、30% の線を割ります。スタックトレースを丸ごと貼ってくださった方に至っては 0.05 でした——いちばん手間をかけて状況を伝えてくれた方に、いちばん英語で返してしまう構造だったわけです。

語単位に数え直すと、同じ本文が 0.70 と 0.20 になります。エラー名がひとつ混じった程度の日本語は、これで救えました。一方で、長いログとスタックトレースは 0.20 前後に沈んだままです。

では閾値を下げればよいのかと思い、0.05 刻みで総当たりしてみました。どの一本を選んでも、11 件のうち 2 件は必ず取り違えます。日本語＋長いログ（0.21・0.20）と、英語に日本語のひと言が混ざった本文（どちらも 0.50）が、比率の上で重なってしまうためです。**一本の線では分けられない**、というのが、測って初めて分かったことでした。

直すべきは線を引く位置ではなく、線の本数でした。

```typescript
// src/ai/replyLanguage.ts
// 本文の文字種比率で返信言語を決める。分けられない帯は、判定せず手動へ回す。

const JP_CHAR = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/u;

/** 語に切り、日本語文字を含む語の比率を返す（文字数比率ではない点が肝） */
...
const HIGH = 0.6;  // ここより上は日本語で返す
const LOW = 0.15;  // ここより下は本文の言語で返す
...
export function routeReplyLanguage(text: string): LanguageRoute {
  ...
  return { kind: "manual", ratio };  // 0.15〜0.60 は自動送信の対象から外す
}
```

この帯を入れると、先ほどの 11 件は自動 6 件・手動 5 件に分かれ、自動側の取り違えはゼロになりました。手動がほぼ半分というのは一見多いのですが、回ってくるのは「ログを貼ってくれた日本語の方」と「英語の中に日本語のひと言を混ぜた方」です——どちらも、私が読んでから返したほうがよいレビューでした。減らしたかったのは手間ではなく、失礼だったのだと気づきました。

`Intl.Segmenter` が使えない実行環境では、素朴版のまま帯だけを広めに取るほうが安全です。判定の精度を上げられないぶん、判定しない範囲を広げるという考え方になります。

**判定できないものは、判定できたふりで通しません。** ハードブロックと同じ線引きを、言語の側にも置いたということです。しきい値を 0.30 から 0.20 へ動かす作業に戻る前に、ご自身の判定器へ手元のレビュー本文を 10 件ほど流し、比率を並べて眺めてみてください。私の 30% は、測っていない数字でした。

## 複数アプリを並行運用すると効いてくる設計判断

ここは個人開発で複数アプリを運営している方向けの章です。私が並行運営しているアプリ群では、合計の月間新規レビュー数が 1,200〜1,800 件あります。アプリ単位の単純な実装では破綻するため、いくつかの設計を入れています。
...
<table>
  <thead>
    <tr><th>Crashlytics 上の状態</th><th>Gemini に渡すコンテキスト</th><th>返信の方向性</th></tr>
  </thead>
  <tbody>
    <tr><td>直近 24h で初検出</td><td>「現在エンジニアリングチームで原因調査中」</td><td>調査中であることを誠実に伝え、再発時の詳細情報提供を依頼</td></tr>
    <tr><td>修正済み・未リリース</td><td>「修正済み・次バージョン v2.1.0 で対応」</td><td>具体的なバージョン名とリリース予定週を伝える</td></tr>
    <tr><td>すでにリリース済み</td><td>「v2.1.0 でこの問題は修正済み」</td><td>ストア更新を案内し、再発時の連絡先を伝える</td></tr>
  </tbody>
</table>
...
ここは技術というより姿勢の話です。AI に下書きさせると決めた時点で、返信から温度が抜けていく可能性は避けられません。その温度をどう保つかは、私にとっては confidence しきい値の調整と同じくらい実務的な課題でした。
...
第三に、承認画面の UI には「3 秒は待ってから承認ボタンを押す」というディレイを入れています。Rork で作ったダッシュボードでは、最初の 3 秒間ボタンが灰色で、その後アクティブになります。承認ボタンを反射的に押す癖がつくと、AI の判断ミスがそのまま出て行ってしまいます。3 秒の遅延は地味ですが、結果として「あ、これは違うな」と気づける機会を生みます。実際、この 3 秒があったおかげで送信を止めた返信が、3ヶ月で 11 件ありました。内訳は誤ったバージョン番号への言及が 6 件、返信対象を取り違えたものが 3 件、トーンが過剰だったものが 2 件です。

技術的な完成度だけでなく、こうした姿勢の部分まで含めて設計するのが、個人開発者としてユーザーに対して誠実であるための、私なりの線の引き方です。
...
4. KV に「修正パッチを当てた未リリース版が存在する」というフラグが立っていたこと（前日に Claude in Chrome × Xcode で修正済み）
...
**費用**: 1日30件のレビューを処理して、Gemini の API 費用は月に $1〜$2 程度でした。プロンプトが 1,000 トークン前後、出力が 300 トークン前後という軽い処理なので、レビュー件数が数倍になっても桁は変わりません。コストが導入の障害になることはまずないと思います。

**モデル ID について（2026年8月時点の追記）**: 本記事の実測値は `gemini-2.0-flash` で取得したものですが、このモデルは 2026年6月1日に停止されました。上のコードは公式の推奨移行先である `gemini-3.6-flash` に差し替えてあります。移行はエンドポイントのモデル ID を変えるだけで、`responseMimeType: "application/json"` と `responseSchema` の指定はそのまま通りました。ただし温度の効き方は世代で変わります。移行直後は `temperature` を据え置きにせず、手元のレビュー 20 件ほどで出力を並べて確認することをおすすめします。私は 0.4 のままで違和感がありませんでしたが、これはアプリのジャンルによると思います。最新の停止予定は [Gemini deprecations](https://ai.google.dev/gemini-api/docs/deprecations) で確認できます。
...
運用面で、もうひとつお伝えしておきたいことがあります。App Store Connect の API キーには有効期限がなく、失効させるまで有効です。便利なのですが、Cloudflare Workers の Secret に置いたまま 1 年以上放置すると、どのキーがどの Worker で使われているのか追えなくなります。私は `ASC_KEY_REVIEW_YYYYMM` という命名で 6 ヶ月ごとに発行し直し、旧キーは切り替え完了を確認してから App Store Connect 側で失効させています。この棚卸しの手順を決める前に一度、使用中のキーを失効させてしまい、レビュー取得が 3 日止まったことがありました。
