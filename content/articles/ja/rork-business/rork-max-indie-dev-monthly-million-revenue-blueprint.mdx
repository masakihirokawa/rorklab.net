...
機能は増えています。レビューの星も悪くありません。それでも数字が動きません。
...
必要なダウンロード数を、先に一度だけ置いておきます。月額1,500円・インストールの5%が有料化すると仮定すると、100万円/月に必要な有料会員は約667人、その在庫を積み上げるまでの累計インストールは約13,300件です。

ただしこれは「積み上がるまで」の数字にすぎません。積み上がったあとに毎月いくつ必要かを決めるのは解約率のほうで、そこは次の節の表に譲ります。日次の目標は解約率しだいで20台から200台まで動きますので、ここでの「100DL/日」は目盛りの一つとして読んでいただければ十分です。
...
App Storeのアルゴリズムはインストールの勢いとレビューの評価を見ています。「何件集まれば順位が動く」という閾値を私は一次データで示せませんので、断定は避けます。確かなのは、レビューがゼロに近いあいだは検索結果で比較の土俵にすら乗らない、という一点です。
...
## レビュー依頼は年3回しか撃てません：置き場所ではなく打順を決める

Appleのレビュー依頼（`requestReview()`）は、同じアプリにつき **365日で3回まで**しか表示されません。実装上は何箇所からでも呼べますが、4回目以降はシステムが黙って握り潰します。

個人開発で運用しているアプリの一本で、私はこの上限を読み飛ばしたまま「呼べる場所すべて」に依頼を置いておりました。起動の翌日、オンボーディングの完了、初めて成果が出た瞬間、7日連続の達成、課金から30日——ユーザーが機嫌よくしていそうな場所を数えて5箇所です。多く置くほど集まるものだと思い込んでいました。

実際には、**置いた数はほとんど結果に効きません**。上限のほうが先に来るからです。

### 上限があるときの配置を、総当たりで計算し直す

5つの候補について「インストールを基準にした到達率」と「そこで依頼したときに星4以上が付く確率」を置き、上限3回のもとで32通りの組み合わせを全部試します。到達は入れ子（後段に届いた人は前段にも届いている）とみなし、依頼は到達した順に消費される前提です。

```python
# review_slots.py — 年3回の上限を踏まえてレビュー依頼の置き場所を選ぶ
import itertools

CAP = 3  # 365日あたりの表示上限（Apple 側の仕様）

# (瞬間, インストール基準の到達率, 依頼したとき星4以上が付く確率)
MOMENTS = [
    ("初回起動の翌日",       0.55, 0.04),
    ("オンボーディング完了", 0.42, 0.07),
    ("初めて成果が出た瞬間", 0.26, 0.22),
    ("7日連続の達成",        0.11, 0.38),
    ("課金から30日",         0.05, 0.45),
]
...
def simulate(selected, installs=1000):
    ...
    reviews = prompts = 0.0
    for k in range(len(MOMENTS)):                 # ちょうど k 段目まで到達した層
        layer = reach[k] - reach[k + 1]
        fired = [i for i in sorted(selected) if i <= k][:CAP]   # 上限で切られる
...
for reviews, prompts, sel in sorted(ranked, reverse=True)[:5]:
    print(f"星4以上 {reviews:6.1f} / 依頼 {prompts:7.1f} / "
          f"1依頼あたり {reviews / prompts:.3f}  "
          + "＋".join(MOMENTS[i][0] for i in sel))
```

到達率と確率は仮置きの値です。ご自身のファネルの実測値に差し替えてお使いください。この仮定のまま回すと、インストール1,000件あたりで次の並びになります。

<table>
  <thead>
    <tr><th>依頼を置く場所</th><th>依頼回数</th><th>星4以上</th><th>1依頼あたり</th></tr>
  </thead>
  <tbody>
    <tr><td>5箇所すべて</td><td>1,230</td><td>108.6</td><td>0.088</td></tr>
    <tr><td>前半3箇所（翌日・完了・成果）</td><td>1,230</td><td>108.6</td><td>0.088</td></tr>
    <tr><td><strong>完了・成果・7日連続</strong></td><td>790</td><td><strong>128.4</strong></td><td>0.163</td></tr>
    <tr><td>成果・7日連続・課金30日</td><td>420</td><td>121.5</td><td>0.289</td></tr>
    <tr><td>7日連続・課金30日</td><td>160</td><td>64.3</td><td>0.402</td></tr>
  </tbody>
</table>

読み方を3つだけ挙げます。

**5箇所すべてと、前半3箇所だけは、結果が1件も変わりません。** 上限は到達した順に消費されるため、後半に置いた確率の高い瞬間には順番が回ってこないのです。保険のつもりで足した2箇所は、コードに書いてあるだけで存在しないのと同じでした。

初回起動の翌日を外すと、依頼回数は1,230から790へ減り、星4以上は108.6から128.4へ増えます。**依頼を減らしたほうが、集まるレビューは増えます。** 上限のある資源では、撃つ回数ではなく撃つ順番が結果を決めます。

いっぽうで、効率だけを見て「7日連続と課金から30日だけ」に絞ると、1依頼あたりは0.402と最良になりますが、総量は64.3まで落ちます。到達率の低い瞬間は、確率が高くても母数を運んできません。効率の最良と総量の最良は、同じ場所にはないのです。

この計算を先にしていれば、と思うのは、5箇所に置いたまま伸びなかった理由を実装の質だと勘違いして、しばらく別のところを探し回ったからです。**上限のある依頼は、置き場所ではなく打順の問題です。**

### 打順を1か所に集約しておく

呼び出し箇所を増やすのではなく、「どの瞬間で撃つか」を一箇所で決める形にしておくと、あとから順番だけを差し替えられます。

```ts
// src/review/reviewSlots.ts — 依頼の打順を1か所に集約する
import * as StoreReview from 'expo-store-review';

type Moment = 'day2' | 'onboarded' | 'first_win' | 'streak7' | 'paid30';

// 上から順に撃つ。年3回の上限に当たる前に、価値の高い瞬間へ届かせる
...
export async function requestReviewAt(
  ...
  if (firedInLast365Days.length >= 3) return 'skipped:yearly_cap'; // 手元でも数えておく
  if (!(await StoreReview.hasAction())) return 'skipped:unavailable';

  await StoreReview.requestReview();
  return 'requested'; // 実際に表示されたかどうかはOSしか知らない
}
```

`requestReview()` から返ってくるのは「依頼した」という事実だけです。表示されたかどうかも、星が付いたかどうかも、アプリ側からは観測できません。観測できない領域をこれ以上増やさないための設計が、打順の固定なのだと思います。

まずは、いま呼び出しが何箇所あるかを数えるところから始めていただければと思います。4箇所以上あるなら、そのうちのいくつかは、書いてあるだけで一度も動いていないかもしれません。
...
そして解約率3%と15%では、平均継続期間が33.3ヶ月と6.7ヶ月になります。同じアプリが、片方では3年近く、もう片方では半年ちょっとで手を離れます。この差が、翌年の自分の作業量を決めます。

この表を先に作っておけばよかったと気づいたのは、機能の本数だけを増やしていた時期を一度通り過ぎたあとでした。
...
安い価格は、必要な有料会員数を跳ね上げます。解約率の節に置いた表を見ていただくと分かりますが、月額480円と月額1,980円では、同じ収益に必要な会員数が4倍以上違います。安さで課金率が多少上がったとしても、この4倍を埋められることはまずありません。
...
ここまでの節は、どれも「作る量」ではなく「どこで漏れているか」を名指しするための道具でした。

最初のアクションは一つで足ります。既存のアプリにトライアル付きのサブスクリプションを一本だけ足して、転換率と解約率を一週間だけ記録してみてください。数字が出たあとで、この記事の表に戻ってきていただければ幸いです。

計算が当たるとは限りません。それでも、当たらなかったことが分かる形にしておくことだけは、いまも続けています。
