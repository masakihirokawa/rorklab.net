---
title: "Rork で作る AI 読書アシスタントアプリ実装 — 本棚スキャン、進捗管理、章別要約の設計と運用"
...
author: "Rork Lab"
description: "本棚を写真で撮るだけで蔵書を登録し、Claude が章別に要約と問いを生成する読書アプリを Rork で実装します。Vision と Claude の二系統マージ、トークン単価の実測、ネタバレ抑止の後処理、課金設計までを扱います。"
...
そこで Rork 上に React Native + Expo + Supabase + Claude API を並べて、本棚スキャンから章別要約、パーソナライズされた読書提案までを実際に組み上げました。コードはそのまま動く粒度で載せ、私自身が実際に詰まった箇所と、月額課金アプリとして成立させるためのサブスクリプション設計まで踏み込みます。
...
### 落とし穴 2: 「画像を小さくすればトークン代が下がる」という思い込み

私が最初に書いたコードには、クライアント側で 4000px の写真を 2048px に落とす処理が入っていました。トークン代を下げるためです。ところが実際に請求を突き合わせてみると、リサイズしてもしなくても金額がほとんど変わりませんでした。

理由は API 側の仕様にあります。Claude の画像入力は、長辺が 1568px を超える画像、あるいは約 1,600 トークンを超える画像を、**サーバー側で先に縮小してからトークン換算します**。つまり 4000px を送ろうと 2048px を送ろうと、課金対象は同じ上限に丸められます。トークン数の概算式は `幅px × 高さpx ÷ 750` で、上限に張り付いた状態では約 1,600 トークンです。

では、クライアント側のリサイズは無駄かというと、そうではありません。効くのは別の三点です。アップロード時間が短くなること。base64 化したペイロードがリクエスト上限に当たらなくなること。そして通信の失敗率が下がること。目的が「コスト」なのか「体験」なのかを取り違えたまま最適化していたのが、私の失敗でした。

実際の単価を置いて計算すると、こうなります（入力 $3 / 出力 $15 per MTok、1 ドル 155 円と仮定。ご自身の環境の実単価に差し替えてください）。

<table>
<thead>
<tr><th>処理</th><th>入力トークン</th><th>出力トークン</th><th>1 回あたり概算</th></tr>
</thead>
<tbody>
<tr><td>本棚スキャン（画像 1 枚 + 指示 800）</td><td>約 2,400</td><td>約 600</td><td>約 2.5 円</td></tr>
<tr><td>章別要約（メモ + 文脈）</td><td>約 1,500</td><td>約 700</td><td>約 2.3 円</td></tr>
</tbody>
</table>

1 回 2〜3 円という数字は、単体で見れば取るに足りません。危険なのは積み上がり方のほうで、それは収益化の節で扱います。無料ユーザーの枠管理として月間スキャン回数を KV か Postgres でカウントする実装は、コスト対策というより**この積み上がりを可視化するための計器**だと考えてください。

### 落とし穴 3: 章別要約のネタバレをユーザーが嫌がる

テスターから一番多かったフィードバックが「知りたくなかった結末まで書かれていた」でした。プロンプトで明示的に「ネタバレを避ける」と書いても、モデルは容赦なく全体要約を出してきます。対策は 2 段階です。第一に、プロンプトで「読み終えた章までの範囲だけを扱い、先の章には触れない」と強く書きます。第二に、出力を後処理で「この後の章では〜」「最終的に〜」などのネタバレフレーズを正規表現でチェックし、検出したら再生成します。再生成が走る割合はプロンプト次第ですが、私の環境では全要約の 1 割弱でした。判定に安いモデルを使っている限り、この上乗せは無視できる範囲に収まります。
...
私の設計はこうです。無料プランでは本棚スキャンが月 1 回、AI 要約は月 3 回までに制限します。有料プラン（月 ¥580 / 年 ¥4,800）では無制限スキャンと無制限要約、パーソナル通知、データエクスポート機能を解放します。有料プランに年間プランを用意して年払いで割引する設計は、解約率を下げる効果があります。

### 無料枠の原価が、黒字を静かに食う

ここが読書アプリで一番見落とされる場所です。先ほどの単価で、実際に積み上げてみます。

有料ユーザー 1 人が月にスキャン 8 回・要約 40 回を使うと、AI 原価は約 113 円。月額 580 円に対して原価率 19.5% です。この数字だけを見れば健全そのもので、私も最初はここで安心していました。

問題は無料ユーザーです。無料枠（スキャン 1 回・要約 3 回）を使い切る 1 人あたりの原価は約 9.5 円。少額ですが、無料と有料の比率が 20:1 なら、有料 1 人の裏側で無料 20 人分＝約 190 円が乗ります。合計原価は約 303 円、原価率は 52% まで跳ね上がります。ここに Apple / Google の 15〜30% 手数料とサーバー費が重なると、粗利はほとんど残りません。

<table>
<thead>
<tr><th>前提</th><th>AI 原価（月）</th><th>580 円に対する原価率</th></tr>
</thead>
<tbody>
<tr><td>有料ユーザー 1 人のみ</td><td>約 113 円</td><td>19.5%</td></tr>
<tr><td>有料 1 人 + 無料 20 人</td><td>約 303 円</td><td>52.2%</td></tr>
</tbody>
</table>

打ち手は「無料枠を絞る」だけではありません。私が有効だと感じている順に三つ挙げます。

第一に、無料枠の**中身を入れ替える**こと。無料ユーザーに毎月スキャンを配るより、初回登録時に 3 回まとめて配って以降ゼロにするほうが、原価は下がりコンバージョンは上がりました。読書アプリの価値は最初の蔵書登録で伝わるからです。継続的に少しずつ配ると、無料のまま「足りているような気がする」状態を延命させてしまいます。

第二に、要約のモデルを分けること。無料ユーザーの要約は Haiku 系の安価なモデルに落とし、有料ユーザーだけ Sonnet を使います。品質差は読み比べれば分かりますが、無料ユーザーにとっては「有料にすると要約がここまで変わるのか」という体験そのものが訴求になります。原価を下げる施策が、そのまま課金理由の提示になっている点が気に入っています。

第三に、原価をユーザー単位で記録すること。`ai_usage` テーブルに `user_id / feature / input_tokens / output_tokens / created_at` を積んでおくと、上位 1% のヘビーユーザーが全体の何割を消費しているかが即座に出ます。私の場合、上位のごく一部が無料枠の消費を押し上げていました。これは推測では絶対に見つからない類の事実で、計器を入れておいてよかったと胸を撫で下ろした場面でした。

価格そのものの設計指針は [Rork アプリのサブスク価格設計 — 継続率を落とさず LTV を最大化する 5 つの原則](/articles/rork-business/rork-app-subscription-pricing-design-principles) に整理してあります。

トライアル期間は 7 日ではなく 14 日を推奨します。読書アプリは、価値を実感するまでに「本を 1 冊読み終える」という時間が要るからです。7 日はその手前で切れます。習慣が立ち上がる直前で終わるトライアルは、「便利そうだけど使いこなせなかった」という記憶だけを残します。延長した 1 週間ぶんの AI 原価は数円で、年単位のサブスクリプションと引き換えにするなら十分に安い賭けだと考えています。

支払いフローの詳細は [RevenueCat の Entitlement でアクセス判定とペイウォールを組む実装メモ](/articles/rork-dev/rork-revenuecat-entitlements-paywall-build-notes) を参考にしてください。
...
まずは今日、自分の本棚を 1 枚撮って、Claude API に「この画像から本のタイトルを JSON で返して」と投げてみてください。所要 10 分ほどで、このアプリの中核体験がどれくらい成立するのかが自分の手で分かります。認識できた冊数と、間違えた冊数。その二つの数字が、作るべきか見送るべきかを教えてくれます。

そのうえで進めるなら、順番は次の通りです。Rork でクライアントを生成し、Supabase に蔵書テーブルを一つ作り、スキャン API を Workers に置く。ここまでが最初の 1 週間。二週目に章別要約とネタバレ後処理を足し、`ai_usage` テーブルで原価の計器を先に入れておく。課金の実装はその後で構いません。原価が見えていない状態で価格を決めると、あとから動かせなくなります。

私自身、このアプリを作る前は「トークン代なんて誤差だろう」と思っていました。実際に積み上げてみて、誤差ではなく設計そのものだと分かったのは収穫でした。同じ落とし穴を踏まずに済む方がいれば嬉しいです。お読みいただきありがとうございました。