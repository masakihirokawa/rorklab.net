...
app.post("/mcp", async (c) => {
  ...
  server.tool(
    "create_task",
    "会話の中でユーザーがこれからやることに触れた時に呼びます。既存タスクの内容変更には使わず、その場合は update_task を呼んでください。",
    {
      title: z.string().describe("ユーザーの発話をそのまま写した短い文。要約や言い換えはしないでください"),
      due_date: z.string().optional().describe("YYYY-MM-DD 形式の期日。ユーザーが期日に触れていない場合は省略します"),
    },
  ...
  server.tool(
    "list_tasks",
    "現在のタスク一覧を取得します。タスクを更新・完了する前に必ず先に呼んでください。",
    {},
    async (_, ctx) => {
      const userId = await resolveUserId(ctx, c.env.DB);
...
ここで意識すべき点は三つです。第一に、署名検証を最初から入れること。OpenAI 側は `x-openai-apps-signature` ヘッダーで HMAC を送ってきますので、共有秘密鍵を Cloudflare Workers の Secrets に入れて毎リクエストで検証します。これを後回しにすると、本番デプロイ後に「誰でも MCP サーバーを叩ける」状態が露出します。第二に、ツールの戻り値は `structuredContent` と `content` を両方返すこと。前者は ChatGPT のモデルが推論で使い、後者はユーザーに見える UI として描画されます。両方を返さないと、会話文脈にデータが残らず、続きの質問でモデルが無駄に再取得しに行きます。

私は最初、`structuredContent` を省いていたのですが、ユーザーが「さっき作ったタスクの期日を変えて」と言った瞬間にモデルがタスク ID を見失うバグに遭遇しました。設計時から両輪で返すと決めておくと、こういう罠を踏みません。

第三に、ツール名の隣に説明文を渡すこと。上のコードで `server.tool()` の第2引数に日本語の説明を置いているのがそれです。ここを省くと、モデルが手にしている手がかりは `create_task` という識別子だけになります。この一点が呼び出し精度をいちばん左右しますので、節を改めて扱います。

## モデルに正しいツールを呼ばせる：説明文・アノテーション・instructions

ツールを定義しても、モデルが適切なタイミングで呼んでくれるとは限りません。ChatGPT の側から見えているのは、ツール名と説明文、そして引数スキーマだけです。ここが痩せていると、「今週やることをまとめて」と話しかけられてもツールは呼ばれず、モデルは一般論を返して会話が終わります。

実際に私が書き換えた前後を並べます。

<table>
<tr><th>項目</th><th>改善前</th><th>改善後</th></tr>
<tr><td>ツールの説明文</td><td>タスクを作成します</td><td>会話の中でユーザーがこれからやることに触れた時に呼びます。既存タスクの内容変更には使わず、その場合は update_task を呼んでください。</td></tr>
<tr><td>引数 <code>title</code></td><td>タスクのタイトル</td><td>ユーザーの発話をそのまま写した短い文。要約や言い換えはしないでください。</td></tr>
</table>

効いているのは長さではありません。**呼ぶ条件と、呼ばない条件の両方が書いてあるか**です。「この用途には使わない」という一文を足しただけで、隣り合うツールの取り違えが目に見えて減りました。引数の `describe()` も同じで、「タスクのタイトル」とだけ書いてあるとモデルは当たり障りのない要約を入れてきますが、「本人の言葉をそのまま写す」と指定すると発話がそのまま残ります。型定義の顔をしたプロンプトだと思って書くのが実感に合っています。

MCP の TypeScript SDK には、ツール定義を設定オブジェクトで渡す `registerTool` も用意されています。こちらは `description` に加えて `annotations` と `outputSchema` を宣言できますので、新しい SDK を使える環境ではこの形が扱いやすいです。
...
server.registerTool(
  ...
  {
    title: "タスクを削除",
    description: "ユーザーが明示的に削除を求めた時だけ呼びます。完了扱いにしたい場合は complete_task を使ってください。",
...
`annotations` はモデルではなくクライアント向けのヒントです。読み取り専用のツールに `readOnlyHint` を付ければ確認なしで実行され、`destructiveHint` を付けたツールには確認が挟まる、といった扱いの差が生まれます。削除系に付け忘れると、会話の勢いのままデータが消える事故につながります。

ツールをまたぐルールは、個々の説明文ではなくサーバー側の `instructions` に置きます。
...
const server = new McpServer(
  ...
  {
    instructions:
      "タスクを更新・完了する前に必ず list_tasks で現在の状態を取得してください。一覧は最大 50 件で打ち切られます。ユーザーが期日に触れていない場合、due_date は推測せず省略してください。",
  }
);
```

`instructions` はクライアントがシステムプロンプト側に載せることを想定した文章です。ツールの説明文と同じ内容を二重に書かず、「呼ぶ順序」「上限」「推測してはいけないこと」といった、ツール単体では表現しきれない制約だけを置きます。

もうひとつ、エラーの返し方でモデルの振る舞いが変わります。ハンドラー内で例外を投げた場合も SDK がエラー結果に変換してくれますが、モデルに届くのは素の例外メッセージです。自分で `isError: true` を返すと、直し方をこちらの言葉で書けます。

```typescript
if (due_date && !isValidDate(due_date)) {
  return {
    content: [
      { type: "text", text: `due_date は YYYY-MM-DD 形式で指定してください。受け取った値: ${due_date}` },
    ],
    isError: true,
  };
}
...
この分岐を入れてから、日付フォーマットの取り違えは会話の中で自動的に解決されるようになりました。ユーザーには何も見えないまま、モデルが正しい形で呼び直します。エラー文はログを読む人間ではなく、モデル宛の指示書として書くと精度が上がります。
...
server.tool(
  "upgrade_to_pro",
  "ユーザーが有料プランへの切り替えを希望した時だけ呼びます。価格を尋ねられただけの場合は呼ばずに回答してください。",
...
手を動かす順序としては、ツールの説明文を書き直すところから始めるのがいちばん費用対効果が高いと感じています。サーバーのコードを組み替えなくても、モデルの振る舞いだけが変わっていきます。

Apps SDK はまだ手探りの領域で、私自身も毎週どこかでつまずいています。ここまでお読みいただき、ありがとうございました。
