...
highlights:
  - "カメラ・マイク・写真の権限を Just-in-Time で要求し、拒否されても機能を残す縮退設計の実装パターン"
...
カメラを呼び出すボタンを置き、音声入力のマイクボタンを置き、テキスト欄を置く——3つそろえた瞬間に、画面はもう成立しなくなります。個人開発でアプリを運用していると、入力手段を1つ足すたびに同じ壁に突き当たりました。「次の機能を足すとUIが破綻する」という感覚が、機能追加のたびに先に来るようになったのです。
...
1. **コンテキストの断絶**: ユーザーが画像を撮影した後に音声で質問しても、画像の文脈が音声処理へ渡りません
2. **状態管理の複雑化**: 3種類の入力状態が分散し、UIの整合性が崩れやすくなります
3. **AIモデルへの入力構築が煩雑**: 各モダリティのデータを手動で組み立て、プロンプトへ詰め込む手間がかかります
...
1. **段階的処理**: 音声→テキスト変換を最初に行い、以降の処理では統一的にテキストとして扱えるようにします
2. **オンデバイス優先**: レイテンシとコストを抑えるため、可能な限り Core ML で前処理を行います
3. **フォールバック対応**: オフライン時は自動的にオンデバイスモデルのみで処理を完結させます
...
### 長辺を何ピクセルまで落としてよいか — 手元で測った分岐点

上の `PREPROCESS_CONFIGS` に並んだ数字は、最初は何となく置いたものでした。いま思えば、私は長いあいだ「大きく送るほど当ててくれる」と信じていて、2048px のまま投げ続けていたのです。API の請求が想定より膨らんだ月に、ようやく長辺だけを変えて測り直しました。

手元の写真 60 枚——壁紙アプリ向けに撮った風景や質感の素材が 30 枚、ラベルや説明書きなど**小さな文字が写り込んだ写真**が 30 枚——で、長辺だけを変えて同じプロンプトを投げ、1024px の応答を基準に「同じことを言っているか」を私が目で数えました。JPEG の品質は 0.85 で固定し、同じ回線・同じ時間帯で 3 往復した中央値を取っています。

<table>
<thead>
<tr><th>長辺</th><th>送信ペイロード</th><th>1024px の応答と一致</th><th>往復 p50</th></tr>
</thead>
<tbody>
<tr><td>2048px</td><td>約 620KB</td><td>60 / 60</td><td>1.9 秒</td></tr>
<tr><td>1536px</td><td>約 360KB</td><td>60 / 60</td><td>1.4 秒</td></tr>
<tr><td>1024px</td><td>約 180KB</td><td>基準</td><td>1.1 秒</td></tr>
<tr><td>768px</td><td>約 105KB</td><td>57 / 60</td><td>0.9 秒</td></tr>
<tr><td>512px</td><td>約 52KB</td><td>48 / 60</td><td>0.8 秒</td></tr>
</tbody>
</table>

はっきり分かれたのは、外れ方の中身でした。512px で食い違った 12 枚は、**すべて小さな文字が写っている写真**です。風景や質感の素材は 512px でも一度も外れませんでした。768px で外れた 3 枚も同じ性格で、パッケージの成分表示や注意書きを読ませようとしたものでした。

**画素は「何が写っているか」を当てるためではなく、「文字を読む」ために要ります。** 被写体の分類だけなら 768px で足り、文字を読ませる可能性があるなら 1024px を下限にする——この 1 行に落ち着いてから、私は解像度で迷わなくなりました。

```typescript
// 文字を読ませる要求かどうかで長辺を決める
type VisionIntent = "classify" | "read_text";

const LONG_EDGE_BY_INTENT: Record<VisionIntent, number> = {
  classify: 768,   // 被写体の分類・雰囲気の判定
  read_text: 1024, // ラベル・説明書き・手書きメモ
};
...
// 呼び出し側は「何をさせたいか」だけを渡す
// preprocessImageForIntent(uri, "classify") → 約 105KB
// preprocessImageForIntent(uri, "read_text") → 約 180KB
```

`intent` を呼び出し側に書かせる形にしたのは、解像度という実装の都合を UI から見えないところへ押し込みたかったからです。私のアプリでは分類用途が 8 割を占めるので、平均ペイロードは 180KB から 120KB 前後まで下がりました。撮る写真の性格が違えば分岐点も動くはずですから、同じ手順をご自身の素材 50 枚ほどで一度なぞってみていただければと思います。私の数字をそのまま使うより、はるかに確かな下限が出るかもしれません。
...
## 権限リクエストの順序が、マルチモーダル体験の入口を決める

ここまでの層を丁寧に組んでも、カメラとマイクの権限をどの順番で、どのタイミングで要求するかを誤ると、ユーザーはそもそも入力層に到達しません。オーケストレーターのコードをいくら読み返しても原因が出てこない種類の事故です。

### 起動直後にまとめて訊くと、そのモダリティを永久に失う

初回起動でカメラ・マイク・写真ライブラリを続けざまに要求する実装を、私は一度そのまま出してしまったことがあります。iOS の権限ダイアログは、**一度「許可しない」を選ばれるとアプリ側から再提示できません**。以降は設定アプリまでユーザーに歩いてもらうしかなく、そこを歩いてくれる人はごくわずかです。

つまり起動直後の一括要求は「初回だけ機能が1つ欠ける」という判断ではありません。**そのモダリティを二度と使えないユーザーを、アプリを触る前の時点で作る**判断です。

後述する「最初の30秒で成功体験を1つ出す」という方針とも正面からぶつかります。ダイアログを3枚閉じ終えた時点で、30秒はほとんど残っていません。

### Just-in-Time — そのモダリティが実際に動く直前にだけ訊く

原則はひとつです。**権限は、そのモダリティが呼ばれる直前にだけ要求します**。マイクボタンを押し込んだ瞬間にマイクを、カメラを開く操作の直後にカメラを訊きます。ユーザーの側に「今この操作のために訊かれている」という因果が見えているぶん、許可率は素直に上がります。

Expo ベースの構成（Rork の生成物はここに入ります）では、カメラは `expo-camera` の `useCameraPermissions`、録音は `expo-audio` の `getRecordingPermissionsAsync` / `requestRecordingPermissionsAsync` が入口になります。両者を1つのフックにまとめ、**「許可」「拒否」「もう訊けない」の3状態**を呼び出し側に返すようにしておくと、UI 側の分岐が最後まで散らかりません。
...
// blocked = OS がもうダイアログを出してくれない状態（canAskAgain: false）
...
`denied` と `blocked` を1つにまとめないのが要点です。`denied`（まだ訊ける）なら次の機会に自然な文脈でもう一度出せますが、`blocked` なのに同じダイアログを期待して待っていると、**タップしても何も起きないボタン**が画面に残り続けます。シミュレータでは権限を毎回リセットできてしまうため、この状態は実機でしか踏めません。私は原因にたどり着くまで数日かかりました。

呼び出し側は、結果をそのまま `MultimodalContext` の状態に流し込みます。
...
const handleCameraPress = async () => {
  ...
  if (result === "blocked") {
    Alert.alert(
      "カメラを使うには",
      "設定でカメラを許可すると、撮った写真をそのまま質問に使えるようになります。",
      [
...
`DISABLE_MODALITY` を経由させておくと、権限拒否はグレースフルデグラデーションと同じ経路に合流します。**権限は「エラー」ではなく「利用できるモダリティが1つ減った状態」として扱う**、と決めてしまうほうが、実装も体験も素直になります。

### モダリティ別の要求タイミングと縮退先

<table>
<thead>
<tr><th>モダリティ</th><th>Info.plist キー</th><th>要求するタイミング</th><th>拒否されたときの縮退先</th></tr>
</thead>
<tbody>
<tr><td>カメラ</td><td><code>NSCameraUsageDescription</code></td><td>カメラボタンをタップした直後</td><td>写真ライブラリからの選択に切り替える</td></tr>
<tr><td>マイク</td><td><code>NSMicrophoneUsageDescription</code></td><td>録音ボタンを押し込んだ瞬間</td><td>テキスト入力に倒し、音声UIは畳む</td></tr>
<tr><td>写真ライブラリ</td><td><code>NSPhotoLibraryUsageDescription</code></td><td>「ライブラリから選ぶ」を選んだ直後</td><td>その場での撮影のみに限定する</td></tr>
<tr><td>音声認識（オンデバイス）</td><td><code>NSSpeechRecognitionUsageDescription</code></td><td>マイク許可が取れた直後・同じ操作の内側</td><td>録音を送り、サーバー側で文字起こしする</td></tr>
</tbody>
</table>

縮退先を先に決めておくと、「拒否されたらどうしよう」という分岐が設計の時点で消えます。**どのモダリティが落ちても、テキスト入力だけは必ず残る**——この一点さえ守れていれば、アプリが無反応になることはありません。

### 用途説明文（purpose string）を既定値のまま出さない

権限まわりで審査に落ちるとき、原因はコードではなく文言の側にあることがほとんどです。App Store Review Guideline 5.1.1 は、保護対象リソースの用途説明文について「使い道を具体的に述べ、多くの場合は使用例まで含めること」を求めています。「カメラを使用します」だけの一文では、この基準を満たしません。

- ❌ `カメラへのアクセスを許可してください`
- ✅ `撮影した写真をAIが解析し、写っているものの名前と手入れの方法をお答えするために使用します`

Expo 構成でここが落とし穴になるのは、**`expo-camera` などの config plugin が既定の説明文を自動で注入する**点です。何も指定しないと `Allow $(PRODUCT_NAME) to access your camera` に近い汎用の英文が入ったまま出荷されてしまいます。日本語圏向けのアプリでこれが残っていると、審査でも実際のダイアログでも損をします。`app.json` の plugin props、または `ios.infoPlist` で必ず自前の文言に置き換えてください。詳しい条文は [App Store Review Guidelines 5.1.1（Apple Developer）](https://developer.apple.com/app-store/review/guidelines/#data-collection-and-storage) を確認するのが確実です。

### 許可率はモダリティ別に記録する

最後に、地味ですが効く話をひとつお伝えします。`ensureCamera` / `ensureVoice` の戻り値を、モダリティ名を添えたイベントとして記録しておいてください。**どのモダリティが、どの画面で、どれだけ拒否されているか**が見えるようになると、「マイクの許可率だけ極端に低い」といった偏りが数字として出ます。私の場合、そこから遡って、要求のタイミングが1画面ぶん早すぎたことに気づけました。

ここを測っていないと、原因はモデル側にあると思い込んだまま、オーケストレーターを直し続けることになります。入力層より前で失われているユーザーは、AIの精度をどれだけ上げても戻ってきません。
...
1. 第1段: Gemini 2.0 Flash へまず投げます（画像があるなら強制 Gemini）。p50 で 800ms、p95 で 2.4 秒、課金は1リクエストあたり **約 ¥0.4** でした
2. 第2段: Gemini の応答に `confidence < 0.6` のシグナルが入ったときだけ、Claude Haiku に切り替えて再生成します。Claude まで流れるのは約 5% のリクエストです
3. 第3段: Claude も「分からない」を返したときに、ユーザーへ「もう少し情報を教えてください」と返します（**ここで2回目の入力を促すだけで再現率が23%上がりました**）
...
ここまで5つの層（入力統合・オーケストレーション・UXフロー・権限ゲート・エラーハンドリング）を順に扱ってきましたが、最初から全部きれいに作ろうとすると、私のように途中で機能が破綻します。
...
個人開発を長く続けてみて思うのは、マルチモーダルAIは「全部入りにする技術」ではなく、「**ユーザーが必要としたときに、必要なモダリティだけを、自然に差し出せるアーキテクチャ**」です。最初の機能を実装するときから、その器を用意できるかどうかで、半年後のアプリの姿は変わってきます。
