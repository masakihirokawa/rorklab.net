...
原則はひとつです。**権限は、そのモダリティが呼ばれる直前にだけ要求する**。マイクボタンを押し込んだ瞬間にマイクを、カメラを開く操作の直後にカメラを訊きます。ユーザーの側に「今この操作のために訊かれている」という因果が見えているぶん、許可率は素直に上がります。
...
最後に、地味ですが効く話をひとつ。`ensureCamera` / `ensureVoice` の戻り値を、モダリティ名を添えたイベントとして記録しておいてください。**どのモダリティが、どの画面で、どれだけ拒否されているか**が見えるようになると、「マイクの許可率だけ極端に低い」といった偏りが数字として出ます。私の場合、そこから遡って、要求のタイミングが1画面ぶん早すぎたことに気づけました。
