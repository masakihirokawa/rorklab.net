...
ここではRork で作った本番 AI アプリに [Langfuse](https://langfuse.com) を組み込み、（1）リクエスト単位のトレース、（2）モデル別・ユーザー別のコスト計測、（3）プロンプトや出力の品質スコアリング（Evals）を一気通貫で回す運用基盤の作り方をまとめました。Rork で AI を使っているけれど、月次のコストと品質に確信が持てない方に向けた実装ガイドです。
