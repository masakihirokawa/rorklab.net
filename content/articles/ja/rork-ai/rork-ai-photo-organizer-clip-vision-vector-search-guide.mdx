---
title: "Rork で『AI が自動でアルバム化する写真整理アプリ』を作る実装ガイド — Vision × CLIP × pgvector で本番品質のパイプラインを設計する"
...
Rork で「写真ライブラリを AI が自動で分類してくれるアプリ」を組むとき、難所は Vision の呼び出し方ではありませんでした。難所は、5 万枚という数がパイプラインのあちこちで静かに前提を壊していくところにあったのです。ここでは、その前提が壊れた場所と、私がどう線を引き直したかを、コードと一緒に書き残します。
...
実装では両方を併用します。Vision で「タグ」を付け、CLIP で「意味埋め込み」を付けておきます。検索時に両方を組み合わせるやり方がいちばん精度が高く、私が試した中でも最終的にこのハイブリッドだけが残りました。片方に寄せたほうが実装は軽くなるのですが、軽くしたぶんだけ「探せない写真」が増えていくのだと気づいたのは、自分のライブラリで半年ほど使い込んだあとでした。
...
class CLIPEmbeddingService {
  ...
  init() throws {
    ...
    // ⚠️ .centerCrop は写真検索では選ばないでください（後述の実測を参照）
  ...
  func embedImage(_ cgImage: CGImage) async throws -> [Float] {
    ...
    guard norm > 1e-6 else { throw NSError(domain: "CLIP", code: -2) }  // 全黒画像などで 0 になります
    return embedding.map { $0 / norm }
  }

  func embedText(_ text: String) async throws -> [Float] {
    // CLIP の BPE で 77 トークン固定長へ。前後に SOT / EOT を入れ、残りは 0 で埋めます
    ...
    // 画像側とまったく同じ正規化を通します。ここが揃わないと内積が意味を失います
    let norm = sqrt(embedding.reduce(0) { $0 + $1 * $1 })
...
テキスト側を「スコープ外」にして先へ進んでしまうと、あとで必ず戻ってくることになります。自然言語で検索する以上、クエリを同じベクトル空間へ落とす担当がいなければパイプラインは完成しないからです。私は最初、画像側だけ組んで満足してしまい、検索 UI に着手した段になって手が止まりました。

幸い、この部分を自分で書く必要はありません。Apple は画像エンコーダーとテキストエンコーダーの**両方**を Core ML 形式で公開しており、[apple/coreml-mobileclip](https://huggingface.co/apple/coreml-mobileclip) からそのまま取得できます。注意する点は 2 つだけです。

- **トークン長は 77 で固定**です。CLIP 系の系譜どおりの制約で、日本語の長いクエリは途中で切り落とされます。検索窓に文字数の目安を出しておくと、ユーザーが「なぜか後半が効かない」と迷わずに済みます。
- **画像側とテキスト側は必ず同じ variant で揃える**必要があります。`MobileCLIP-S1` の画像エンコーダーと `S2` のテキストエンコーダーを混ぜると、エラーにはならないまま検索結果だけが無意味になります。型が合ってしまうぶん、この取り違えは表に出てきません。

**同じ空間に射影されている保証は、モデルの組み合わせでしか担保されません。** ここだけは、動いているように見えても疑うようにしています。

### `.centerCrop` は写真検索では選ばない

先ほどのコードで `imageCropAndScaleOption` を `.scaleFill` にしたのには理由があります。`.centerCrop` は入力画像の中央の正方形だけを切り出してモデルへ渡すため、写真ライブラリのように縦横比がばらつく素材では、画面の端に写ったものが埋め込みから消えてしまうのです。

どれくらい消えるのかは、縦横比だけで決まります。手元で並べてみました。

<table>
<thead>
<tr><th>素材</th><th>元の画素数</th><th>centerCrop 後</th><th>残る面積</th></tr>
</thead>
<tbody>
<tr><td>標準カメラ 4:3（横／縦）</td><td>4032 × 3024</td><td>3024 × 3024</td><td>75.0%</td></tr>
<tr><td>16:9 の動画スクリーンショット</td><td>3840 × 2160</td><td>2160 × 2160</td><td>56.2%</td></tr>
<tr><td>iPhone 15 Pro のスクリーンショット</td><td>1179 × 2556</td><td>1179 × 1179</td><td>46.1%</td></tr>
<tr><td>パノラマ</td><td>10000 × 2000</td><td>2000 × 2000</td><td>20.0%</td></tr>
</tbody>
</table>

スクリーンショットで半分以上、パノラマに至っては 8 割が捨てられます。「あの夕焼けの海」を探したいのに、夕焼けが左端にしか写っていなければ、その写真は検索に出てきません。私はしばらくこれを「CLIP の精度が低い」と読み違えており、原因がモデルではなく前処理にあったのだと分かったときは、ばつが悪い思いをしました。

**精度を疑うのはあと、モデルに何が届いているかの確認が先。** 埋め込みを扱うときは、この順番を守るようにしています。

なお、`.scaleFill` は縦横比を無視して引き伸ばすため、細長い素材では被写体が歪みます。パノラマの比率が多いアプリなら、正方形へ 1 枚に潰さず、重なりを持たせて 2〜3 枚のタイルに切り、それぞれの埋め込みを保存して最大値を採る方法もあります。行数は増えますが、横に長い写真を扱うなら費用対効果は合います。
...
ところが、この関数は期待どおりに動かない場面があります。タグで絞り込むほど返ってくる枚数が減り、ひどいときには 30 件頼んだのに 3 件しか返らないのです。しかもエラーは出ません。

理由は、`where` 句が HNSW インデックスの**あと**に効くからです。インデックスはまず距離の近い候補をひとかたまり取り出し、そこへ `labels && filter_labels` が適用されます。候補のほとんどがフィルタで落ちれば、残りは当然 `limit` に届きません。ライブラリ全体では 800 枚あるビーチの写真が、ベクトル的に近い上位には 3 枚しか入っていなかった、というだけの話です。

pgvector 0.8.0 で入った反復スキャンが、この状況のための機能です。候補が足りなければグラフをさらに辿り、`limit` を満たすか探索を打ち切るまで候補を追加してくれます。
...
begin
  -- 絞り込みが効いた状態でも match_count 件を満たすまでグラフを辿ります
  set local hnsw.iterative_scan = 'relaxed_order';
  set local hnsw.max_scan_tuples = 20000;   -- 打ち切り上限（既定値）
  set local hnsw.ef_search = 100;           -- 1 回あたりの候補数。既定は 40
...
`strict_order` と `relaxed_order` のどちらを選ぶかは、UI が何を見せるかで決まります。厳密な距離順が要るなら `strict_order`、多少の前後を許して取りこぼしを減らしたいなら `relaxed_order` です。写真検索はサムネイルのグリッドで一覧するだけで、1 位と 3 位が入れ替わっても誰も気づきません。**順位の厳密さより、あるはずの写真が出てくることのほうが体験に効く**と考えて、私は `relaxed_order` を選んでいます。

`set local` にしているのは、関数の外へ設定を漏らさないためです。セッション単位で `set` してしまうと、反復スキャンが不要な他のクエリまで重くなります。

<Callout type="warning">
反復スキャンは pgvector 0.8.0 以降の機能です。Supabase のプロジェクトが古いままだと `unrecognized configuration parameter` で落ちます。`select extversion from pg_extension where extname = 'vector';` で先に確認しておいてください。0.8.0 未満なら、`filter_labels` をベクトル検索の前に効かせる（該当ラベルの `id` を先に絞ってから `where p.id = any(...)` で渡す）ほうが、結果的に速く確実です。
</Callout>
...
1. **HNSW インデックスを作らずに ivfflat を選ぶ**: 古い記事だと ivfflat を勧めるものが多いですが、pgvector 0.5+ なら HNSW の方が明確に高速で、構築時のチューニングも楽です。`m = 16, ef_construction = 64` から始めて、精度が足りなければ `m` を上げます。
...
2. **CLIP 埋め込みのバージョン管理を忘れると、検索精度が一晩で使いものにならなくなる**: モデルを `MobileCLIP-S1` から `S2` へ上げたとき、新旧の埋め込みが同じテーブルに混在し、まったく別のベクトル空間の値が距離計算に巻き込まれました。型は合っているので例外は出ず、検索結果だけが少しずつおかしくなります。`embedding_model_version` カラムを必ず持ち、バージョンの違うものは検索対象から外すか、再計算ジョブを積んでください。
...
Vision フレームワークの深掘りには、Apple の [WWDC24 のセッション「What's new in Vision」](https://developer.apple.com/videos/play/wwdc2024/10163/) が現時点で最も実装に近い情報源です。
...
私自身、最初のリリースは Vision だけの「タグ検索アプリ」でした。CLIP を足したのは、レビューや問い合わせで「もっと自由な言葉で探したい」という声が続いたあとです。順序が逆でなくてよかったと、いまも感じています。**設計は手元で、優先順位は使われ方から。** この順番だけは、締め切りが近い日でも崩さないようにしています。

関連する記事として、検索基盤の理論部分は [Rork × Supabase pgvector でセマンティック検索アプリを構築する](/articles/rork-ai/rork-supabase-pgvector-semantic-search-app) で別角度から解説しています。端末側でモデルを動かす部分は [Rork × Core ML カスタムモデル開発 × オンデバイス AI](/articles/rork-ai/rork-core-ml-custom-model-on-device-ai-guide) に、カメラ・音声・テキストをまたいで扱う設計は [Rork × マルチモーダル AI の設計パターン](/articles/rork-ai/rork-multimodal-ai-app-design-patterns) に整理しています。本記事と組み合わせて読んでいただけると、前後がつながるかと思います。

写真という、人によって中身がまるで違うデータを扱う話に最後までお付き合いくださり、ありがとうございました。