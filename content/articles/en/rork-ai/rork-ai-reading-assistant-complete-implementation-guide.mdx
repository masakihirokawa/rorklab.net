...
For a broader view of how multimodal context shapes AI UX, [Rork Multimodal AI App Design Patterns](/articles/rork-ai/rork-multimodal-ai-app-design-patterns) pairs well with this section.
...
### Pitfall 2: Believing That Smaller Images Lower Your Token Bill

My first version of the client downscaled every 4000px photo to 2048px before upload, purely to save on tokens. When I reconciled the invoices, the savings were essentially nil.

The reason lives in the API itself. Claude scales an image down server-side before counting tokens whenever its long edge exceeds 1,568 pixels or the image would exceed roughly 1,600 visual tokens. Send 4000px or send 2048px — both land on the same ceiling. The estimation formula is `width px × height px ÷ 750`, which caps out around 1,600 tokens once you are past the limit.

That does not make client-side resizing pointless. It buys you three other things: shorter upload times on a phone connection, a base64 payload that stays clear of request-size limits, and a lower failure rate on flaky networks. My mistake was optimizing for cost while actually improving latency, and never checking which one I had moved.

Here is the arithmetic with concrete assumptions ($3 in / $15 out per MTok, ¥155 to the dollar — substitute your own current rates):

<table>
<thead>
<tr><th>Operation</th><th>Input tokens</th><th>Output tokens</th><th>Approx. cost per call</th></tr>
</thead>
<tbody>
<tr><td>Bookshelf scan (1 image + 800-token instruction)</td><td>~2,400</td><td>~600</td><td>~¥2.5</td></tr>
<tr><td>Chapter summary (notes + context)</td><td>~1,500</td><td>~700</td><td>~¥2.3</td></tr>
</tbody>
</table>

Two or three yen per call is nothing on its own. The danger is in how it accumulates, which I unpack in the monetization section. Counting scans on the backend (Cloudflare KV or Postgres) is less a cost control than an <strong>instrument panel</strong> that makes that accumulation visible before it hurts.

### Pitfall 3: Readers Get Angry About Spoilers

The single most common complaint from beta testers was "you summarized parts of the book I had not reached yet." Models will happily do this even when the prompt says "no spoilers." The fix is two-layered. First, tighten the prompt to explicitly confine the summary to the chapters already read. Second, post-process the output with a cheap model pass (Gemini Flash works well) that flags spoiler phrases like "later in the book" or "eventually" and triggers a regeneration. How often a regeneration fires depends entirely on your prompt — in my case it was just under one summary in ten. As long as the spoiler check itself runs on a cheap model, that overhead stays in the noise.
...
My concrete plan shape looks like this. The free tier allows one bookshelf scan per month and three AI summaries per month — enough to experience the "aha" moment without giving the whole product away. The paid tier at ¥580 / ¥4,800 a year removes those limits, adds personal notifications, and unlocks data export. Offering an annual plan at a meaningful discount reduces churn because the subscriber pre-commits to an entire year of habit formation.

### The Free Tier Quietly Eats Your Margin

This is the part of reading-app economics that almost nobody models before launch. Take the per-call costs from the pitfalls section and stack them up.

A paid user who runs 8 scans and 40 summaries in a month costs roughly ¥113 in AI spend. Against ¥580 that is a 19.5 percent cost of goods — a number so comfortable that I stopped looking at it for a while.

The free tier is where it turns. A free user who exhausts their allowance (1 scan, 3 summaries) costs about ¥9.5. Small — until you apply a realistic ratio. At 20 free users per paying user, that is roughly ¥190 riding on the back of each subscriber. Total cost of goods climbs to about ¥303, or 52 percent. Layer on Apple and Google's 15–30 percent cut plus hosting, and the margin you thought you had is gone.

<table>
<thead>
<tr><th>Scenario</th><th>Monthly AI cost</th><th>Cost of goods vs. ¥580</th></tr>
</thead>
<tbody>
<tr><td>One paying user in isolation</td><td>~¥113</td><td>19.5%</td></tr>
<tr><td>One paying user + 20 free users</td><td>~¥303</td><td>52.2%</td></tr>
</tbody>
</table>

Squeezing the free allowance is only one lever, and not the most interesting one. Three that worked better for me, in order:

Change the <em>shape</em> of the free tier rather than its size. Granting three scans once at signup and none thereafter beat granting one scan every month — lower cost, higher conversion. A reading app makes its case during that first library import. Drip-feeding a small monthly allowance keeps users in a comfortable "this is probably enough" limbo indefinitely.

Split models by tier. Free-tier summaries run on a Haiku-class model; paid summaries run on Sonnet. Put the two side by side and the difference is legible to a reader — which turns a cost reduction into the clearest argument for upgrading you will ever write. I like this one because the economics and the pitch point in the same direction.

Instrument cost per user. An `ai_usage` table holding `user_id / feature / input_tokens / output_tokens / created_at` tells you immediately what share of spend the top one percent of users consume. In my case a very small group was driving most of the free-tier burn — the kind of fact you will never reach by reasoning about it from your desk.

Use a 14-day free trial, not the typical 7. Reading apps take longer for a user to experience the core value than, say, a photo editor does. A trial that ends just before the habit forms converts far worse than one that lets a user finish an actual book, and the extra week of AI spend per trial is a few yen against a subscription worth thousands over a year.

For pricing itself, [Rork App Subscription Pricing Design: 5 Principles to Maximize LTV Without Losing Retention](/articles/rork-business/rork-app-subscription-pricing-design-principles) covers the reasoning I use.

For the full RevenueCat + StoreKit 2 wiring, [Building Rork Subscriptions Around RevenueCat Entitlements](/articles/rork-dev/rork-revenuecat-entitlements-paywall-build-notes) covers the canonical setup.
