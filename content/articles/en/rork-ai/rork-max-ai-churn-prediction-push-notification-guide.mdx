...
For a long time I blamed that 40% on my copywriting. I rewrote those notifications more times than I want to admit. The number did not move.

What changed my mind was putting the notification log and the session log on the same timeline. The copy was never the problem. The problem was that the moment of delivery had nothing to do with what the person was actually doing.

Send "Haven't used the app in a while?" to someone who opened it two hours ago, and that single message teaches them the app isn't paying attention. Whatever the wording was, it disappears behind that conclusion.

And a revoked notification permission does not come back. No matter how politely you ask inside the app, very few people will walk back into iOS Settings to re-enable it. Push permission behaves less like a channel and more like a resource you can spend exactly once.
...
This is the number I got wrong myself, so let me work it out from actual token counts rather than intuition.

The prompt above runs about 450 tokens, and the JSON that comes back is roughly 80. Claude Haiku 4.5 bills $1 per million input tokens and $5 per million output tokens, which puts one notification here:

<table>
<thead>
<tr><th>Component</th><th>Per notification</th><th>300/day</th><th>30 days</th></tr>
</thead>
<tbody>
<tr><td>Input, ~450 tokens</td><td>$0.00045</td><td>$0.135</td><td>$4.05</td></tr>
<tr><td>Output, ~80 tokens</td><td>$0.00040</td><td>$0.120</td><td>$3.60</td></tr>
<tr><td><strong>Total</strong></td><td><strong>$0.00085</strong></td><td><strong>$0.26</strong></td><td><strong>$7.65</strong></td></tr>
</tbody>
</table>

As long as the 300/day cap holds, this never reaches ten dollars a month. Generation cost is not a reason to abandon the system.

What actually hurts is the cap coming off. Generate for all 10,000 users every day at the same unit price and you are at $8.50/day, $255/month. Cost blowups here come from a targeting bug, essentially never from the model choice.

If you want it cheaper anyway: notification copy has no latency requirement, so it fits the Batch API, which halves the bill and takes that $7.65 down to $3.83. Prompt caching on the fixed portion of the prompt trims the input side further.

Beyond cost, keeping Claude for scores of 50+ and hand-written templates for 30–49 also narrows the blast radius when a generation goes sideways. That, more than the money, is why I run it in two tiers.
...
**Make the job idempotent before you trust any of them**

Every option above can fire twice. A redeploy re-triggers the schedule, a timed-out request gets retried, and suddenly two runs are walking the same user list. Checking `last_churn_notification_at` won't save you, because both runs read it before either one writes.

Claiming a single slot for the day in a table does save you:
...
```typescript
// At the top of the Edge Function — proceed only if we won today's slot
...
With `ignoreDuplicates: true`, the upsert returns an empty array when the row already exists, so the return value is itself the "already ran today" signal. No separate lock required.

I added this after a retry sent the same person two win-back messages back to back. From their side that isn't churn prevention — it's just spam with extra steps. A batch that sends notifications deserves as much care about not sending twice as it does about not sending to the wrong people.
