...
## Making the Model Pick the Right Tool: Descriptions, Annotations, and Instructions

Registering a tool doesn't mean the model will reach for it. All ChatGPT can see is the name, the description, and the argument schema. When those are thin, a user says "help me get organized this week," no tool fires, and the model answers from general knowledge instead.

Here is a before and after from my own server.

<table>
<tr><th>Field</th><th>Before</th><th>After</th></tr>
<tr><td>Tool description</td><td>Creates a task.</td><td>Call this when the user mentions something they intend to do later. Do not use it to change an existing task &mdash; call update_task for that.</td></tr>
<tr><td>Argument <code>title</code></td><td>Task title</td><td>The user's own wording, copied verbatim. Do not summarize or rephrase.</td></tr>
</table>

Length is not what made the difference. What made the difference was stating <strong>both when to call it and when not to</strong>. Adding one sentence about the negative case visibly cut mix-ups between neighbouring tools. The same goes for `describe()` on each argument: "Task title" invites a bland paraphrase, while "copy the user's own wording" preserves what they actually said. These are prompts wearing the costume of type definitions.

The MCP TypeScript SDK also offers `registerTool`, which takes a config object and lets you declare `annotations` and `outputSchema` alongside the description. Where your SDK version supports it, this shape is easier to live with.
...
server.registerTool(
  ...
  {
    title: "Delete Task",
    description: "Call only when the user explicitly asks to delete. To mark something finished, call complete_task instead.",
...
Annotations are hints for the client, not the model. A tool marked `readOnlyHint` can run without a confirmation prompt; one marked `destructiveHint` gets one. Forgetting the hint on a delete tool is how conversational momentum turns into deleted data.

Rules that span tools belong in the server's `instructions`, not in any single description:

```typescript
const server = new McpServer(
  { name: "task-app", version: "1.0.0" },
  {
    instructions:
      "Always call list_tasks before updating or completing a task. Lists are capped at 50 items. If the user hasn't mentioned a date, omit due_date rather than guessing.",
  }
);
```

Clients are expected to surface `instructions` near the system prompt, so it shouldn't repeat what the tool descriptions already say. Keep it to ordering, limits, and things the model must not infer on its own.

One more lever: how you return errors changes how the model behaves. If your handler throws, the SDK still turns it into an error result, but what reaches the model is a raw exception string. Returning `isError: true` yourself lets you write the correction in your own words.

```typescript
if (due_date && !isValidDate(due_date)) {
  return {
    content: [
      { type: "text", text: `due_date must be YYYY-MM-DD. Received: ${due_date}` },
    ],
    isError: true,
  };
}
...
After that one branch went in, date-format mismatches started resolving themselves mid-conversation. The user sees nothing; the model quietly retries with the right shape. Write error text as instructions addressed to the model rather than to a human reading a log, and accuracy climbs.
...
server.tool(
  "upgrade_to_pro",
  "Call only when the user asks to move to a paid plan. If they merely ask about pricing, answer without calling this.",
