...
highlights:
  ...
  - "What Expo SDK 54 did to createUploadTask, the expo/fetch rewrite that replaces it, and the upload progress you give up by moving"
  - "Keeping the 20GB project-wide Files API ceiling visible once the bytes stop passing through your server, plus fps and media resolution as billing controls"
---
...
The per-user daily quota exists because issuing a destination is, in practice, a reservation for one analysis. Stopping someone here means the ceiling gets hit before the analysis is ever dispatched.
...
import * as FileSystem from 'expo-file-system/legacy'; // SDK 54 and later: see below
...
## The morning the function was gone

That code needs a footnote.

Expo SDK 54 rebuilt `expo-file-system`. The new API that shipped as `expo-file-system/next` back in SDK 52 was promoted to stable, the `next` entry point disappeared, and the old API moved to `expo-file-system/legacy`. Which means: leave your import as `expo-file-system`, bump the SDK, and `FileSystem.createUploadTask` is simply not there anymore. If you're lucky the types catch it. In plain JavaScript you get `undefined is not a function` at the moment the user picks a video, and it took me a few round trips through the logs to realize the failure wasn't in the upload — it was in the thing that was supposed to start the upload.

That's why the import above points at `expo-file-system/legacy`. It works. But the legacy upload helpers are deprecated as of SDK 54, and the recommended path now is `expo/fetch`.

The new `File` class implements the `Blob` interface, so it drops straight into an `expo/fetch` body. Since the second leg of a resumable upload is nothing but raw bytes, that's arguably a better fit than the helper ever was.

```typescript
// app/lib/uploadVideoModern.ts — the SDK 54+ shape
...
export async function putVideo(uploadUrl: string, localUri: string) {
  ...
  // File is a Blob, so it goes in the body directly and never lands in the JS heap
...
Shorter. No `FileSystemUploadType` to specify, so no way to confuse `BINARY_CONTENT` with `MULTIPART`.

There is a cost. **`expo/fetch` gives you no upload progress callback** — the very progress I was congratulating myself about one paragraph ago. On a thin connection, a three-minute video means roughly 40 seconds of silence.

<table>
  <thead>
    <tr><th></th><th>legacy createUploadTask</th><th>expo/fetch + File</th></tr>
  </thead>
  <tbody>
    <tr><td>SDK 54 status</td><td>Works, deprecated</td><td>Recommended</td></tr>
    <tr><td>Upload progress</td><td>Callback available</td><td>None</td></tr>
    <tr><td>Loads into JS heap</td><td>No</td><td>No</td></tr>
    <tr><td>Lines of code</td><td>More options to get wrong</td><td>Fewer</td></tr>
  </tbody>
</table>

I'm still on legacy for now. Uploading a video is the longest wait this app asks of anyone, and a progress bar that appears frozen there costs me more than one deprecation warning in the build log does. **Deprecated is not a deadline; it's notice that a destination exists.** I'd rather move once I can render progress myself.

If I were starting fresh, I'd probably go the other way. For anything where progress doesn't need showing — syncing a settings file, shipping a log bundle — `expo/fetch` is the simpler choice from day one.
...
There's a second ceiling that bites well before the 48 hours does: the Files API stores 20GB per project. Two gigabytes for any single file, twenty for everything you have.

Going direct made that number much harder to see. When the Worker carried the bytes, a day's volume was one log query away. Now the Worker knows only how many destinations it handed out.

At 96MB a video, roughly two hundred uploads fill the project. For an indie app that's not an abstract number — it's one decent week of attention after launch. And the failure surfaces during the `start` call, so from the user's side the symptom is "I pressed upload and nothing happened," which is the hardest kind of broken to explain.

So the issuer now tallies declared bytes alongside the count. It's a few lines on top of `issueUploadUrl`.

```typescript
// Track declared bytes, not just issuances, against the 20GB project ceiling
const bytesKey = `upload_bytes:${new Date().toISOString().slice(0, 10)}`;
const bytesUsed = Number((await env.SESSIONS.get(bytesKey)) ?? '0');

const SOFT_CEILING = 14 * 1024 * 1024 * 1024; // start sweeping at 14 of the 20
if (bytesUsed + sizeBytes > SOFT_CEILING) {
  // Don't block the issuance — clean up first
  await sweepExpiredFiles(env);
}
await env.SESSIONS.put(bytesKey, String(bytesUsed + sizeBytes), { expirationTtl: 172800 });
```

`sweepExpiredFiles` walks `files.list` and calls `files.delete` on anything already analyzed, reclaiming space without waiting out the 48 hours. The KV total is a sum of self-reported sizes, so it isn't an accurate balance. I treat it as a signal that it's time to sweep, nothing more.

**A limit you route around is a limit you stop seeing.** That was the most useful thing the rewiring taught me. Shortening a path makes it faster, and it also quietly retires whatever was doing the counting along the way. Writing down what the middle was measuring, before removing the middle, would have saved me this particular detour.
...
For the broader cost picture, [How I Cut My Rork App's AI Costs from $350 to $35/Month](/articles/rork-dev/rork-cloudflare-ai-gateway-cost-optimization-guide) covers the caching layer, and [Rork × Multimodal AI: Design Patterns for Camera, Voice, and Text](/articles/rork-ai/rork-multimodal-ai-app-design-patterns) covers handling these inputs together in one app.
...
For handling footage straight from the camera, [Rork × Vision Camera v4](/articles/rork-dev/rork-vision-camera-v4-frame-processor-production-guide) covers that side, and [Rork × LiveKit: Call Infrastructure for Production AI Voice Agents](/articles/rork-ai/rork-livekit-voice-video-agent-production-guide) covers the realtime side of audio and video.
