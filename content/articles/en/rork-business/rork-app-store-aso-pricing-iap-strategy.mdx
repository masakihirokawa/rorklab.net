...
author: "Rork Lab"
description: "How to take a Rork-built app from invisible to ranked, priced right, and earning real money on the App Store. ASO, price psychology, IAP design, and subscription tactics drawn from running indie apps solo."
...
A great app does not earn money on its own. After years of shipping and running indie apps on my own, I am certain of one thing: **how your app is found and sold on the store matters as much to revenue as how the app is built.**
...
## Deciding purchase vs. subscription with twelve months of numbers

The category guidance above is a starting heuristic. The hard cases are the apps sitting right on the line, and for those I would rather look at numbers than at instinct.

The script below runs the same install volume through both models for twelve months and prints where subscription overtakes a one-time purchase.

```js
// model-breakeven.mjs
// usage: node model-breakeven.mjs <monthly installs> <one-time price> <purchase CVR%> <sub price> <trial conversion%> <monthly churn%>
const [installs, oneTimePrice, oneTimeCvr, subPrice, trialConv, churn] =
  process.argv.slice(2).map(Number);

const FEE_FIRST_YEAR = 0.30; // first-year subscriptions and regular IAP
const FEE_SMALL_BIZ  = 0.15; // Small Business Program

let oneTimeCum = 0, subCum = 0, subscribers = 0, crossover = null;

for (let m = 1; m <= 12; m++) {
  // one-time: a single payment from that month's installs
  oneTimeCum += installs * (oneTimeCvr / 100) * oneTimePrice;

  // subscription: carry last month's base, subtract churn, add new conversions
  ...
  console.log(`${String(m).padStart(2)} | ${oneTimeCum.toFixed(0).padStart(6)} | ${subCum.toFixed(0).padStart(6)} | ${subscribers.toFixed(0).padStart(5)}`);
}

const net = (g, fee) => Math.round(g * (1 - fee));
console.log(`crossover month: ${crossover ?? "not within 12 months"}`);
console.log(`after 30% fee: one-time ${net(oneTimeCum, FEE_FIRST_YEAR)} / sub ${net(subCum, FEE_FIRST_YEAR)}`);
console.log(`after 15% fee: one-time ${net(oneTimeCum, FEE_SMALL_BIZ)} / sub ${net(subCum, FEE_SMALL_BIZ)}`);
console.log(`average subscriber lifetime: ${(100 / churn).toFixed(1)} months`);
```

The important line is the one that carries `subscribers` forward. A subscription is a balance that survives into next month minus churn, not a fixed amount added each month. Model it as a flat multiplication and you will overstate subscription revenue badly.

Run with 1,000 monthly installs, $2.99 one-time at 8% purchase rate, $3.99/month at 3% trial conversion and 8% monthly churn, on Node v22.22.3:

<table>
<thead><tr><th>Month</th><th>One-time cumulative</th><th>Subscription cumulative</th><th>Active subscribers</th></tr></thead>
<tbody>
<tr><td>1</td><td>$239</td><td>$120</td><td>30</td></tr>
<tr><td>3</td><td>$718</td><td>$681</td><td>83</td></tr>
<tr><td>4</td><td>$957</td><td>$1,105</td><td>106</td></tr>
<tr><td>12</td><td>$2,870</td><td>$7,075</td><td>237</td></tr>
</tbody>
</table>

Crossover lands in month 4. After the 30% fee, twelve months nets $2,009 from the one-time model and $4,952 from the subscription.

That gap hangs entirely on one input. Drop monthly churn from 8% to 5% and the same run nets $5,475 instead of $4,952, while average subscriber lifetime stretches from 12.5 to 20.0 months. <strong>Subscription revenue is decided by churn, not by price</strong> — that was the part that only became concrete once the two outputs sat side by side. How to actually move churn belongs to <a href="/articles/rork-business/rork-retention-ltv-maximization-guide">retention and LTV design</a>.

When you plug in your own numbers, read the crossover month against how long you honestly intend to keep shipping updates. If crossover falls later than that horizon, subscription is the wrong choice, and a one-time price is the more honest offer. For how the three models sit inside a full revenue mix, see <a href="/articles/rork-business/rork-app-revenue-model-comparison-ads-subscription-onetime">the ads, subscription, and one-time comparison</a>.


## The 70/85 boundary, seen over twenty-four months

When I wrote the script in the previous section, I collapsed the whole fee question into one constant, `FEE_FIRST_YEAR = 0.30`. For a twelve-month comparison that is fine. The trouble started when I tried to look further out. A subscription's split changes partway through, and my model kept applying the same number to the last month that it applied to the first.

Rereading the rules, I found I had been quietly translating "one year" into "one year on the calendar." What Apple counts is **days of paid service**, which is a different thing, and the gap between the two shows up in the ledger rather than in the model.

### Read the rules before the spreadsheet
...
<tbody>
<tr><td>First year</td><td><strong>70%</strong> of the subscription price at each billing cycle, minus applicable taxes</td></tr>
<tr><td>After one year</td><td>Once a subscriber accumulates one year of paid service, net revenue rises to <strong>85%</strong></td></tr>
<tr><td>Counts as paid service</td><td>Introductory, promotional, and offer-code offers that use paid pricing (pay as you go, pay up front)</td></tr>
<tr><td>Does not count</td><td>Free trials and renewal extensions</td></tr>
<tr><td>Scope</td><td>Per subscription group. Upgrades, downgrades, and crossgrades inside the same group leave the year intact</td></tr>
<tr><td>Interruptions</td><td>On expiry, accumulation stops. Renew within 60 days and it resumes; resubscribe after 60 days and it resets</td></tr>
<tr><td>Billing grace period</td><td>Recover inside the grace window (3, 16, or 28 days) and neither the days nor the revenue break</td></tr>
<tr><td>Small Business Program</td><td>Enrolled developers receive 85% from the start, regardless of accumulated paid service</td></tr>
</tbody>
</table>

The third row from the bottom is the one that earns its keep. If monthly churn sits near double digits, very few subscribers ever arrive at the one-year mark at all. The primary sources are [Auto-renewable Subscriptions](https://developer.apple.com/app-store/subscriptions/) and the [App Store Small Business Program](https://developer.apple.com/app-store/small-business-program/) pages.

### Running cohorts across twenty-four months

Same inputs as before, stretched to twenty-four months, with the split switching at the right moment. Each joining month becomes its own cohort, and the rate is chosen from that cohort's age.

```js
// sub-proceeds-horizon.mjs
// usage: node sub-proceeds-horizon.mjs <monthly conversions> <price> <monthly churn %> <months> [free trial months]
const [joins, price, churn, months, trial = 0] = process.argv.slice(2).map(Number);

const RATE_Y1 = 0.70;           // under one year of paid service
const RATE_Y2 = 0.85;           // one year of paid service reached
const PAID_MONTHS_TO_Y2 = 12;

// cohorts[k] = the group that joined k months ago; trial months never count as paid
let cohorts = [];
...
for (let m = 1; m <= months; m++) {
  ...
  for (const c of cohorts) {
    const paidMonths = c.age - trial;   // months that count as paid service
    if (paidMonths <= 0) continue;      // no billing during the trial
    const g = c.n * price;
  ...
  console.log(`${String(m).padStart(2)} | ${Math.round(heads)} | ${Math.round(y2Heads)} | ${grossCum.toFixed(0)} | ${netCum.toFixed(0)}`);
}

const pct = n => (n / netCum * 100).toFixed(1);
console.log(`effective rate ${(netCum / grossCum * 100).toFixed(2)}%`);
console.log(`70% band ${netY1Cum.toFixed(0)} (${pct(netY1Cum)}%) / 85% band ${netY2Cum.toFixed(0)} (${pct(netY2Cum)}%)`);
console.log(`vs. a flat 70% across the whole period: +${(netCum - grossCum * RATE_Y1).toFixed(0)}`);
console.log(`vs. a flat 85% (Small Business Program): +${(grossCum * RATE_Y2 - netCum).toFixed(0)}`);
```

Thirty conversions a month, $3.99 monthly, 8% monthly churn — the same figures as the previous section. Output on Node v22.23.2, milestones only:

<table>
<thead><tr><th>Month</th><th>Active</th><th>At the 85% rate</th><th>Gross cumulative</th><th>Net cumulative</th></tr></thead>
<tbody>
<tr><td>12</td><td>237</td><td>0</td><td>$7,075</td><td>$4,952</td></tr>
<tr><td>13</td><td>248</td><td>11</td><td>$8,065</td><td>$5,652</td></tr>
<tr><td>18</td><td>291</td><td>54</td><td>$13,562</td><td>$9,615</td></tr>
<tr><td>24</td><td>324</td><td>87</td><td>$21,029</td><td>$15,111</td></tr>
</tbody>
</table>

Month twelve reproduces the previous section's totals exactly, because the conditions are unchanged and only the horizon moved. What matters is what comes after: even at twenty-four months, revenue billed at 85% accounts for just 14.6% of net. The effective rate lands at 71.86%, which is $390 more than a flat 70% would have produced.

### Why the bump is small, and what is not

Look at who is left. Of the 324 active subscribers in month twenty-four, only 87 have a full year of paid service behind them. At 8% monthly churn the average lifetime is 12.5 months, so most subscribers leave shortly before the better rate would have applied. Drop churn to 5% and the 85% band grows to 19.6%; push it to 12% and it shrinks to 9.5%. <strong>The second-year 85% is a reward paid only to developers who fixed churn first.</strong>

The other comparison in the same output moves far more. Against a flat 85% — which is what Small Business Program enrollment gives you — the gap is $2,764 over the same twenty-four months, roughly seven times the $390 earned by waiting for subscribers to mature.

**I would rather claim the 15% I can apply for today than wait a year for the same 15%.** Enrollment is open to developers whose proceeds were at or below one million USD in the prior calendar year, and to developers new to the store. Cross that threshold during a year and the standard rate applies to later sales; fall back below it and you can requalify the year after.

Free trials deserve one line here too. Adding a one-month trial to the same run pulls twenty-four-month gross down to $18,156 and shrinks the 85% band from 14.6% to 13.5%, because trial days never count toward paid service and push the crossover later for every cohort. A longer trial buys conversion and pays for it partly in this delay.

I started running this calculation after a year-end review where the deposits came in lower than my own arithmetic had promised. The cause was not the price or the conversion rate. It was me, applying one fee to a revenue stream that charges two. Lowering churn itself belongs to <a href="/articles/rork-business/rork-retention-ltv-maximization-guide">retention and LTV design</a>.
...
In my experience an icon-only A/B test can move conversion 30-50%. This is revenue improvement that requires zero code changes. The Japanese keyword field has its own quirks around word segmentation, which I keep separate in <a href="/articles/rork-business/rork-app-store-keyword-field-japanese-aso-guide">the Japanese keyword field write-up</a>.

## When to believe an A/B test result

The 30-50% figure above describes what happens once enough impressions accumulate. Indie apps run on small numbers, and at a few hundred product page views most of what looks like improvement is noise.

Deciding the required volume before you start saves you from checking the dashboard every morning and reacting to nothing.

```js
// ab-page-views.mjs
// usage 1: node ab-page-views.mjs plan <baseline CVR%> <relative lift to detect%>
// usage 2: node ab-page-views.mjs check <A views> <A installs> <B views> <B installs>
...
function check(nA, cA, nB, cB) {
  ...
  const z = (pB - pA) / se;
  // two-sided p value, Abramowitz-Stegun approximation
...
if (mode === "plan") {
  ...
  console.log(`baseline ${a[0].toFixed(2)}% -> target ${(r.p2 * 100).toFixed(2)}%`);
  console.log(`views needed: ${r.n.toLocaleString()} per variant / ${(r.n * 2).toLocaleString()} total`);
} else {
  const r = check(a[0], a[1], a[2], a[3]);
  console.log(`A: ${(r.pA * 100).toFixed(2)}%  B: ${(r.pB * 100).toFixed(2)}%  relative ${(((r.pB / r.pA) - 1) * 100).toFixed(1)}%`);
  console.log(`z = ${r.z.toFixed(2)} / p = ${r.p.toFixed(4)} -> ${r.p < 0.05 ? "hard to explain as noise" : "still inside the noise band"}`);
}
```

Planning output at a 3% baseline conversion rate:

<table>
<thead><tr><th>Relative lift to detect</th><th>Target CVR</th><th>Views per variant</th><th>Total</th></tr></thead>
<tbody>
...
Smaller effects get expensive fast. Detecting a 10% relative lift takes over 100,000 total views, which is months of traffic for a new app. That is the practical argument for starting with a whole new icon rather than a reworded subtitle: big swings are the only ones a small app can actually measure.

The `check` mode is the one I reach for more often. Here is the same apparent 30% improvement at two different sample sizes:

<table>
<thead><tr><th>Views per variant</th><th>A</th><th>B</th><th>Relative</th><th>z / p</th><th>Verdict</th></tr></thead>
<tbody>
<tr><td>800</td><td>3.00%</td><td>3.88%</td><td>29.2%</td><td>0.96 / 0.3368</td><td>inside the noise band</td></tr>
<tr><td>8,000</td><td>3.00%</td><td>3.90%</td><td>30.0%</td><td>3.12 / 0.0018</td><td>hard to explain as noise</td></tr>
</tbody>
</table>

The headline improvement is nearly identical; the conclusion reverses. Call a winner at the first row and you will remember an ineffective change as an effective one, then carry it into your next app.

App Store Connect does report confidence intervals of its own. I still run the planning step first, because having the number written down beforehand is what lets me leave a running test alone.
