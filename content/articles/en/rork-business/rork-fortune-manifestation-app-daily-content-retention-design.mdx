---
title: "Building Fortune & Manifestation Apps with Rork — Daily Delivery Design That Survives Day Three"
...
## Measuring My Own PRNG — Two Holes I Didn't Expect

That function ran unchanged in my apps for a long time. What made me suspicious was a review, read late one evening, saying "I keep getting the same lines week after week." I was sure the pool was big enough, so my first guess was a stale cache on the device.

Guessing is a poor way to fix anything, so I pulled the function out and ran it on my machine. Twenty thousand synthetic user IDs, ten days each — 200,000 draws. Here is what came back.

<table>
<tr><th>Rank</th><th>Measured frequency</th><th>Rank</th><th>Measured frequency</th></tr>
<tr><td>1 (best)</td><td>5.03%</td><td>7</td><td>7.55%</td></tr>
<tr><td>2</td><td>15.13%</td><td>8</td><td>7.40%</td></tr>
<tr><td>3</td><td>8.83%</td><td>9</td><td>13.40%</td></tr>
<tr><td>4</td><td>8.69%</td><td>10</td><td>6.05%</td></tr>
<tr><td>5</td><td>8.71%</td><td>11</td><td><strong>0.00%</strong></td></tr>
<tr><td>6</td><td>16.22%</td><td>12 (worst)</td><td>3.00%</td></tr>
</table>

The numbers pointed straight at my own branching. `Math.floor(rand() * 4) + 3` returns 3 through 6, and `Math.floor(rand() * 4) + 6` returns 6 through 9. Ranks 6 and 9 sit on the seam between two branches, so they show up roughly twice as often as their neighbors. Meanwhile `Math.floor(rand() * 2) + 9` only ever yields 9 or 10, and the final `else` is pinned to 12. **Rank 11 never appeared once in 200,000 draws.** I had shipped a twelve-level system that only ever handed out eleven levels.

While I was there, I tested my other assumption: "I wrote 500 messages, so repeats can't happen for a while." Candidates are filtered by `rankMin` and `rankMax` only, so the real pool isn't the 25 messages in a cell — it's every theme in that band, 125 messages. I counted how many days it takes for a repeat when you draw one of those 125 independently each morning, across 5,000 simulated users.

<table>
<tr><th>Elapsed days</th><th>Users who saw the same message twice</th></tr>
<tr><td>Within 7 days</td><td>4.4%</td></tr>
<tr><td>Within 14 days</td><td>18.0%</td></tr>
<tr><td>Within 21 days</td><td>35.6%</td></tr>
<tr><td>Within 30 days</td><td>60.9%</td></tr>
</table>

The median first repeat landed on day 27. I had assumed three weeks of breathing room; in practice one user in three hit a repeat inside three weeks. This is the birthday problem wearing a different hat — drawing 21 times from 125 items, the odds of zero collisions sit below one in five. Growing the pool to 1,000 messages doesn't remove the curve. It only slides it to the right.

**Pool size doesn't guarantee freshness. The draw does.**

### Changing the Draw — A Per-Band Shuffle Bag

Storing history on a server would solve it, but that trades away the two properties I picked this design for: determinism and offline operation. What I use instead is a shuffle bag per rank-band. Convert the date into a running day number, divide it by the pool length, and treat the quotient as the "cycle" and the remainder as the position within that cycle. For each cycle, build a shuffled ordering from a user-specific seed and hand out messages in that order. The shuffle is deterministic, so opening the app five times in one day still returns the same reading.

```typescript
// src/lib/dailyFortuneBag.ts
// No repeats within a cycle — deterministic, offline, no stored history.
// mulberry32 / hash32 / RANK_LABELS are reused from dailyFortune.ts

const RANK_WEIGHTS = [3, 7, 10, 11, 12, 12, 13, 11, 9, 6, 4, 2]; // sums to 100
const RANK_TO_BAND = [0, 0, 0, 1, 1, 1, 2, 2, 3, 3, 3, 3] as const; // rank 1-12 → band

/** Map a 0-1 random value to rank 1-12 with no double-fed seams */
function pickRank(r: number): number {
  let acc = 0;
  for (let i = 0; i < RANK_WEIGHTS.length; i++) {
    acc += RANK_WEIGHTS[i] / 100;
    if (r < acc) return i + 1;
  }
  return RANK_WEIGHTS.length;
}

/** Seeded Fisher-Yates: the same seed always produces the same ordering */
function shuffleWithSeed<T>(pool: readonly T[], seed: number): T[] {
  const a = pool.slice();
  const rand = mulberry32(seed);
...
/** Turn a local "YYYY-MM-DD" into a running day number */
function dayIndex(dateISO: string): number {
  const [y, m, d] = dateISO.split("-").map(Number);
  return Math.round((Date.UTC(y, m - 1, d) - EPOCH_UTC) / 86400000);
}

type BagMessage = { themeId: string; band: 0 | 1 | 2 | 3; body: string };

export function generateDailyFortuneV2(
  userId: string,
  dateISO: string, // pass the return value of todayInUserTimezone()
  messages: readonly BagMessage[],
): FortuneResult {
  const day = dayIndex(dateISO);

  // Separate seeds for rank and copy, so tuning one doesn't shift the other
  const rank = pickRank(mulberry32(hash32(`${userId}:rank:${day}`))());
  const band = RANK_TO_BAND[rank - 1];

  const pool = messages.filter((m) => m.band === band);
  if (pool.length === 0) throw new Error(`No messages for band ${band}`);

  const cycle = Math.floor(day / pool.length);
  // Normalize the remainder so dates before the epoch don't go negative
  const position = ((day % pool.length) + pool.length) % pool.length;

  const bag = shuffleWithSeed(pool, hash32(`${userId}:bag:${band}:${cycle}`));
  const picked = bag[position];
...
I re-ran the same measurement. Across 2,000 users over 125 days, not one received the same message twice inside a cycle, and the rank distribution came out matching the weight table.

<table>
<tr><th>What I measured</th><th>Original implementation</th><th>Shuffle bag</th></tr>
<tr><td>Users hitting a repeat within 21 days</td><td>35.6%</td><td>0%</td></tr>
<tr><td>Users hitting a repeat within a 125-day cycle</td><td>— (60.9% repeat within 30 days)</td><td>0 of 2,000</td></tr>
<tr><td>Ranks that never appear</td><td>11</td><td>None (all 12 appear)</td></tr>
<tr><td>Server-side history required</td><td>No</td><td>No</td></tr>
</table>

One honest caveat. At a cycle boundary, the last message of the old cycle and the first of the new one can coincide. Counting across 1,500 users, that happened 0.73% of the time — the theoretical rate is 1/125, which is the band length. If it bothers you, swap the first two entries whenever the new cycle opens with the message the previous one closed on; it's a few lines. At this rate I've decided to leave it alone for now.

Everything I changed came to a few dozen lines. What let me trace a one-sentence review back to its cause wasn't intuition — it was counting. **Random code that runs is not the same as random code that's correct.**
...
## The Silent 64-Notification Ceiling — Rotating Copy Without Losing the Schedule

The code in the previous section uses a fixed `repeats: true` trigger. Run the app for a while and the next want shows up on its own: you start wanting the notification copy to rotate daily too. A line matched to today's rank should lift open rates, and it does.

That want pushes most implementations in the same direction — pre-schedule a batch of date-specific notifications. Write ninety days of copy, register ninety notifications in one pass. It works. For a while.

iOS keeps at most **64 pending local notifications per app**. Everything past the 64th is discarded with no exception and no warning. `scheduleNotificationAsync` still resolves. Nothing appears in the log.

That silence is the expensive part. An app that thinks it scheduled ninety days stops notifying on day sixty-five. In the numbers, that shows up as a single cohort — users about two months past install — dropping together. I misread it as content fatigue for a long stretch. Expanding the message pool did nothing, of course. The failure was on the notification side.

<table>
<tr><th>What you see</th><th>What is actually happening</th><th>How to confirm it</th></tr>
<tr><td>Only the ~2-months-post-install cohort loses retention</td><td>Everything past the 64-notification ceiling was silently dropped</td><td>Check whether <code>getAllScheduledNotificationsAsync()</code> caps out at exactly 64</td></tr>
<tr><td>Notifications stop entirely from one day onward</td><td>The re-arm runs only on app launch and never fires when the app opens from a notification tap</td><td>Log the re-arm separately for cold start and notification-tap launches</td></tr>
<tr><td>Notifications shift by an hour after a DST change</td><td>Absolute <code>date</code> triggers were queued with the pre-change offset baked in</td><td>Switch the device to a timezone that observes DST and reproduce</td></tr>
<tr><td>The same reading appears two days in a row after international travel</td><td>The date used in the notification identifier is UTC-based and drifts a day from local</td><td>Change the device timezone and print the generated identifiers</td></tr>
<tr><td>Android delivery lags by minutes to tens of minutes</td><td>Doze deferral; exact-time delivery needs an extra permission on Android 12+</td><td><code>adb shell dumpsys deviceidle force-idle</code> to force Doze and reproduce</td></tr>
</table>

The fix is to stop scheduling the full horizon and switch to a **14-day rolling window**, re-armed every time the app comes forward. You stay far below the ceiling, and the copy can still differ per day.

```typescript
// src/lib/rearmDailyNotifications.ts
// Re-arm per-day notification copy on a 14-day rolling window
...
/** Build "YYYY-MM-DD" from local time — never from UTC */
...
export async function rearmDailyNotifications(
  ...
  // Cancel only what this module scheduled, so other features survive
  ...
  for (let i = 1; i <= WINDOW_DAYS; i++) {
    ...
    await Notifications.scheduleNotificationAsync({
      ...
      // A date trigger is what lets the copy vary per day
      trigger: { type: "date", date: fireAt } as Notifications.NotificationTriggerInput,
    });
    scheduled++;
  }

  // Safety net for users who do not open the app for 15+ days
  await Notifications.cancelScheduledNotificationAsync(FALLBACK_ID).catch(() => {});
  await Notifications.scheduleNotificationAsync({
    identifier: FALLBACK_ID,
    content: {
      title: "Today's message is waiting",
      body: "A few new lines have gathered while you were away",
    },
...
Where you call this matters more than the function itself. Hooking `AppState` transitions to `active` is not enough. A launch that comes from tapping a notification has to pass through the re-arm as well. Miss that path and the users who only ever open the app through notifications quietly disappear after fourteen days — which is precisely your most habituated segment.
...
export function useNotificationRearm(rearm: () => Promise<number>) {
  useEffect(() => {
    void rearm(); // on launch

    const sub = AppState.addEventListener("change", (state) => {
      if (state === "active") void rearm();
    });
    // notification-tap launches must pass through here too
...
The single fallback notification deserves a word. A rolling window assumes the app gets opened; the moment it stops being opened, the mechanism switches itself off. Leaving one plain `repeats: true` notification at the tail costs nothing and keeps a door open for users who might come back. Whether that entry exists visibly changed my reactivation rate.

Android has no 64-item ceiling, but it has Doze instead. If you truly need minute-level precision you are in WorkManager territory rather than `expo-notifications`, and Android 12+ requires a permission for exact alarms. Daily-content notifications do not need that precision. I let Android deliver late on purpose and treat a 7:15 design as a 7:00–7:30 band, which turns out to be the more stable way to think about it anyway.
...
The way I've come to hold it is that the app borrows the form of a prayer. Omikuji slips and old almanacs never hid how the draw worked, and people still found meaning in them. Transparency and meaning are not in conflict. Once you accept that premise, the implementation decisions get concrete quickly.
...
After all of this design talk, the single concrete thing worth trying tomorrow is to add one line of logging to your delivery code and count how many days it takes for the same message to come back to the same user. That's where I found out my own three-week assumption was wrong. Growing the pool can wait until after you've counted — and counting tells you how far you actually need to grow it.

I still wrestle with the same design problems every time I ship a new app. There's no finished playbook here — just a long sequence of small hypotheses, tested in production. I'd be glad to keep learning alongside you. Thanks for reading.