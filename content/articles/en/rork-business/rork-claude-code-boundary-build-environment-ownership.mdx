...
There is a **fourth state** the branch doesn't cover, though. `expo prebuild --clean` rebuilds `ios/` from scratch, so by the time you run the check the directory itself may be gone. Run `check` in that state and you get this:

```
Conflict: generation rewrote your native settings
-> move the affected lines into a config plugin
```

It isn't a conflict. The raw message from `git apply --check` is `error: ios/Info.plist: No such file or directory` — there is simply nothing to apply the patch to yet. Read that as a conflict and you'll start a config-plugin migration for a problem that a single `prebuild` would have solved. I only avoided that because I happened to be watching the raw `--check` output; trusting the script's wording alone would have cost me the afternoon.

Telling the two apart takes one pass over the patch's target paths before the existing branch runs:

```bash
    # insert at the top of the check branch
    missing=0
    while IFS= read -r f; do
      [ -e "$f" ] || { echo "  missing target: $f"; missing=$((missing+1)); }
    done < <(grep '^+++ b/' "$PATCH" | sed 's|^+++ b/||')
    if [ "$missing" -gt 0 ]; then
      echo "Native directories not generated yet (${missing} missing). Run prebuild first."
      exit 0
    fi
```

Exiting 0 is deliberate, since this isn't a failure. Wired into CI, a pre-prebuild job shouldn't go red for it. **What deserves to fail the build is moving forward without noticing something vanished — not the fact that it hasn't been generated yet.** Blur that line and the check stops being taken seriously.
...
Taking the three scripts above into your own setup, these are the traps I hit or nearly hit. Each one quietly turns the numbers into a lie, and none of them announces itself as a failure.
...
### 4. Deleted files, and renamed ones, are still being counted

I found this one by going back and doubting my own script, and it's the worst-behaved of the four. What `git log --name-only` hands you is every path that ever appeared in the history — not the files that exist now. So once you've moved an `ios/` setting into a config plugin and deleted the original, that path keeps counting as native work forever.

I reproduced it in a scratch repository: six JS screens, five native config files, a dozen screen updates, and then `App.entitlements`, `Podfile` and `gradle.properties` moved into config plugins and removed.

<table>
<thead>
<tr><th>Counting method</th><th>Native config</th><th>Config &amp; plugins</th><th>JS/TS</th><th>Landing rate</th></tr>
</thead>
<tbody>
<tr><td>Every path in the history (the original script)</td><td>5</td><td>4</td><td>6</td><td>60.0%</td></tr>
<tr><td>Only files that still exist</td><td>2</td><td>4</td><td>6</td><td>50.0%</td></tr>
</tbody>
</table>

Ten points on the landing rate, but look at the native row on its own: five files reported, two real. Six out of ten are ghosts. And the error only ever runs one direction — the further along a migration gets, the more deleted paths pile up, so **the projects that worked hardest to get out of the native layer keep reporting the highest landing rates**. The number drifts toward "you should own the build environment" exactly when the opposite became true.

In the same experiment I renamed one screen file, and the JS row went from six to seven. The old path and the new one count as two separate files.

One change covers both. Intersect the history's paths with what git currently tracks:

```bash
# replace the input to the while loop in native-landing.sh with these two lines
git ls-files | sort -u > /tmp/alive.txt
git log -n "$LIMIT" --name-only --pretty=format: | grep -v '^$' | sort -u \
  | comm -12 - /tmp/alive.txt
```

`comm -12` emits only the lines present in both. Deleted paths and pre-rename paths are absent from `git ls-files`, so they drop out on their own. That the rename double-count disappears too was not something I predicted.

The tradeoff is that you lose the view of how much native work you did historically. What I want to know is **which layer I'll be maintaining from here**, so I count the surviving files — but in a month where the migration progress itself is the question, I run both and read them side by side. The gap between them is how much you've put down.
...
Run `native-landing.sh` against your own project. It takes a couple of minutes, and the number will probably differ from what you expect. Put the `comm -12` lines in before you do — without them, the landing rate reads high by however much of the migration you've already finished.
