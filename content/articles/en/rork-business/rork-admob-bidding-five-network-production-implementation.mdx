...
The second item reads like one line and behaves like three. The ad unit IDs have to be resolved *after* `fetchAndActivate()` has settled — if you let that promise float, the session that runs on rollback day still requests against the old IDs. This is the shape I use in the wallpaper apps.

```tsx
// hooks/useAdUnitIds.ts
import { useEffect, useState } from "react";
import remoteConfig from "@react-native-firebase/remote-config";

// Defaults point at the pre-migration set on purpose. A device that cannot
// reach Remote Config should fall back to serving, not to serving nothing.
const LEGACY = {
  banner: "ca-app-pub-XXXXXXXXXXXXXXXX/1111111111",
  interstitial: "ca-app-pub-XXXXXXXXXXXXXXXX/2222222222",
};
const BIDDING = {
  banner: "ca-app-pub-XXXXXXXXXXXXXXXX/3333333333",
  interstitial: "ca-app-pub-XXXXXXXXXXXXXXXX/4444444444",
};

export const useAdUnitIds = () => {
  const [ids, setIds] = useState(LEGACY);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    const load = async () => {
      try {
        await remoteConfig().setDefaults({ ads_use_bidding: false });
        await remoteConfig().setConfigSettings({
          // Default is 12 hours. I ship 1 hour for the first migration week.
          minimumFetchIntervalMillis: 60 * 60 * 1000,
        });
        await remoteConfig().fetchAndActivate();
        const useBidding = remoteConfig().getValue("ads_use_bidding").asBoolean();
        setIds(useBidding ? BIDDING : LEGACY);
      } catch {
        // On failure we stay on LEGACY.
      } finally {
        setReady(true);
      }
    };
  ...
  // No ad requests while ready is false.
  return { ids, ready };
};
```

The `ready` flag is the part I would not skip again. When the hook returned only `ids`, the very first request after launch went out against the old unit, and an app I had supposedly rolled back kept reporting a mix of both sets. It took me two evenings to find that, and the fix was four lines.

`minimumFetchIntervalMillis` defaults to 12 hours, and if you leave it there, a rollback you trigger in the morning starts reaching devices in the evening — which rather defeats the point of having a switch that needs no rebuild. I now ship the shortened interval as its own build during the migration week, and only flip bidding on the week after.

**Ship the way back before you want to take it.** Of everything in this rollback design, the timing of the build turned out to matter more than the steps themselves.
