...
## Each storefront indexes two locales, not one

Everything so far has treated locales one at a time. Users, though, stand in a country's store — and a single store usually indexes two locales at once.

Japan indexes Japanese alongside English (U.S.). Mainland China indexes Simplified Chinese alongside English (U.K.). The United States indexes English (U.S.) alongside Spanish (Mexico). Each country has its own pair, and if you only ship one half of a pair, the other hundred characters sit unused in that market.

<table>
  <thead>
    <tr><th>Store</th><th>Locales indexed</th><th>Easy to miss</th></tr>
  </thead>
  <tbody>
    <tr><td>United States</td><td>en-US + es-MX</td><td>Loanwords land in both, so the same term gets paid for twice</td></tr>
    <tr><td>Japan</td><td>ja + en-US</td><td>Your U.S. English field is what Japanese users see in English search</td></tr>
    <tr><td>Mainland China</td><td>zh-Hans + en-GB</td><td>You probably ship en-US only, leaving en-GB empty</td></tr>
    <tr><td>Germany</td><td>de-DE + en-GB</td><td>Same gap — none of your English terms reach it</td></tr>
    <tr><td>Canada</td><td>en-CA + fr-CA</td><td>Easy to assume en-US covers it</td></tr>
  </tbody>
</table>

I started paying attention to these pairs while trying to work out why English terms produced nothing in mainland China. The only English locale I shipped was en-US, and that store reads en-GB. Terms cannot rank in a field that does not exist. The cause was not the terms and not the implementation — the field simply was not there.

The second half of this matters just as much: repeating a term inside a pair does not double its weight. The index counts a word once. Putting `aesthetic` in both en-US and es-MX means one of those ten-character spends buys nothing. Loanwords are where this happens, because when you write Spanish keywords yourself, the terms that already work in English are the ones your hand reaches for.

The same JSON file will tell you where both problems are.

```js
#!/usr/bin/env node
// aso-storefront-lint.mjs — treat the locales a storefront indexes as one pair,
// then report duplicates inside the pair and halves you never shipped.
import { readFileSync } from 'node:fs';

// Copy only the countries you actually ship from App Store Connect's
// supported-languages list, and revisit the table once a year.
const STOREFRONTS = {
  US: ['en-US', 'es-MX'],
  JP: ['ja', 'en-US'],
  CN: ['zh-Hans', 'en-GB'],
...
const LIMIT = 100; // keywords field only
const len = (s) => [...(s ?? '')].length;
const norm = (s) => (s ?? '').normalize('NFKC').toLowerCase().trim();
const tokensOf = (s) => (s ?? '').split(',').map(norm).filter((t) => t !== '');
const CJK = /[぀-ヿ㐀-鿿]/u;

function lintPair(locales, data) {
  const present = locales.filter((l) => data[l]);
  const missing = locales.filter((l) => !data[l]);
  const lines = [];

  const used = present.reduce((a, l) => a + len(data[l].keywords), 0);
  const budget = locales.length * LIMIT;
  lines.push(`${used} of ${budget} indexed characters in use (${present.join(' + ') || 'none'})`);

  for (const l of missing) {
    lines.push(`EMPTY  ${l} not localized (${LIMIT} characters unavailable in this storefront)`);
  }

  // Duplicates within the pair, with the same CJK containment check as before
  if (present.length === 2) {
    const [a, b] = present;
    const surfaceB = norm(`${data[b].name ?? ''} ${data[b].subtitle ?? ''} ${data[b].keywords ?? ''}`);
    const setB = new Set(tokensOf(data[b].keywords));
    const dup = tokensOf(data[a].keywords).filter(
      (t) => setB.has(t) || (CJK.test(t) && t.length >= 2 && surfaceB.includes(t)),
    );
    if (dup.length) {
      const saved = dup.reduce((s, t) => s + len(t) + 1, 0);
      lines.push(`DUP    ${a} and ${b} share ${dup.join(' / ')} (counted once; ~${saved} characters recoverable)`);
    }
  }
  return lines;
}
...
const path = process.argv[2];
if (!path) {
  console.error('usage: node aso-storefront-lint.mjs <metadata.json>');
  process.exit(2);
}
const data = JSON.parse(readFileSync(path, 'utf8'));

for (const [store, locales] of Object.entries(STOREFRONTS)) {
  const [head, ...rest] = lintPair(locales, data);
  console.log(`[${store}] ${head}`);
  for (const l of rest) console.log(`     ${l}`);
}
```

The input is the file from the previous section, with the post-comma spaces and duplicate terms cleaned out of en-US and an es-MX block added.

```json
  "en-US": {
    "name": "Beautiful HD Wallpapers",
    "subtitle": "4K backgrounds for your phone",
    "keywords": "aesthetic,live wallpaper,lock screen,nature,minimal"
  },
  "es-MX": {
    "name": "Fondos de Pantalla HD",
    "subtitle": "Imagenes 4K para tu telefono",
    "keywords": "aesthetic,fondos gratis,pantalla de bloqueo,naturaleza,minimal"
  }
```

```text
$ node aso-storefront-lint.mjs metadata.sample.json
[US] 113 of 200 indexed characters in use (en-US + es-MX)
     DUP    en-US and es-MX share aesthetic / minimal (counted once; ~18 characters recoverable)
[JP] 73 of 200 indexed characters in use (ja + en-US)
[CN] 19 of 200 indexed characters in use (zh-Hans)
     EMPTY  en-GB not localized (100 characters unavailable in this storefront)
[DE] 0 of 200 indexed characters in use (none)
     EMPTY  de-DE not localized (100 characters unavailable in this storefront)
     EMPTY  en-GB not localized (100 characters unavailable in this storefront)
```

Two loanwords cost eighteen characters in the U.S. pair. Mainland China has an entire hundred-character field sitting empty in a place no amount of polishing the Japanese or U.S. fields can reach. Germany is using none of its two hundred.

**A term goes into a pair once. When half a pair is missing, create the field before you choose the terms.** Those two rules are what I check when I am unsure whether to add another locale.

One caveat worth stating plainly. Apple does not publish this pairing as an indexing specification. My own table comes from App Store Connect's supported-languages list plus switching the store language and seeing what actually renders. Before adding a country, I look at how my app displays in that store first — slower than assuming, and far faster than being wrong in sixteen locales at once.
...
3. Run `node aso-storefront-lint.mjs metadata.json` and write down the countries where you ship only half a pair
4. Spend the recovered characters — but only on terms already on your candidate list, not new ideas
5. Record the live date and review date, then leave the numbers alone for two weeks

Step four is deliberate. Reorganizing the budget and swapping terms at the same time means that when numbers move, you cannot tell which change did it. Close the holes first, then reconsider the contents. Since adopting that order, my decisions got visibly faster.
