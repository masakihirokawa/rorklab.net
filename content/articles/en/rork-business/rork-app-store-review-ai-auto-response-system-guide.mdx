...
## I Finally Measured the Language Detector Instead of Guessing at It

In the traps section above I mentioned that I stopped trusting `locale` and started deciding the reply language from the character mix in the review body. Re-reading this article, I realised something uncomfortable: when I first picked 30% as the cutoff, I never actually measured anything. It felt about right, the obvious mistakes stopped, and I left it there.

So I sat down and measured it. I wrote out eleven review bodies in the shapes people actually send, and compared two ways of counting.

- Naive: Japanese characters as a share of all characters in the body
- Word-aware: split with `Intl.Segmenter`, then count words containing Japanese characters

<table>
  <thead>
    <tr><th>Shape of the review body</th><th>Naive</th><th>Word-aware</th><th>Language I want to reply in</th></tr>
  </thead>
  <tbody>
    <tr><td>Japanese only</td><td>0.91</td><td>1.00</td><td>Japanese</td></tr>
    <tr><td>Japanese + product names (iCloud, Wi-Fi)</td><td>0.63</td><td>0.82</td><td>Japanese</td></tr>
    <tr><td>Japanese + a short exception name</td><td>0.19</td><td>0.70</td><td>Japanese</td></tr>
    <tr><td>Japanese + a long crash log</td><td>0.05</td><td>0.21</td><td>Japanese</td></tr>
    <tr><td>Japanese + a full stack trace</td><td>0.05</td><td>0.20</td><td>Japanese</td></tr>
    <tr><td>English + one Japanese phrase</td><td>0.27</td><td>0.50</td><td>English</td></tr>
    <tr><td>English only</td><td>0.00</td><td>0.00</td><td>English</td></tr>
  </tbody>
</table>

**The more effort a Japanese user puts into pasting a log, the more likely a naive character ratio is to answer them in English.** A body as ordinary as "it crashes on open, I see Fatal Exception: NSInvalidArgumentException" scores 0.19 and falls below the 30% line. Someone who pasted a whole stack trace scored 0.05. The people taking the most care to tell me what broke were the ones getting a reply in the wrong language.

Counting by word instead of by character moves those same bodies to 0.70 and 0.20. That rescues a body with one exception name in it. Long logs and stack traces still sit around 0.20.

My next instinct was to lower the threshold, so I swept it in 0.05 steps. Whatever single line I chose, two of the eleven came out wrong. Japanese with a long log (0.21 and 0.20) and English with one Japanese phrase in it (0.50 both times) overlap on the ratio axis. **One line cannot separate them** — that is the part I only learned by measuring.

What needed fixing was not where the line sat, but how many lines there were.

```typescript
// src/ai/replyLanguage.ts
// Decide the reply language from the character mix. Send the ambiguous band to manual.

const JP_CHAR = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/u;

/** Share of word-like segments containing Japanese characters (not a character ratio) */
export function japaneseWordRatio(text: string): number {
  const segmenter = new Intl.Segmenter("ja", { granularity: "word" });
  const words = [...segmenter.segment(text)].filter((s) => s.isWordLike);
  if (words.length === 0) return 0;
  const jp = words.filter((w) => JP_CHAR.test(w.segment)).length;
  return jp / words.length;
}

export type LanguageRoute =
  | { kind: "auto"; language: "ja" | "body" }
  | { kind: "manual"; ratio: number };

const HIGH = 0.6;  // above this, reply in Japanese
const LOW = 0.15;  // below this, reply in the language of the body

export function routeReplyLanguage(text: string): LanguageRoute {
  const ratio = japaneseWordRatio(text);
  if (ratio >= HIGH) return { kind: "auto", language: "ja" };
  if (ratio <= LOW) return { kind: "auto", language: "body" };
  return { kind: "manual", ratio };  // 0.15-0.60 never reaches auto-send
}
```

With the band in place, those eleven bodies split six automatic and five manual, and nothing on the automatic side came out wrong. Close to half going manual looks expensive until you see who lands there: the person who pasted a log, and the person who dropped one Japanese phrase into an English review. Both are reviews I'd rather read myself before answering. What I wanted to cut was never the effort — it was the rudeness.

If `Intl.Segmenter` isn't available in your runtime, keep the naive count and widen the band instead. You can't sharpen the judgment, so you widen the range where you decline to judge.

**Don't let something unjudgeable pass as if it had been judged.** It's the same line I drew for the hard-block topics, applied to language. Before you go back to nudging a threshold from 0.30 to 0.20, run ten real review bodies through your own detector and look at the ratios side by side. My 30% was a number I had never measured.
