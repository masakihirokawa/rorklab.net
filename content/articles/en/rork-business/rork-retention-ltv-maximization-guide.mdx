...
## Recounting what "active" actually contains

The reason I couldn't quite disbelieve that 78% is that it described people who *can currently use the app* — not people who will pay again. At the end of one month I sorted the RevenueCat event log by date and finally looked at the composition. Three genuinely different states were sitting inside that single active count.

Some subscribers are on track to renew. Some have already tapped cancel and are using the app until their expiration date. Some have a failed payment and are being held in a billing grace period. The last two groups are restricted in no way at all inside the app — and, it turned out, in no way inside my numbers either.

I split the end-of-period active count three ways for one of my apps and lined up three months.

<table>
  <thead>
    <tr>
      <th>Month</th>
      <th>Active at period end</th>
      <th>Expected to renew</th>
      <th>Cancel scheduled</th>
      <th>Billing issue (grace)</th>
      <th>Expected-renewal rate</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
What surprised me was the steadiness: roughly a fifth of the active count is always made up of people who have already decided to leave. And because they drop on their expiration dates, they leave in clusters — so a monthly average reads as "last month was fine, this month is bad" when nothing actually changed. I noticed it in a month where a group who had been sitting in cancel-scheduled for three weeks all disappeared on the same day.

**Retention counts who can use the app. The expected-renewal rate counts who will pay next month.** The LTV formula wants the second one. After swapping it in, the figure I had been looking at came down to about four-fifths of what I thought it was.

Splitting the three apart on the client takes a few lines.

```ts
// Pull only "expected to renew" out of a RevenueCat EntitlementInfo.
...
export type RenewalState =
  | 'renewing'         // on track to renew next period
  | 'cancel_scheduled' // cancelled, still usable until expiration
  | 'billing_issue'    // payment failed, held by the grace period
  | 'trial';           // free period, first charge not yet cleared

export function classifyRenewal(e: EntitlementInfo): RenewalState {
  if (e.periodType === 'TRIAL' || e.periodType === 'INTRO') return 'trial';
  if (e.billingIssueDetectedAt) return 'billing_issue';
  if (e.unsubscribeDetectedAt || e.willRenew === false) return 'cancel_scheduled';
  return 'renewing';
}

export async function tagRenewalState(entitlementId = 'pro') {
  const info: CustomerInfo = await Purchases.getCustomerInfo();
  const e = info.entitlements.active[entitlementId];
  if (!e) return null; // non-subscribers exit here

  const state = classifyRenewal(e);
  // Never used for access control — this is a label for counting only.
  await analytics().setUserProperty('renewal_state', state);
  return { state, expiresAt: e.expirationDate };
}
```

One decision matters more than the code: entitlement checks stay on `isActive`, and the classification lives only in `renewal_state`. Take access away from someone in a grace period and you've evicted a person who would have come back the moment their card cleared. Access is decided by `isActive`; the split exists only when I count — I had that order backwards for a month once, and a support email is how I found out.

Rather than sampling once at month end, I'd rather accumulate the `CANCELLATION`, `BILLING_ISSUE`, and `RENEWAL` webhooks daily. The client-side check only sees people who open the app, so it misses the cancellations of people who have stopped opening it. I keep both: webhooks for the daily count, the SDK for what the screen shows.

I'd also rather not read a scheduled cancellation as a complaint. Using out the period you already paid for is a calm decision, and dropping a discount into that moment may only disturb it. I send nothing to people in cancel-scheduled, then once — after the expiration date has passed — a note about what's new.
