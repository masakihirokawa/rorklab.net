...
Access control orthodoxy says deny by default: no evidence, no entry. That is how I wrote it first, without much thought.
...
What made fail open the right call here only landed for me once I hit the screen myself. On failure, fall back to the last config that did arrive; if there is none, open the gate.
...
## When OTA updates shift the value you are comparing

Moving this implementation onto an Expo app surfaced one more mix-up. There is more than one value inside the app that reads like "the current version."

<table>
<thead>
<tr><th>Value</th><th>What it points at</th><th>Changes on OTA update?</th></tr>
</thead>
<tbody>
<tr><td><code>Application.nativeApplicationVersion</code> (expo-application)</td><td>The installed binary's <code>CFBundleShortVersionString</code> / <code>versionName</code></td><td>No</td></tr>
<tr><td><code>Constants.expoConfig?.version</code> (expo-constants)</td><td>The app config version carried by the JS bundle currently running</td><td>Yes</td></tr>
<tr><td><code>Updates.runtimeVersion</code> (expo-updates)</td><td>The compatibility unit between native and JS</td><td>No — updates only ship within the same runtime</td></tr>
</tbody>
</table>

I had been handing the gate the second one, `Constants.expoConfig?.version`. It behaves correctly on my desk. It drifts the moment an OTA update lands.

Ship a JS bundle whose app config version has been bumped, and the store binary stays where it was while the gate's idea of "current version" moves up. It clears the threshold, so the devices you meant to stop walk right through. The reverse happens too: set the threshold to the newest store build, and a device that has not received the OTA reports an older version, taps through to the store, and finds a binary with the very number it already has.

A forced update is telling the user one thing — reinstall from the store. So the value to compare is the number of the binary sitting on the store, which is the first row.

```js
import * as Application from "expo-application";

// Points at the store-delivered binary; OTA never moves it
const storeVersion = Application.nativeApplicationVersion ?? null;

export function shouldBlockForStoreUpdate(minSupported) {
  // null in Expo Go and on web: no evidence, so open (same direction as fail-open)
  if (!storeVersion) return false;
  return isBelowMinimum(storeVersion, minSupported);
}
```

When what I need to stop is a JS-side mismatch — the API contract moved and the bundle has not caught up — I keep that on `Updates.runtimeVersion` rather than letting it ride the same gate. The two ask the reader for different actions: one sends them to the store, the other asks them to close the app and open it again. Put both behind one screen and you get a state where neither instruction can be followed.

I went back and forth on opening the gate where `nativeApplicationVersion` returns `null`. Letting development builds through may leave a hole in my own testing. Even so, not bending the "no evidence, no closing" rule per environment is what I'd rather keep.
