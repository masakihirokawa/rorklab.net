...
Let me answer the most common question first: yes, you can stay on the free plan indefinitely. Nothing expires out from under you. The constraint isn't whether you can keep using it — it's **whether it carries you to a shipped app**.
...
### Find the Month Where Annual Overtakes Monthly

"Annual is cheaper" is only true **if you stay**. Knowing which month annual pulls ahead takes one division — but laying the mid-year exit costs next to it changed how I read the discount.

```js
#!/usr/bin/env node
// annual-payback.mjs — find the month annual overtakes monthly, and what leaving early costs.
// Check the official pricing page and substitute your own numbers.
// Usage: node annual-payback.mjs <monthly price> <annual total>
const [monthly, annual] = process.argv.slice(2).map(Number);
if ([monthly, annual].some(Number.isNaN)) {
  console.error("Usage: node annual-payback.mjs <monthly price> <annual total>");
  process.exit(1);
}
const breakEven = annual / monthly;

console.log(`Annual overtakes monthly at month ${breakEven.toFixed(1)} (a full year saves you ${(12 - breakEven).toFixed(1)} months' worth).`);
for (const m of [3, 6, 9, 12]) {
  const byMonthly = monthly * m;
  const diff = annual - byMonthly;
  const verdict = diff > 0
    ? `monthly would have cost $${diff.toFixed(0)} less`
    : `annual saved you $${(-diff).toFixed(0)}`;
  console.log(`  leaving at month ${String(m).padStart(2)}: monthly $${byMonthly} / annual $${annual} → ${verdict}`);
}
```

Running it with an illustrative $25 monthly and $250 annual:

```
$ node annual-payback.mjs 25 250
Annual overtakes monthly at month 10.0 (a full year saves you 2.0 months' worth).
  leaving at month  3: monthly $75 / annual $250 → monthly would have cost $175 less
  leaving at month  6: monthly $150 / annual $250 → monthly would have cost $100 less
  leaving at month  9: monthly $225 / annual $250 → monthly would have cost $25 less
  leaving at month 12: monthly $300 / annual $250 → annual saved you $50
```

Month ten reframes the whole decision. Annual isn't "two months off" — it's **prepaying for confidence that you'll still be here in ten months**. And that $175 at month three isn't a discount you missed; it's money that doesn't come back.

One more thing worth saying: estimates of how long you'll keep using something skew optimistic. I subtract a month or two from my own answer before comparing it to the break-even point. If it still clears, annual is fine.
...
If I compress those three stages into one line: **pay for what you need, only once you need it.** What I remember isn't the months I overpaid — it's the stretches where my hands stopped while I was still deciding.
...
If you do one thing today, feed this month's three numbers into `credit-pace.mjs` and look at where you land. The plan question resolves much faster once that line is on screen.

Thank you for reading this far. If it helped you stop second-guessing the plan picker, that's enough for me.
