...
Apps shipped with Rork are no different. If you skip SDK setup or rush the SKAdNetwork 4.0 conversion-value design, every dollar of ad spend becomes a guessing game. What follows is the exact stack I now run on multiple indie apps — how I pick between AppsFlyer and Adjust, and how the pieces get wired into a Rork (Expo + React Native) project that holds up in production.
...
Before the working version, here's the one I shipped first. It reads perfectly well, and it has two holes in it.

```typescript
// ❌ My first attempt. Looks fine, loses measurement in two places.
let cumulativeScore = 0;

export async function reportEvent(event: Event): Promise<void> {
  const next = Math.min(63, cumulativeScore + SCORE_MAP[event]);
  if (next > cumulativeScore) {
    cumulativeScore = next;
    appsFlyer.updateConversionValueWithErrorCallback(next, (e) => console.warn(e));
  }
}
```

The first hole is that `cumulativeScore` lives in process memory. Kill the app, reopen it the next morning, and it starts from 0 — so every value you compute from there is lower than what you already reported. SKAN accepts monotonically increasing values only, so the low value is dropped. No error, no warning. The shape of the failure is oddly specific: everything up to onboarding looks right, and the 32-point trial signal never arrives.

The second hole is that only `fine` is ever set. Under SKAN 4.0, `fine` can only ride the first postback (activity up to two days post-install); postbacks two and three can carry `coarse` only. If you never set a coarse value, those postbacks come back as `coarse: none`. That is the most common reason a dashboard shows a healthy window 0 and nothing after it. I had written a whole section about coarse buckets while my code was still SKAN 3.0.

**Designing for 4.0 while implementing 3.0 fails silently — the report just goes empty.**

Here's the version that closes both.

```typescript
// src/lib/conversion-value.ts
import AsyncStorage from '@react-native-async-storage/async-storage';
import appsFlyer from 'react-native-appsflyer';
import { updateSkanConversionValue, type CoarseValue } from './skan';
...
const STORE_KEY = 'skan:cumulative-fine';

// Derive coarse from where fine landed. Tune the cutoffs to your own LTV distribution.
function toCoarse(fine: number): CoarseValue {
  if (fine >= 32) return 'high';   // reached trial
  if (fine >= 8) return 'medium';  // first content view and beyond
  return 'low';
}
...
export async function reportEvent(event: Event, payload?: Record<string, unknown>): Promise<void> {
  ...
  // 2. Read the cumulative score from storage — a module variable resets to 0 on relaunch
  const current = await readScore();
  const next = Math.min(63, current + SCORE_MAP[event]);
  if (next <= current) return;

  await AsyncStorage.setItem(STORE_KEY, String(next));

  // 3. Update fine and coarse together; close the window once the trial signal lands
  await updateSkanConversionValue({
    fine: next,
    coarse: toCoarse(next),
    lockWindow: event === 'trial_started',
  });
}
```

`updateSkanConversionValue` is a thin bridge over `SKAdNetwork.updatePostbackConversionValue(_:coarseValue:lockWindow:)`, available from iOS 16.1. It's about thirty lines with the Expo Modules API, so there's no need to wait for your MMP's SDK to expose it.

```swift
// modules/skan/ios/SkanModule.swift — a thin Expo Modules bridge
...
public class SkanModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("update") { (fine: Int, coarse: String, lockWindow: Bool, promise: Promise) in
      // Below iOS 16.1 there is no coarse value at all, so send fine only.
      guard #available(iOS 16.1, *) else {
        if #available(iOS 15.4, *) {
          SKAdNetwork.updatePostbackConversionValue(fine) { error in
            if let error { promise.reject("skan", error.localizedDescription) } else { promise.resolve(nil) }
          }
        } else {
          promise.resolve(nil)
        }
...
One boundary matters here. If you've enabled a server-side conversion-value product such as AppsFlyer's SKAN Conversion Studio, do not also update values from the app. Whichever writes last wins, and debugging that is miserable. **Pick one owner for the conversion value — the app or the server — and keep it.** I keep it in the app today, though the server side gets more attractive as the number of media partners grows, so I expect to revisit that.

Setting `lockWindow: true` sends the postback without waiting out the rest of the window. Reserve it for a point where you know the score can't move again, such as trial start; setting it eagerly throws away measurement opportunities for everything that follows.

Expected output: open `Cohort Report → SKAN Postbacks` in the AppsFlyer dashboard and you'll see a `fine` distribution in postback window 0, and low/medium/high `coarse` distributions in windows 1 and 2. As long as windows 1 and 2 read `none`, your coarse value still isn't landing.
...
7. **Keeping the cumulative score in memory only.** As above, a relaunch resets it to 0 and every later update is dropped by the monotonic guard. To reproduce it locally: reach onboarding-completed, force-quit the app, relaunch, then push through to trial start. If the postback's `fine` is still stuck at 4, you're in this hole.
...
```typescript
// src/lib/attribution-adjust.ts — react-native-adjust v5
...
export function initAdjust(appToken: string) {
  ...
  // v4 used Adjust.create(config). v5 renamed this to initSdk.
...
Check your major version before you copy anything. react-native-adjust v5 moved the entry point: the v4 `Adjust.create(config)` is now `Adjust.initSdk(config)`, and pasting an older sample gets you `Adjust.create is not a function` at launch. Session-level callback parameters were renamed too — `addSessionCallbackParameter` became `addGlobalCallbackParameter`, while the event-level `addCallbackParameter` stayed put. Adjust's [SDK v5 migration guide](https://dev.adjust.com/en/sdk/migration/react-native/v4-to-v5/) lists the rest.

The most common stumbling block in an AppsFlyer → Adjust migration is having to re-implement the ATT consent flow. Both SDKs wrap the native ATT API, but their preferred initialization order differs. Adjust will internally wait for ATT to resolve even if you call `Adjust.initSdk` early, which makes the launch sequence feel marginally snappier than the AppsFlyer equivalent. You may also need to update your CMP rules — Adjust expects "tracking" consent in slightly different terminology than AppsFlyer.
...
const SENT_KEY_PREFIX = 'skan:sent:';

// The listener also fires on renewals. This is the gate that keeps each signal to one send.
async function sendOnce(event: 'trial_started' | 'paid_started', payload: Record<string, unknown>) {
  const key = `${SENT_KEY_PREFIX}${event}`;
  if (await AsyncStorage.getItem(key)) return;
  await AsyncStorage.setItem(key, '1');
  await reportEvent(event, payload);
}
...
export function bindRevenueCatToSKAN() {
  Purchases.addCustomerInfoUpdateListener((info: CustomerInfo) => {
    ...
    const payload = { product: active.productIdentifier };
    // periodType returns 'TRIAL' / 'NORMAL' / 'INTRO' — mind the casing
    if (String(active.periodType).toUpperCase() === 'TRIAL') {
      void sendOnce('trial_started', payload);
    } else {
      void sendOnce('paid_started', payload);
    }
  });
}
```

In the earlier version, both branches sent `trial_started`. The comment promised a clamp to the top of the range for the first paid period; the code sent the same event either way. SKAN's monotonic guard swallows the duplicate without an error, so the only symptom is a `high` bucket that never grows. Read both branches of a conditional out loud — that's the habit I've settled on after this one.

`customerInfo` also fires on renewals. Sending every time is mostly harmless thanks to the monotonic guard, but your event counts inflate and you lose the ability to tell where the first purchase actually was. Persisting a "sent" flag, as above, pays off the first time you have to reconstruct a cohort from logs. Don't forget to add `paid_started` to `SCORE_MAP`.
