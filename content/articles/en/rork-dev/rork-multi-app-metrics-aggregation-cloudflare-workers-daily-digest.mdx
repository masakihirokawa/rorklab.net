...
| Source | Metrics | Auth | Freshness | Rate limit |
| --- | --- | --- | --- | --- |
| App Store Connect API | downloads, revenue, IAP counts per app | JWT (ES256) | T-1 day | 200 req/min |
| RevenueCat REST v1 | MRR, retention, cancellations | Bearer Token | near real-time | 1,200 req/min |
| AdMob Reporting API | eCPM, impressions, revenue per app | OAuth 2.0 | T-1 day | 750 req/min |
| Firebase Crashlytics (via BigQuery export) | crash-free users, impacted users | Service Account | T-1 day (requires export) | BigQuery quotas |
...
| Item | Before (manual) | After (Workers aggregation) |
| --- | --- | --- |
| Morning metrics review | 30–40 min | under 5 min |
| Anomaly-to-awareness latency | 4–6 hours on average | immediate at 08:00 |
| Missed crash-rate spikes (six months) | 3 | 0 |
| AdMob eCPM-drop response time | next afternoon | same morning |
