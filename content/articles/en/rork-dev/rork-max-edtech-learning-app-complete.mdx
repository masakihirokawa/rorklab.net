...
For detailed auth configuration with Supabase, see the [Firebase & Supabase Auth Guide](/articles/rork-dev/rork-auth-firebase-supabase).
...
For a deeper dive on Stripe subscription setup, see the [Rork Max Stripe Subscription Guide](/articles/rork-dev/rork-max-stripe-subscription).
...
For state management patterns that keep this dashboard performant, see the [Rork Max State Management Guide](/articles/rork-dev/rork-max-state-management).
...
For the full offline-first architecture pattern, refer to the [Offline-First Architecture Guide](/articles/rork-dev/rork-max-offline-first-architecture).
