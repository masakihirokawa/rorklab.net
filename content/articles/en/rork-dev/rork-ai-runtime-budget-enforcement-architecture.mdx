...
| Layer | Responsibility | Purpose | When it fires |
| --- | --- | --- | --- |
| Client | Pre-flight estimation, UI gating | Prevent unnecessary calls | Before the request |
| Edge (Workers) | Per-user / per-app balance check | Block abuse and runaway | At request ingress |
| Origin | Hard caps, model restrictions | Last-resort provider-side limit | At request arrival |
