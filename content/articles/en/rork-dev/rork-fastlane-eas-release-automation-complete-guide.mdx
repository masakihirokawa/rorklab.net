...
lane :ios_beta do
  ...
  #    Use the store-distribution profile (`beta` in eas.json). Leaving this on
  #    `preview` produces an Ad-hoc signed build that TestFlight will reject (pitfall 2)
  ...
  UI.user_error!("Build history is empty — did `eas build` finish?") if build.nil?
...
The `dig` call is deliberate too. When the most recent build failed, or is still queued, `eas build:list` returns that build with `artifacts` set to `null`. Chaining square brackets crashes with `NoMethodError: undefined method for nil` at exactly that point, so the friendly `UI.user_error!` you wrote one line below never runs. Switching to `dig` and printing `status` alongside the failure means one glance at the CI log tells you whether the build is still running or whether it died — a distinction worth having at 1 a.m.
...
`concurrent_simulators(true)` is 2–3× faster, but below 16 GB of RAM Simulator tends to crash. On Apple Silicon GitHub runners (`macos-15` and newer) I keep it on. On local Intel Macs I keep it off.
...
lane :android_beta do
  ...
  build = JSON.parse(build_json).first
  artifact_url = build&.dig("artifacts", "buildUrl")
  UI.user_error!("Could not resolve the AAB URL") if artifact_url.to_s.empty?
...
jobs:
  ios:
    ...
    runs-on: macos-15          # Apple Silicon by default. Never macos-latest (pitfall 5)
    steps:
...
### 1. Assuming `Invalid token` means the API key expired

The first time CI started failing with `Invalid token`, my opening hypothesis was that the App Store Connect API key had expired. That hypothesis cost me half a day, because it is wrong.

The `.p8` private key itself has no expiry. What expires is the JWT signed with that key, and Apple caps its lifetime at 20 minutes. Fastlane mints a fresh token during the lane, so this is almost never something you touch directly.

<table>
<thead>
<tr><th>Credential</th><th>Expires?</th><th>How it actually dies</th></tr>
</thead>
<tbody>
<tr><td>API key (<code>.p8</code> private key)</td><td><strong>No</strong></td><td>Only when revoked in App Store Connect. A revoked key cannot be reinstated</td></tr>
<tr><td>JWT signed with that key</td><td><strong>20 minutes max</strong></td><td>Times out; Fastlane reissues it per run</td></tr>
<tr><td>Google Play service account key</td><td>No</td><td>Deleted in Google Cloud, or its Play Console role is removed</td></tr>
</tbody>
</table>

So when `Invalid token` shows up, work through these three in order instead:

- **Mismatched `key_id` / `issuer_id`.** If you belong to more than one team, it is easy to paste another team's `issuer_id` into Secrets.
- **Someone revoked the key.** Common after a credential audit, or when a collaborator cleans house.
- **Runner clock drift.** If the JWT's `iat` lands in the future, Apple rejects it immediately. I hit this on a self-hosted runner whose NTP sync had quietly stopped.

Passing a raw PEM while `is_key_content_base64: true` is set produces the same error, incidentally. Running `bundle exec fastlane run app_store_connect_api_key ...` once on your own machine before wiring it into CI keeps key problems and CI problems in separate boxes, which is most of the debugging battle.
...
### 5. Pinning a runner label that then gets retired

`macos-latest` eventually bumps Xcode under you, and the day it does, your `snapshot` tests break. That much is well known.

The sharper edge is that the label you pinned can disappear before you're ready. GitHub's macOS 14 images — including `macos-14` and `macos-14-xlarge` — entered phased deprecation on July 6, 2026 and become fully unsupported on November 2, 2026. During the wind-down, jobs on those labels are deliberately failed at intervals to make the change impossible to miss. This guide originally recommended `macos-14-xlarge`; if you're building the pipeline today, pin **`macos-15`** instead (Apple Silicon by default), or `macos-26` if you want to track the newer image.

Pinning is not a decision you make once. Put a twice-yearly reminder on your calendar to check the [actions/runner-images announcements](https://github.com/actions/runner-images/issues) and re-pin. That single habit is what separates "our CI is stable" from "our CI was stable until one Tuesday morning."
