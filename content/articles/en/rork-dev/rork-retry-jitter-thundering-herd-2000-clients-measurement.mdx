...
author: "Rork Lab"
description: "My backend came back but the 429s did not stop. I rebuilt the scenario in a 2,000-client discrete simulator and measured what plain exponential backoff, delay caps, 10% jitter, and honoring Retry-After cost in seconds — then reran everything with server capacity that degrades under load."
...
## Measuring again, with capacity that degrades

There is a lie buried in every table above. The simulator treats server capacity as a flat 120 req/sec.

Real servers do not behave that way. Under load the queue grows, GC kicks in, the connection pool drains. The more traffic arrives, the less of it the server can actually clear.

I had been carrying that caveat around without checking it. Adding the effect turned out to be a single expression, so I stopped putting it off.

```js
// Discount effective capacity by how far arrivals exceed nominal capacity
...
`EXP` controls how sharply capacity falls away, `FLOOR` sets the floor below which it never drops. Starting with the gentler setting, here is the comparison against the flat-capacity runs.

<table>
  <thead>
    <tr><th>Strategy</th><th>99% done (flat capacity)</th><th>99% done (degrading)</th><th>Change</th><th>Requests after recovery</th></tr>
  </thead>
  <tbody>
    <tr><td>Fixed 5s</td><td>74.0s</td><td>135.2s</td><td><strong>+82.7%</strong></td><td>9,433 → 22,827</td></tr>
...
What stood out is how little the peak-suppressing strategies cared about the change in assumptions.

Full jitter, peaking at 150 QPS, moved by 0.4%. Decorrelated jitter, peaking at 298, moved by 4.0%. Fixed 5s, peaking at 1,168, moved by 82.7% — and its request count swelled from 9,433 to 22,827.

Rejected, retried, capacity drops further, rejected harder. Once that loop starts turning, peak QPS stops being a momentary number and becomes the reason the tail keeps stretching.

**Peak QPS protects your server, and it also protects you when your assumptions turn out to be wrong.**

Does a harsher collapse widen the gap? I ran EXP=1.0 with FLOOR=0.2 as well.

<table>
  <thead>
    <tr><th>Strategy</th><th>99% done (harsh collapse)</th><th>Requests after recovery</th><th>Peak QPS</th></tr>
  </thead>
  <tbody>
    <tr><td>Fixed 5s</td><td><strong>241.6s</strong></td><td>45,961</td><td>1,168</td></tr>
...
Fixed 5s lands at 241.6 seconds and 45,961 requests — 4.9x the flat-capacity run. An implementation without jitter only gets worse as the assumptions get harder.

The reversal I expected never arrived. I had assumed that once capacity degraded, full jitter's lower peak would pull it ahead of decorrelated jitter. It came out 85.0s against 80.8s, in the same order as before.

The gap did close, though. Ten and a half seconds under flat capacity, four point two under the harsh collapse. With a steeper curve, or a lower floor, the order may well flip somewhere — I may be missing a regime where it does.

I am not confident enough to claim where that point is. What I can hand you is the finding that the order held while the margin shrank, plus the two constants to turn if your own setup disagrees.
...
The open question now is the initial burst at the 30 second mark. Those requests were already scheduled while the service was down, so no client-side strategy removes them. Throttling on the server side is the next thing worth measuring.

That the order survived degrading capacity genuinely surprised me. I had expected the peak-suppressing strategy to come out ahead, and only the margin moved. Being wrong is usually where the measurement earns its keep.
