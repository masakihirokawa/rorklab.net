...
export function buildKey(
  ...
  const pairs = Object.entries(query)
    ...
    .sort(); // absorb ordering differences here
...
```ts
// The entry shape with a controller is built in the next section
export function resetInflight(reason: string) {
  for (const [key, entry] of inflight) {
    entry.controller.abort(); // waiters receive an AbortError
    inflight.delete(key);
  }
...
Where the entitlement itself comes from is a separate topic, but if you hold subscriptions on your own server, reading this beside [Outgrowing RevenueCat: A Self-Hosted Guide to App Store Server Notifications V2 for Rork Apps](/articles/rork-dev/rork-app-store-server-notifications-v2-self-hosted-subscription) makes it easier to decide what belongs in the key.

### What must never go into the key
...
1. A cache buster built from `Date.now()`
2. A trace ID minted per request
3. A `JSON.stringify` of an options object that contained the `AbortSignal`
...
## Trap three: the request count doubles in development only

The morning after I added reference counting and called it done, I opened the inspector again and `/me/entitlements` was still listed twice. One of the two carried the abort marker. I had counting in place and still could not get to one request, and for a while I just sat there.

React StrictMode was behind it. In development builds, StrictMode runs every effect twice in the order setup → cleanup → setup. From the point of view of `useCoalescedQuery`, that means `release` is called once immediately after mount. Even with three screens asking for the same key, the moment all three of those first cleanups run, `refs` drops to zero, the in-flight request is aborted right there, and the second round of setups fires a fresh one.

I reproduced the same ordering locally and counted the fetches that actually ran, with three callers asking for one key.

<table>
  <thead>
    <tr><th><code>release</code> implementation</th><th>Fetches that ran</th><th>Aborted</th><th>Completed</th></tr>
  </thead>
  <tbody>
    <tr><td>Immediate <code>abort()</code> (the code above)</td><td>2</td><td>1</td><td>1</td></tr>
    <tr><td>Grace-period <code>release</code></td><td>1</td><td>0</td><td>1</td></tr>
  </tbody>
</table>

StrictMode is off in release builds, so none of this doubling reaches your users. But if the number you look at during development is always twice the real one, your fold-rate instrument is wrong and so is every decision you draw from it. While I was rereading the coalescer convinced the implementation was broken, what was actually broken was the premise of the measurement. If any of this looks familiar, I may be missing a subtler cause in your case, but StrictMode is worth suspecting before the code is.

The fix is plain enough. Do not abort the instant `refs` hits zero — wait for one beat. If someone comes back during that beat, cancel the abort you scheduled.
...
const inflight = new Map<string, Entry<unknown>>();
const ABORT_GRACE_MS = 0; // one setTimeout tick is enough to span StrictMode's round trip
...
export function coalesce<T>(
  ...
  if (!entry) {
    const controller = new AbortController();
    const promise = run(controller.signal).finally(() => {
      inflight.delete(key); // trap one: always delete on settle
    });
    entry = { promise, controller, refs: 0, pendingAbort: null };
    inflight.set(key, entry as Entry<unknown>);
  }

  // someone came back, so cancel the abort we scheduled
  if (entry.pendingAbort !== null) {
  ...
  let released = false; // calling the same release twice must do nothing
  const release = () => {
    if (released) return;
    released = true;
    entry!.refs -= 1;
...
A grace of `0` is enough, because a single `setTimeout` already spans the gap between StrictMode's cleanup and its second setup. On tabs the user flips back and forth, stretching it to 50–100ms also saves the refetch when they return. The longer you make it, though, the longer a genuinely abandoned request keeps running. What worked for me was leaving the default at zero and raising it only on the few screens that see heavy back-and-forth.

The `released` flag earns its line too. As the conditions around a caller grow, cleanups really do run twice. Without the flag, `refs` goes negative and the abort fires while someone is still waiting. When I checked it locally, one of two callers calling `release` twice was enough to hand the other one — which had done nothing wrong — an `AbortError`.

**The right to stop something shared belongs to the last person leaving, one beat after they are gone.** I thought reference counting had already drawn that line, and StrictMode showed me it had not. Looking back, the hard part was never the counting itself but deciding when the counting is finished.
...
<table>
  <thead>
    <tr><th>Layer</th><th>Protects against</th><th>Lifetime</th><th>Wrong fit for</th></tr>
  </thead>
  <tbody>
    <tr><td>Single-flight</td><td>Concurrent duplicate <strong>reads</strong></td><td>Until the one request settles</td><td>Writes of any kind</td></tr>
    <tr><td>TTL cache</td><td>Repeated <strong>refetches</strong> over a short window</td><td>The TTL you set</td><td>Values that must be current</td></tr>
    <tr><td>Idempotency key</td><td>Duplicate <strong>writes</strong> from retries and double taps</td><td>The server's retention window</td><td>Saving request count</td></tr>
  </tbody>
</table>
...
```ts
it("concurrent calls with the same key fold into one", async () => {
  let runs = 0;
  const run = () =>
  ...
  // after settle, a fresh request runs (regression test for trap one)
...
In production, the effect shows up in how launch feels more than in raw request counts. The setup I use to keep watching launch weight over time is written up separately in [Rork × MetricKit — Building a Solo Diagnostics Pipeline for Crashes, Hangs, and Battery Drain](/articles/rork-dev/rork-metrickit-production-diagnostics-complete-guide).
...
Remote config in particular is a strong candidate, since nearly every screen pulls it at launch. The distribution side of flags is covered in [Making Feature Flags Survive Production: Kill Switches and Gradual Rollouts in Rork Max](/articles/rork-dev/rork-max-feature-flags-remote-config), and how the received values stack against defaults and user preferences in [Resolving App Config in Three Layers: Merging Defaults, User, and Remote With Bounded Overrides](/articles/rork-dev/rork-config-precedence-resolver-bounded-override-typed-layer). Keeping the layer that folds fetches separate from the layer that decides values lets you swap either one later without touching the other.

The coalescer fits in one file. You do not need to rewrite all existing fetch code at once. Swap only the reads that are firing in duplicate onto `coalesce`, and as long as you keep the three points — deleting in `finally`, the reference count, and the grace before aborting — it starts helping from the moment you swap it in.
