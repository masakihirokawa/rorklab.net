...
## Catching mismatches between your consent code and Play's data disclosure

Getting UMP and ATT right is not the whole job. If what you declared in Google Play's Data safety section does not match what the SDKs in your build actually collect, a flawless consent flow will still get you a rejection.

The awkward part is that the mismatch rarely comes from code you wrote. It comes from SDKs the builder or the template pulled in for you. If the Rork-generated scaffold already shipped with an analytics library and a crash reporter, you are on the hook for declaring every data type those libraries touch. The ones you do not remember adding are exactly the ones that go undeclared.

A small script that derives "data types you probably need to declare" from your dependency list turns that pre-submission check into a few seconds of work. Drop it at the project root and run `node audit-data-safety.mjs`.

```javascript
// audit-data-safety.mjs
import { readFileSync, existsSync } from "node:fs";

// SDK name -> data types commonly requiring a Play Data safety declaration
...
if (!pkg) {
  console.error("No package.json here. Run this from the project root.");
  process.exit(2);
}

const deps = Object.keys({ ...pkg.dependencies, ...pkg.devDependencies });
const required = new Map(); // data type -> SDKs it came from
...
console.log(`SDKs detected: ${deps.filter((d) => SDK_DATA_TYPES[d]).length}`);
for (const [type, sources] of required) {
  const mark = declared.has(type) ? "OK  " : "MISS";
  console.log(`  ${mark} ${type}  <- ${sources.join(", ")}`);
}
if (extra.length) console.log(`\nDeclared but not traced to any SDK: ${extra.join(", ")}`);
if (missing.length) {
  console.error(`\nPossibly undeclared: ${missing.length}`);
  process.exit(1);
}
console.log("\nNo undeclared data types found.");
```

`data-safety.declared.json` is a transcript of what you actually selected in Play Console. The Console is the source of truth; this file is only a copy you can diff against.
...
Running it against a build with ads, analytics, crash reporting, billing, and location produced this:
...
Possibly undeclared: 4
```

With only three types declared on purpose, four gaps surfaced. Location is the interesting one. `expo-location` tends to arrive with some small "find places near me" feature, entirely separate from anything you were thinking about when you designed the consent flow. To Play, it is the same category of thing: data you collect.

Because it exits with code 1, wiring it into an EAS Build pre-hook or the first step of CI closes off the path where an undeclared build reaches submission at all.

The mapping table is deliberately incomplete. It is a skeleton you extend with the SDKs you actually use. Add one line on the day you add a dependency, and the version of you six months from now will be grateful.

## What tends to break alongside a target API level bump

From August 31, 2026, Google Play requires target API level 36 (Android 16) or higher for both new apps and updates to existing ones. If you cannot make that date, the extension form in Play Console keeps you shipping until November 1 — but the request itself has to be filed before the deadline.

This looks like a separate topic from consent management, and in practice it lands on the same working day. Raising the target API level pulls ad SDK and Firebase updates along with it, and everything touching your UMP calls moves at once.

Verify the generated target level with your own eyes first. The point is not to take the builder's scaffold at its word.

```bash
# Check what the Expo config layer specifies
grep -rn "targetSdkVersion\|compileSdkVersion" app.json app.config.js 2>/dev/null

# If you have prebuilt, check the generated Gradle files too
grep -rn "targetSdkVersion" android/build.gradle android/app/build.gradle 2>/dev/null
```

If nothing turns up, you are inheriting the Expo SDK default — and that default shifts between SDK versions. Running `npx expo config --type introspect` and reading the resolved value out of that output is the more dependable basis for a decision.

Around the bump, these are the three consent-related checks worth doing on a real device.

<table>
<thead>
<tr><th>Check</th><th>How</th><th>Symptom if missed</th></tr>
</thead>
<tbody>
<tr><td>UMP form still appears</td><td>Pin debug geography to EEA, cold start</td><td>Ad requests fire without consent and stay non-personalized</td></tr>
<tr><td>Consent withdrawal still works</td><td>Withdraw from settings, restart, confirm the form returns</td><td>Reviewers read it as having no withdrawal path</td></tr>
<tr><td>Ad SDK initializes after consent</td><td>Log on both sides of init and read the device log</td><td>Revenue dips only in the first session</td></tr>
</tbody>
</table>

For the ordering check, two `console.log` lines and a glance at the device log beat any more elaborate tooling. After being caught out a few times by initialization timing shifting under an SDK upgrade, I have stopped skipping this unglamorous step.

The closer the deadline gets, the more tempting it becomes to bump the target level, ship, and revisit consent later. The trouble is that revenue-shaped bugs progress quietly, and by the time you notice, you are looking at weeks of difference. Same day as the bump, if you can.
...
For deeper context, pair this with [Maximizing ATT opt-in rate for AdMob revenue in Rork apps](/articles/rork-dev/rork-att-opt-in-rate-maximization-admob) and [Fixing your Google Play Data safety declaration](/articles/rork-dev/rork-google-play-data-safety-section-fix-guide). Together they cover the full iOS + Android monetization privacy surface.