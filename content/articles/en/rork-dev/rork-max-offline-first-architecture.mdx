...
premium: true
highlights:
  - "Lessons from operating a wallpaper app series that reached 50M+ cumulative downloads — concrete design decisions that prevent crashes on subways, international flights, and rural areas with unstable connectivity"
...
The thirty minutes spent on a subway, the multi-hour international flight, the rural valley where signal drops out without warning — mobile users spend far more time disconnected than developers tend to imagine. I'm Masaki Hirokawa, an indie developer who's been shipping iPhone and Android apps since 2014. My wallpaper apps have reached 50 million cumulative downloads, and across years of running them, the single most consistent complaint in store reviews has been offline-related: "the app freezes when I'm on the train," "favorites disappeared in airplane mode," "it stopped loading in flight."

Offline-first architecture is the design philosophy that fixes those quiet, easy-to-overlook user pains. It also directly impacts the business side — AdMob fill rates, in-app purchase restoration timing, and server cost — none of which most offline-first tutorials touch. With Rork Max now generating native Swift code directly, the kind of precise sync engines that used to require a dedicated infrastructure team are within reach of solo developers. The scope of this article is the actual implementation patterns I've arrived at after running production apps at 50M-download scale: the pitfalls I've stepped into, the recovery patterns I've shipped, and the decision criteria I now apply when designing offline behavior.
...
```swift
// Recommended: SQLite WAL mode + explicit checkpoint
...
// Transactions auto-commit, but when you want to force the sync explicitly:
...
1. **Data with realistic conflict potential** (collaboratively edited notes, multi-device drawings) → CRDT is justified
2. **Last-write-wins is safe** (current scroll position, UI preferences, theme selection) → Skip CRDT, use timestamps
3. **Aggregate data** (favorite counts, download history) → Compute server-side, treat client as read-only cache
...
1. **Airplane mode cold launch** — Complete a full navigation loop fully offline. No crashes, no blank screens, no infinite loading states.
2. **Network Link Conditioner: 3G + 5% packet loss** — App must remain usable on slow, flaky links.
3. **Sync queue saturation** — Generate a large number of writes while offline, then verify the batch syncs correctly on reconnect.
...
When I first touched the internet in 1997 at 16 years old over a dial-up line, connections dropped constantly. I learned programming from strangers across the world in spite of the unreliability of that link, and looking back now, that experience of "wanting to stay connected through the instability" is the root of why I obsess over offline behavior in my apps today. With Rork Max, the kind of precise sync engines that were unthinkable for an indie developer back then are now within reach.
