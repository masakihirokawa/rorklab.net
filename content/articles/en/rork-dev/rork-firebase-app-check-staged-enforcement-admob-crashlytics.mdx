...
author: "Rork Lab"
description: "A practical, staged rollout for Firebase App Check in Rork apps that keeps Crashlytics reporting, Realtime Database listeners, and AdMob payouts intact — written from running AdMob-funded indie apps in production."
...
highlights:
  ...
  - "Three concrete failure modes where App Check silently killed Crashlytics and Realtime Database traffic in a live AdMob app, plus how to detect each one within a day"
...
App Check is well documented and the official setup guides are clear enough. What they tend not to dwell on is how easy it is to take a production app down by toggling a single switch from a console UI. As an indie developer whose apps are funded by ads, App Check is still the one piece of Firebase infrastructure that makes me nervous every time I roll it out. So this is the design I now use for Rork apps, written from the side of someone who has already taken the blast.
...
The `.catch` instead of an `await` inside the boot path is deliberate. The worst experience an indie app can ship is a splash screen that freezes during App Check initialization. After a couple of incidents in production, the rule that settled in was: if App Check init fails, continue silently, and let Remote Config decide whether the rest of the app switches into a degraded mode.
...
## Absorbing the first few seconds after install

On iOS, the App Attest attestation key is generated on the device the first time a freshly installed app launches. Until that generation completes, `getToken()` keeps failing. `isTokenAutoRefreshEnabled: true` does not save you here, because the retry cadence is left to the SDK's own backoff — so if your boot path attaches a Realtime Database listener before the first token exists, that particular connection is rejected with a 401.

What makes this failure mode nasty is that it only affects newly installed devices. It never reproduces on your own phone, because your device already holds a provisioned attestation key. The bug ships invisibly, and the only sessions that quietly go missing are the ones from users opening your app for the very first time.

The fix is a small gate in front of Firebase-dependent initialization — one that refuses to wait forever and falls through to a degraded mode instead.
...
export async function waitForAppCheckToken(): Promise<string | null> {
  ...
  for (const delay of BACKOFF_MS) {
    ...
    try {
      // forceRefresh: false — returns a cached token without a network round trip
      const { token } = await appCheck().getToken(false);
      if (token) return token;
    } catch {
      // Attestation key is still being provisioned; back off and retry.
    }
    await new Promise((resolve) => setTimeout(resolve, delay));
  }

  return null; // Continue in degraded mode.
}
```

`forceRefresh` stays `false` on purpose. From the second launch onward the cached token comes back immediately, so the gate costs nothing. Setting it to `true` would tax every launch for every user with a network round trip, just to solve a problem that only exists on first install.

The 4-second ceiling is derived from how long a splash screen can reasonably stay up, not from any property of App Attest. Tune it to your own startup experience — but keep the invariant that the gate always exits before the user starts wondering whether the app is stuck.

Decide up front what degraded mode actually does:

<table>
<thead>
<tr><th>Feature</th><th>Normal mode</th><th>Degraded mode</th></tr>
</thead>
<tbody>
<tr><td>Realtime Database</td><td>Attach listeners at boot</td><td>Render from local cache, connect once a token arrives</td></tr>
<tr><td>Crashlytics</td><td>Report normally</td><td>Keep reporting, but treat delivery as unconfirmed on the monitoring side</td></tr>
<tr><td>Remote Config</td><td><code>fetchAndActivate</code></td><td>Boot from the last activated values, re-fetch after the token arrives</td></tr>
</tbody>
</table>

The part people forget is the reconnect. Even after `waitForAppCheckToken()` returns `null`, retry `appCheck().getToken(false)` once in the background and re-attach the Realtime Database connection if it succeeds. Before I added that retry, first-install users had to kill and reopen the app before anything synced. It is a small slice of the userbase — and it is precisely the slice forming its first impression of the product.
...
I reach for the same hash-bucket primitive for every ad-related rollout I run. It is the cheapest stable bucketing I have found that does not require any server-side state. The literal `'anonymous'` fallback keeps signed-out users in a deterministic bucket; whether to include them at all is governed by a separate Remote Config flag, because some apps want anonymous traffic excluded from sensitive enforcement during the rollout window.
...
## Where each rejection shows up — and where it doesn't

Staging the rollout only helps if you know where to look for rejections. The awkward part is that App Check rejections surface in a different place for each Firebase product — and for one of them, nowhere at all. Here is what I found by checking them one at a time against my own apps.

<table>
<thead>
<tr><th>Product</th><th>Where the rejection is recorded</th><th>How it looks in the app</th><th>Realistic time to notice</th></tr>
</thead>
<tbody>
<tr><td>Cloud Functions</td><td>Cloud Logging (invocation logs)</td><td>Caller receives an error</td><td>Minutes</td></tr>
<tr><td>Cloud Storage</td><td>Cloud Logging</td><td>Images fail to load</td><td>Minutes</td></tr>
<tr><td>Realtime Database</td><td>Drop in concurrent connections</td><td>Listeners simply never attach</td><td>Half a day to a day</td></tr>
<tr><td>Remote Config</td><td>Unverified request count in the App Check console</td><td>App keeps running on cached values</td><td>Over a day</td></tr>
<tr><td>Crashlytics</td><td>Nowhere — the send fails client-side</td><td>Reports stop arriving</td><td>You may never notice</td></tr>
</tbody>
</table>

The bottom two rows cannot be caught by hunting for rejection logs. That Monday morning I described at the top was exactly this: I was looking for something that had failed, and the place I was looking had itself gone quiet.

**You cannot count rejections. You can only count arrivals.** For as long as I had that backwards, none of my alerts ever fired.

So alongside the real request paths, I keep one callable whose whole job is to report the verification result instead of acting on it. Leave `enforceAppCheck` as `false` on that one function and log whether `request.app` was populated — the outcome lands in your logs as data rather than as a failure.

```ts
// functions/src/appCheckHeartbeat.ts
import { onCall } from 'firebase-functions/v2/https';
import { logger } from 'firebase-functions';

export const appCheckHeartbeat = onCall(
  // This path exists to count arrivals, so it must never reject
  { enforceAppCheck: false },
  async (request) => {
    logger.info('app_check_heartbeat', {
      // request.app is only populated when the App Check token verified
      verified: request.app !== undefined,
      enforced: request.data?.enforced === true,
      version: request.data?.version ?? 'unknown',
    });
    return { ok: true };
  },
);
...
The client calls it once per session and swallows the result. Failure to arrive is the signal, so nothing here may throw.

```ts
// lib/appCheckHeartbeat.ts
import functions from '@react-native-firebase/functions';
import { nativeApplicationVersion, nativeBuildVersion } from 'expo-application';
import { isAppCheckEnforced } from './appCheckGate';

let sentThisSession = false;

export async function sendAppCheckHeartbeat() {
  if (sentThisSession) return;
  sentThisSession = true;

  try {
    await functions().httpsCallable('appCheckHeartbeat')({
      enforced: isAppCheckEnforced(),
      version: `${nativeApplicationVersion}(${nativeBuildVersion})`,
    });
  } catch {
    // Not arriving is the measurement. Nothing to do here.
  }
}
```

In the morning, this is the only query I run. Unverified sessions come back broken down by app version.

```bash
gcloud logging read \
  'jsonPayload.message="app_check_heartbeat" AND jsonPayload.verified=false' \
  --project "$FIREBASE_PROJECT_ID" --freshness=24h \
  --format='value(jsonPayload.version)' \
  | sort | uniq -c | sort -rn
```

If old build numbers sit at the top of that list, the cause is not your enforcement — it is those versions still being out in the wild. If the newest build is at the top, suspect a forgotten debug provider or the first-launch attestation window instead. The App Check console cannot split unverified requests by app version, and this heartbeat log has been the only practical way I have found to do it.

The cost is honest enough: one extra callable per session. I run it for everyone during the two weeks of a rollout, then dial it down to about 10% with a Remote Config percentage once things settle. Every real request path stays at `enforceAppCheck: true`. This single function is the only thing I loosen.
...
The "unverified requests" count in the App Check console itself. If this is still non-zero after a full 100% rollout, the most likely culprits are forgotten debug providers on staff devices, or older app versions still in the wild. Which of the two it is comes straight out of the heartbeat log from the previous section, so that is the first thing I open. Older versions need a forced upgrade strategy of their own, but in the meantime a Remote Config flag like `appCheckMinVersion` lets you scope enforcement only to clients above a given build number.
