...
author: "Rork Lab"
description: "How to protect a Rork app's production with three layers of safety: iOS Phased Release, Android Staged Rollout, and EAS Update OTA hotfixes — with the numeric thresholds I use to pause a rollout, a Sentry monitor, and the App Store Connect API call that pauses a phased release from your phone."
...
---
I still think about one particular evening. I submitted the build, watched the approval notification arrive, and went to sleep reassured. The next morning the first thing I opened wasn't email — it was the review feed, where three separate people had written some version of "it won't launch."

The cause was small and dull: date parsing that failed in exactly one locale. Fixing it took twenty minutes. What weighed on me was the other part — there was no way to pull the update back off the devices that already had it.

**What a release protects isn't "no bugs shipped." It's "what fraction of users can I stop it at."** Once I made that swap, the knot in my stomach before submitting got noticeably smaller.

Apps built with Rork are no exception here. The generated code is remarkably tidy, but no implementation runs perfectly across every device, locale, and network condition. So the time is better spent building a path to stop small than chasing a perfect build.

What follows is how I assemble that path in three layers — iOS Phased Release, Android Staged Rollout, and EAS Update OTA hotfixes — with what each one protects, what it doesn't, and the numbers I use to decide. I'll spend the most time on the parts a single read of the docs won't surface: what actually happens when you submit a fix mid-rollout, why native crashes can't be patched over the air, and the fact that pausing a phased release draws down a 30-day budget.
...
Running a wallpaper app over many years, the change that helped most wasn't a feature. It was deciding, while everything was calm, where I'd roll back to when something broke. Careful work isn't getting it right on the first pass — it's assembling things so you can return to the place you got it wrong. Staged rollout is that idea, handed off to the store's own machinery.
...
I avoid the per-release manual click by scripting this with fastlane's `deliver` action, which I cover in detail in [shipping screenshots, metadata, and builds with one command](/articles/rork-dev/rork-fastlane-eas-release-automation-complete-guide) — the same script can toggle Phased Release on automatically, so you don't have to remember the checkbox.

### Pitfall the docs don't spell out, #3: pausing draws down a 30-day budget

You can pause a phased release. What's easy to miss is that the pausing itself is what's capped. Apple's help page states that you may pause for a **total of 30 days** per version, with no limit on how many separate pauses you use. Pause for ten days and resume, and you have twenty days of budget left.

The resume behavior is worth knowing too, because it changes how heavy the decision feels. The rollout doesn't rewind to 1% — it **picks up on the day it left off**. Pause on Day 4 (10%), resume three days later, and you continue into Day 5 (20%). If you believe pausing costs you a restart, you'll hesitate at exactly the moment hesitation is expensive.

The same page carries two notes that matter more in practice than they look on the page.

<table>
<thead>
<tr><th>What the docs say</th><th>What it means for your rollout</th></tr>
</thead>
<tbody>
<tr>
<td>Apps and app updates in phased release can be manually downloaded from the App Store by anyone at any time</td>
<td>What's being throttled is the <strong>automatic-update percentage</strong> — not the number of people who can reach the new build. Set your monitoring thresholds assuming a bit more exposure than Day 1's 1% implies</td>
</tr>
<tr>
<td>Removing the app from sale stops phased release permanently for that version; when reinstated, it goes to all users immediately regardless of the percentage reached</td>
<td>"Just remove it from sale" can be the worst available move. If you only want to stop, use Pause. To get a controlled rollout back, make the version unavailable for download and submit a new version with phased release enabled</td>
</tr>
</tbody>
</table>

The second one is the trap I think is easiest to fall into while panicking. Early on I assumed stopping meant removing from sale. Pause and Remove from Sale behave completely differently on the way back.

**Pause to stop it; remove from sale only to end it.** That line sits at the top of my middle-of-the-night checklist.

### Pausing from your phone: the App Store Connect API

Pausing from the web UI is a few clicks. Even so, I wanted a way to stop a rollout before opening a Mac when an alert wakes me. The [App Store Connect API](https://developer.apple.com/documentation/appstoreconnectapi/app-store-version-phased-releases) has an endpoint for exactly this.

First, read the in-progress phased release. The returned attributes can change over time, so I dump the whole object rather than reaching for named fields.

```bash
# TOKEN is a JWT generated from your App Store Connect API key
...
Then use that `id` to move the state to `PAUSED`.
...
To resume, send the same shape with `phasedReleaseState` set back to `ACTIVE`.

One error here is genuinely hard to read the first time you hit it. Depending on the version's state, this PATCH comes back as a 409 with "An attribute value is not acceptable for the current resource state." Usually that means the version isn't in a distributable state (Ready for Distribution) yet, or the phased release has already completed — a constraint on the resource, not a mistake in your payload.

Which is why I don't wire this call to fire automatically from an alert. **Alerting is automatic; stopping is my finger on the button.** Retrying a 409 without reading it is slower than having a one-command path ready next to the Slack notification.
