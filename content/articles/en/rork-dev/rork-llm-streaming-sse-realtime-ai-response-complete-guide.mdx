---
title: "LLM Streaming in Rork Apps: Building ChatGPT-Style Real-Time AI Responses with Expo and SSE"
slug: "rork-llm-streaming-sse-realtime-ai-response-complete-guide"
category: "rork-dev"
level: "advanced"
premium: true
date: "2026-04-13T20:15"
updated: "2026-08-20T11:30"
author: "Rork Lab"
description: "Field notes on shipping LLM streaming (SSE) in React Native and Expo. Anthropic, OpenAI and Gemini behind one interface, AbortController cancellation and retries, a Cloudflare Workers proxy, context compaction, and a mock SSE server for testing at zero API cost."
...
**React Native's global `fetch` does not give you `response.body`.** RN's `fetch` is a polyfill layered on XMLHttpRequest, and stream support was never implemented ([facebook/react-native#27741](https://github.com/facebook/react-native/issues/27741) has been open for years). Upgrading your Expo SDK does not change this.
...
<tbody>
<tr><td><code>body</code> is undefined, no error thrown</td><td>You are on the global fetch</td><td>Add <code>import &#123; fetch &#125; from 'expo/fetch';</code></td></tr>
<tr><td>Still undefined after importing</td><td>Expo SDK 51 or earlier</td><td>Upgrade to SDK 52+, or fall back to a library such as <code>react-native-sse</code></td></tr>
<tr><td>Works on web, fails on device only</td><td><code>EXPO_PUBLIC_USE_RN_FETCH=1</code> is set</td><td>That flag restores the RN implementation as the global. Named imports are unaffected, so check your import path first</td></tr>
</tbody>
</table>
...
// ⚠️ The global fetch cannot stream. This named import is required.
import { fetch } from 'expo/fetch';
...
// For error events that arrive after an HTTP 200. A dedicated class keeps them
// distinguishable from ordinary per-chunk JSON parse failures.
...
export function useStreamingChat(): StreamingState {
  ...
  // Whether we've painted at least one character. Gates retry eligibility.
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    let retryCount = 0;
    hasEmittedRef.current = false; // reset on every send
    const maxRetries = 2;
    ...
    const attemptFetch = async (): Promise<void> => {
      try {
          ...
          for (const event of events) {
            for (const line of event.split('\n')) {
              ...
              try {
                ...
                // ⚠️ Easy to miss: an error event can arrive *after* a 200 OK.
                // Drop it and a truncated answer looks like a clean finish.
                ...
                // Do not swallow StreamAbortedError — let it reach the handler
        ...
        // ⚠️ Retrying after even one character has painted rewinds the UI.
        // accumulatedText is re-initialised inside attemptFetch, so the first
        // paint of the retry replaces visible text with a shorter string.
        if (hasEmittedRef.current) {
          const message = err instanceof Error ? err.message : 'Unknown error';
          setError(`Response ended early: ${message}`);
          return;
        }
...
The solution is a thin proxy layer — in this case a Cloudflare Worker that holds the key server-side and forwards requests. See also the [Hono × Cloudflare Workers REST API guide](/articles/rork-dev/rork-hono-cloudflare-workers-rest-api-guide) for related patterns.
...
**Embedding API keys in app code**: The binary is downloadable and inspectable. Even obfuscated keys get extracted. Always use a server-side proxy. See also the [hiding API keys behind a Worker with rate limiting and key rotation](/articles/rork-dev/rork-expo-api-key-proxy-worker-rate-limit-rotation) for more on this architecture.
