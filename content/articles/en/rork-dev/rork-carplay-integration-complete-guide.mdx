...
author: "Rork Lab"
description: "What it actually took to get CarPlay support shipped on a Rork-built app — writing the entitlement request, configuring UIScenes, choosing CPTemplate APIs, sharing data through App Groups, and the review notes I only learned about the hard way, with category-specific design notes for audio, navigation, and messaging."
...
Every rental car I pick up seems to have CarPlay in it now. Yet when I open the App Library on the head unit, the list of third-party apps is still short enough to count. This gap isn't due to implementation difficulty — it's because most indie developers assume CarPlay is reserved for big brands. In practice, with a well-scoped category and a clear justification, individual developers can and do get CarPlay entitlements approved. Apple benefits from a wider ecosystem of CarPlay apps, so the review criteria are strict but not prejudiced against solo developers.
...
What surprised me when I went back through the numbers on my own audio app wasn't that in-car sessions ran longer — it was how little that length varied from one day to the next. Commutes are fixed, so the listening is too. I may be reading too much into it, but the steadiness felt more useful than the size of the lift, because it made the effect something I could plan around.
...
There's no `synchronize()` call. Manual synchronization has been discouraged for years, and writes land on their own. Calling it isn't harmful, but it gives you a false sense that the data definitely arrived — the real inconsistency lives on the read side, in the other process.

The second is registration. Without it, `NativeModules.SharedStorage` comes back `undefined` and the failure surfaces in JavaScript rather than anywhere near your App Group configuration. One bridging file covers it.
...
RCT_EXTERN_METHOD(saveEpisodes:(NSArray *)episodes
...
Read from the same suite in the CarPlay scene. The most common mistake there is using `UserDefaults.standard`, which maintains separate stores for the iPhone and CarPlay processes. Always use `UserDefaults(suiteName:)`.
...
Image sizing deserves the same treatment. My first version of this code baked in the number I'd found while reading around. That didn't work out: the cap varies by app category, so artwork that rendered fine in one vehicle went missing in another, and I spent a while chasing reports I couldn't reproduce.

Apple exposes the limit itself as [`CPListItem.maximumImageSize`](https://developer.apple.com/documentation/carplay/cplistitem/maximumimagesize) — a resolution-independent value in points, returned for the category your app actually runs under. Asking is safer than remembering.

```swift
extension UIImage {
    /// Returns a copy clamped to the runtime CarPlay list image limit
    func fittedForCarPlayList(scale: CGFloat) -> UIImage {
        let limit = CPListItem.maximumImageSize
        guard size.width > limit.width || size.height > limit.height else { return self }
...
An oversized image simply doesn't appear. No exception, no log line — one of the quietest bugs in CarPlay development.

List length fails just as quietly. `CPListTemplate` caps both the number of sections and the total number of items, and anything past the cap is dropped without an exception or a log line. Those caps can shift between OS releases, so read them from the API at runtime instead of hardcoding numbers.

```swift
extension CPListTemplate {
    /// Build a template clamped to the runtime limits
    static func clamped(title: String, sections: [CPListSection]) -> CPListTemplate {
        let maxSections = min(sections.count, CPListTemplate.maximumSectionCount)
        var remaining = CPListTemplate.maximumItemCount
        var safeSections: [CPListSection] = []

        for section in sections.prefix(maxSections) {
            guard remaining > 0 else { break }
            let items = Array(section.items.prefix(remaining))
            remaining -= items.count
            safeSections.append(CPListSection(items: items, header: section.header, sectionIndexTitle: nil))
        }
        return CPListTemplate(title: title, sections: safeSections)
    }
}
```

The subtlety is that `maximumItemCount` applies to the template as a whole, not per section. Assume it's per-section and you get a bug where only the trailing sections vanish — and only on device, where your content set is larger than your test fixture. That one cost me a full day of hunting a "last category doesn't appear" report.

Rendering speed is the other simulator blind spot. The CarPlay window runs on your Mac's hardware, so a heavy list build feels instant there and sluggish on a real head unit. I budget 16ms for template construction, load artwork asynchronously, and return a placeholder item first so the list is never blocked on network I/O.

Disconnect behavior is testable too: closing the CarPlay window in Xcode fires `sceneDidDisconnect`. Cycle through connect, play, disconnect, reconnect a few times before you submit. Reviewers do check that flow, and a stale interface controller reference is an easy way to crash on reconnect.
...
**2. Works in the simulator, fails on device.** In my case the entitlement was the problem. My development profile carried the CarPlay capability, but I'd never regenerated the distribution profile, so the archived build shipped without it. Now I run `codesign -d --entitlements :- YourApp.app` against the built product and read the keys with my own eyes before uploading. The simulator never checks signing, so this is exactly the gap it can't cover for you.
...
After launch, track CarPlay-specific metrics separately — in-car session duration, continuous playback time, and crash rates during driving — in App Store Connect. For related architectural context, [Building Production-Ready iOS Apps with Rork](/articles/app-dev/rork-ios-app-production-guide) and [Rork Max App Store Approval Guide](/articles/rork-business/rork-max-app-store-review-guide) pair well with this article.
...
Before any of it works, you need to be able to separate in-car activity from phone activity. Tag every event with the surface it came from, driven by the scene lifecycle — retrofitting this later means the first month of data is unrecoverable.

```swift
final class CarPlaySessionTracker {
    static let shared = CarPlaySessionTracker()
    ...
    func markDisconnected() {
        defer { isConnected = false; connectedAt = nil }
        guard let start = connectedAt else { return }
        Analytics.log("carplay_session_end", params: [
            "duration_sec": Int(Date().timeIntervalSince(start))
        ])
    }

    var surface: String { isConnected ? "carplay" : "phone" }
}
```

Attach `surface` as a global parameter on every event you emit. With that in place, the playbook below becomes measurable rather than anecdotal.
...
It's worth deciding early whether Android Auto is in scope. It's a separate Google platform with its own APIs, review process, and UI conventions, so treat it as a second implementation track rather than a port. Committing to both without budgeting for that doubles the maintenance surface you just signed up for.
...
## Your First Step — Pick One Category and Write the Request

For a long time I treated CarPlay as an implementation problem. Looking back, the part I kept getting stuck on came earlier than any code. I was choosing templates and answering review questions before I'd decided which category I was shipping under, so every answer moved.

So the first step I'd suggest isn't code — it's opening [Apple's CarPlay developer page](https://developer.apple.com/carplay/), picking the single category closest to what your app already does, and writing roughly 500 characters on the three criteria: in-drive necessity, safety consideration, category fit. If you can write it, the implementation will wait for you. If you can't, what's missing probably isn't Swift — it's the value that only exists while someone is driving.

**If the request doesn't fit in one paragraph, the feature isn't ready for the car yet.** That's the one line I try not to cross, even on days when the code is already running.

Thank you for reading this far. The in-car context is still unfamiliar ground for me, and I'll keep writing down what I find.
