...
1. **Fetching JWKs on every request inflates both latency and your request count to Apple**
...
2. **On key-rotation day, `kid` won't match and every sign-in fails at once**
...
```ts
// server/apple-jwks.ts — force exactly one refetch when the kid is missing
...
async function fetchJwks(force = false): Promise<Jwk[]> {
  ...
  const res = await fetch(JWKS_URL); // Apple returns Cache-Control: max-age=...
  await cache.put(cacheKey, res.clone()); // cache as-is, honoring max-age
...
export async function getApplePublicKey(kid: string): Promise<Jwk> {
  ...
  if (!key) {
    // just after rotation the new key isn't cached yet → refetch once
...
3. **An expired `client_secret` (ES256 JWT) turns both `revoke` and `token` exchange into blanket 500s**
...
4. **Confirming the deletion when `revoke_token` fails leaves ghost accounts behind**
...
5. **Mail to private-relay addresses won't arrive if you send it straight from your own SMTP**
