...
author: "Rork Lab"
description: "The default AI Gateway cache key is built from the request you actually send. Here is why prompt normalization silently does nothing, how to wire it in with cf-aig-cache-key, and the Workers and React Native code I run in production."
...
In the gateway settings, enable "Cache Responses" and set a default TTL. I use 24 hours for classification tasks. Caching is off until you do this, which is worth saying plainly: I swapped the URL one evening, came back the next morning expecting a hit rate, and spent a while staring at 0% before I found the toggle.

You can override the default per request with headers, which is what the Workers code below does.

## What the Cache Key Is Actually Made Of

Before the code, the thing I wish I had read first.

By default the gateway builds the cache key by concatenating five things and hashing the result with SHA-256, as described in the [AI Gateway caching documentation](https://developers.cloudflare.com/ai-gateway/features/caching/):

<table>
<thead><tr><th>Component</th><th>What it means in practice</th></tr></thead>
<tbody>
<tr><td>Provider (openai, anthropic, …)</td><td>Nothing is shared across providers</td></tr>
<tr><td>Endpoint (the API path)</td><td>A different path is a different cache</td></tr>
<tr><td>Model</td><td>Swap the model and you start from an empty cache</td></tr>
<tr><td>Provider auth header</td><td><strong>Rotate your API key and the cache turns over with it</strong></td></tr>
<tr><td>Full request body</td><td>One character of difference in messages, tools, or parameters is a separate entry</td></tr>
</tbody>
</table>

Two consequences follow, and both cost me time.

The first: a normalization function only helps if the normalized string is what you actually send, or if you override the key yourself with `cf-aig-cache-key`. Writing the function and leaving the original prompt in the body changes nothing. The proxy below sends `normalizedPrompt`, and also sets an explicit key so the behaviour does not depend on a detail someone might refactor away later.

**The key is decided by the request that leaves your Worker, not by the helper you wrote next to it.**

The second: because the auth header is part of the key, a cost spike the day after a key rotation may be nothing more than a cold cache. That is a much calmer thing to discover on purpose than at two in the morning.
...
app.post('/ai/generate', async (c) => {
  ...
  const cacheKey = buildCacheKey(normalizedPrompt, model, 'generate')
  ...
  if (openAIResult.success) {
    // Pass HIT/MISS through so the client can log why a call felt slow
    return c.json(openAIResult.data, 200, {
      'x-ai-cache-status': openAIResult.cacheStatus,
  ...
  if (geminiResult.success) {
    return c.json(geminiResult.data, 200, {
      'x-ai-cache-status': geminiResult.cacheStatus,
...
async function tryOpenAI(params: {
  ...
  | { success: true; data: unknown; cacheStatus: string }
  | { success: false; errorCode: number; errorMessage: string }
> {
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${params.apiKey}`,
    // Time to the first byte of the response, enforced by the gateway
    'cf-aig-request-timeout': '8000',
    // Transient failures are retried before they reach this code
  ...
  if (params.cacheEnabled) {
    // Setting the key opts this request into caching and pins it to the
    // normalized prompt, independent of the rest of the body
    headers['cf-aig-cache-key'] = params.cacheKey
    headers['cf-aig-cache-ttl'] = '86400'
  } else {
    // Note: cf-aig-cache-ttl: '0' does NOT disable caching. This does.
...
> {
  ...
  if (params.cacheEnabled) {
    // Prefix the key: the provider is part of the default key, so once you
    // override it you have to keep providers apart yourself
    headers['cf-aig-cache-key'] = `gemini:${params.cacheKey}`
    headers['cf-aig-cache-ttl'] = '86400'
  } else {
    headers['cf-aig-skip-cache'] = 'true'
  }
...
// FNV-1a: no dependencies, and it produces the same value in Workers
// and in React Native. Share this file between both sides.
...
// The model is part of the key so a model swap never serves stale answers
export function buildCacheKey(normalized: string, model: string, task: string): string {
  return `${task}:${model}:${fnv1a(normalized)}`
}
...
interface GenerateResult {
  ...
  cacheStatus: string   // HIT or MISS at the gateway
  provider: string
}

// Same file as the Worker uses. If the two drift, the two tiers
// start caching under different keys and neither one looks broken.
import { buildCacheKey, normalizePrompt } from '../shared/cacheKey'
...
export function useAI() {
  ...
  const generate = useCallback(async (opts: GenerateOptions): Promise<GenerateResult | null> => {
    ...
    if (cache) {
      try {
        const key = `aicache:${buildCacheKey(normalizePrompt(prompt), model, 'generate')}`
    ...
    try {
      ...
      const result: GenerateResult = {
        ...
        cacheStatus: res.headers.get('x-ai-cache-status') ?? 'UNKNOWN',
        provider: res.headers.get('x-ai-provider') ?? json._provider ?? 'openai',
      }
...
If you have chat-like features that need real-time streaming output, keep them on a separate endpoint (`/ai/stream`) that sets `cf-aig-skip-cache: true`. Reserve the cacheable `/ai/generate` endpoint for batch-style tasks like classification, summarization, and extraction.

One correction I owe anyone who read an earlier version of this: `cf-aig-cache-ttl: 0` is not a way to disable caching. That header sets the duration of a request that is already being cached, and the accepted range runs from **60 seconds to one month**. The header that bypasses the cache is `cf-aig-skip-cache`. I had the wrong one in production for a while, and the reason it never announced itself is that caching was off at the gateway level anyway — so the code looked correct right up until the day I turned caching on.
...
Without normalization, these are three separate cache entries. With the normalization function in the Workers code above — collapsing whitespace, lowercasing, normalizing punctuation — they become one. In my implementation that change moved cache hit rate from 12% to 61%.

The part that is easy to miss: normalizing is only half of it. The normalized string has to be the one that leaves your Worker, or it has to reach the gateway as `cf-aig-cache-key`. I spent a fortnight watching a flat 12% because my normalizer ran, returned a tidy string, and then the original prompt went out in the body regardless. **A helper function that nothing downstream reads is indistinguishable from no helper at all.**
...
Two notes on reading these numbers. The dashboard is for trends; for any single request, `cf-aig-cache-status` comes back as `HIT` or `MISS`, which is why the proxy above forwards it to the client. When someone reports that the app felt slow, knowing whether that one call was a MISS narrows the search considerably.

And 73.4% is the hit rate for this app's prompt mix — mostly classification and summarization. An app built around open-ended conversation will not land anywhere near it. **I would rather you watched your own Cache Hit Rate for a week than trusted my number.** The gateway cache is also volatile: the documentation notes that two identical requests arriving at once may both miss, because the first has not finished storing when the second arrives. Cache warming shortens the queue at the front; it does not eliminate it.

## Where to Start

The implementation in this article is production-ready, but deploy incrementally:

**Week 1**: Set up AI Gateway in Cloudflare and point your existing Workers code at the gateway URL. Caching is off by default, so nothing about your behaviour changes — you simply start seeing your baseline traffic in the dashboard. Log `cf-aig-cache-status` from day one, even while every response is a MISS.
