---
title: "Adding a Notification Service Extension to a Rork App — Rich Push, Encrypted Payloads, and Dynamic Rewriting"
...
author: "Rork Lab"
description: "Wiring a Notification Service Extension into a Rork-built iOS app — image attachments, end-to-end encrypted payloads, and dynamic APNs payload rewriting — with working Swift code and the production pitfalls that bit me along the way."
...
highlights:
  ...
  - "You'll get a battle-tested pattern for staying inside three separate ceilings — the 30-second runtime, the ~24 MB memory budget, and the attachment size limits — while still doing image downloads, localization, and personalization"
---
...
I've walked that road. The first time I built a Notification Service Extension (NSE), I had to stitch together fragments from ten different Apple docs, Stack Overflow threads, and forum posts before anything actually rendered. The awkward part was that none of those sources were wrong. They just never agreed on what to do first.

So what follows is ordered the way I eventually settled on doing it: **minimum image attachment → decrypting end-to-end encrypted payloads → rewriting the payload on-device → the monitoring that starts mattering the week after you ship.** The code assumes you've generated native iOS targets with Rork Max, but the same sequence holds if you're coming from an Expo bare workflow or a React Native project.
...
Before any code, it's worth being clear about when an NSE actually runs and what it can and can't do. Without this mental model, the 30-second deadline and the memory ceiling will feel like random gotchas.
...
The hard constraints: **30 seconds of wall-clock time, and roughly 24 MB of memory**. If you're getting close to 30 seconds, the system calls `serviceExtensionTimeWillExpire()` so you can return your best-effort result before being killed. If you ignore it and overrun, the user sees the original payload — usually meaning no image and no decrypted body.

The memory number is one I had wrong for a long time. I'd mentally filed extensions together and assumed something near 50 MB, built image handling on that assumption, and only found out when a device crash log came back with `EXC_RESOURCE / RESOURCE_TYPE_MEMORY (limit=24 MB)`. **A notification extension's memory budget can't be estimated from how the main app behaves.** 24 MB is small enough that decoding a single 4K image with `UIImage(data:)` will exceed it on its own, so downsampling isn't an optimization here — it's the only way the path works at all.
...
URLs without a clean extension (CDN URLs with query strings, signed URLs) are particularly nasty. `UNNotificationAttachment` infers the type from the saved file's extension, so if you save the file as `imagefile` with no `.jpg`, the system silently drops the attachment. I lost two hours to this once before realizing the cache headers had nothing to do with it.

Guessing at filenames is the fragile version. Declaring the type outright is the durable one:

```swift
import UniformTypeIdentifiers

// Derive the UTI from the response's Content-Type instead of trusting the path
...
Pass the UTI through `UNNotificationAttachmentOptionsTypeHintKey` and the extension on disk stops mattering. If your CDN returns things like `?w=1024&fm=webp`, I'd make this the default rather than the special case.

## The Third Reason Attachments Disappear — Size and Format Ceilings

`mutable-content: 1` is set. The download returned 200. The image still doesn't show. When those three are all true, the remaining suspect is the attachment itself.

`UNNotificationAttachment` enforces per-type size limits, and anything over the line is rejected at attach time.

<table>
<thead>
<tr><th>Type</th><th>Limit</th><th>What you actually see</th></tr>
</thead>
<tbody>
<tr><td>Audio</td><td>5 MB</td><td>Notification delivers. Attachment simply absent</td></tr>
<tr><td>Image</td><td>10 MB</td><td>Same — and the download log still reads as a success</td></tr>
<tr><td>Video</td><td>50 MB</td><td>Same</td></tr>
</tbody>
</table>

What makes this hard to catch is that the failure arrives as a **valid result** rather than an exception. In the code above, `catch` runs `contentHandler(bestAttemptContent)` and the user gets a perfectly reasonable text notification. Without telemetry, that's indistinguishable from success. Meanwhile the gorgeous 12 MB PNG your designer exported downloads in under a second on office Wi-Fi, so it never reproduces locally.

What worked for me was measuring the file before building the attachment, and skipping — with a log line — anything close to the ceiling. After downsampling you'll usually land around 200 KB, so the check almost never fires. That's the point: when it does fire, it's telling you a CDN variant rule broke.

```swift
private static let attachmentByteLimit = 10 * 1024 * 1024  // image ceiling
...
private func makeAttachment(from url: URL, mime: String) throws -> UNNotificationAttachment? {
    ...
    guard size > 0, size < Self.attachmentByteLimit else {
        ...
        return nil   // drop the attachment, keep the notification
    }
    let utType = UTType(mimeType: mime) ?? .jpeg
...
**The body is the promise; the attachment is the bonus.** Separating those two has made triage on notification bug reports much faster for me.
...
## Living Inside 30 Seconds and 24 MB
...
override func serviceExtensionTimeWillExpire() {
    ...
    finish(bestAttemptContent, outcome: "expired")   // single exit, defined below
}
```

You'll often see the expiry path call `contentHandler` directly. When it does, that path quietly skips both your timing measurements and your double-call guard. Keep one exit.

For memory, downsample aggressively. Loading a full image with `UIImage(data:)` is the easy way to blow past 24 MB. Use `CGImageSourceCreateThumbnailAtIndex` to size the image down to what the notification UI actually needs:
...
Worth wiring the pieces together explicitly, because a helper that exists but never gets called is easy to miss in review — I shipped exactly that for a while, with the download handler attaching the original file. Downsampling belongs immediately before the attachment is built.
...
let task = URLSession.shared.downloadTask(with: imageURL) { [weak self] localURL, response, error in
    ...
    // Never attach the original: shrink it, measure it, then attach it
...
With `downsample → makeAttachment → finish` as a single line, adding webp support or multiple attachments later only changes the middle.

Notification UI rarely needs more than 800–1024 px. If your CDN keeps handing you 4K originals, a server-side resized variant is the better fix. Doing it in the NSE is a fallback for when you can't change the server.
...
**Layer 1 — Local payload simulator.** During development, you don't want to wait for real APNs traffic to test extension changes. Drop a JSON file into your project (e.g. `test-payload.apns`) and drag it onto the running simulator while the NSE scheme is active. The system invokes your extension just as it would for a real push.

```json
{
  "Simulator Target Bundle": "com.example.myapp",
  "aps": {
    "alert": { "title": "New message", "body": "New message" },
    "mutable-content": 1,
    "sound": "default"
  },
...
Leave out `Simulator Target Bundle` and the simulator silently ignores the file — no error, no log line. Everyone hits that once. This is the fastest feedback loop and where 80% of NSE bugs should be caught.
...
Funnel every outcome — success, failure, and expiry — through a single exit point. Collapsing them there also kills the double-`contentHandler` bug for good.

```swift
private let startedAt = CFAbsoluteTimeGetCurrent()
private let log = Logger(subsystem: "com.example.myapp.nse", category: "delivery")

/// Every path out of the extension goes through here
private func finish(_ content: UNNotificationContent, outcome: String) {
    guard let handler = contentHandler else { return }  // second call stops here
...
Each layer catches a different class of failure, and none of them substitutes for the others:

<table>
<thead>
<tr><th>Layer</th><th>Catches</th><th>Misses</th></tr>
</thead>
<tbody>
<tr><td>Local <code>.apns</code></td><td>Payload parsing, rewrite logic, attachment failures</td><td>Environment and certificate differences, real network latency</td></tr>
<tr><td>TestFlight + production APNs</td><td>Gateway mismatches, distribution profiles, throttling</td><td>The spread across real networks and devices</td></tr>
<tr><td>Field telemetry</td><td>The latency tail, breakdown of failure reasons</td><td>Root cause — it observes, it doesn't explain</td></tr>
</tbody>
</table>
...
- Image downsampling is in place; memory peak stays under 20 MB in Xcode's Memory Gauge, against a ~24 MB ceiling
- Byte size is checked before attaching (10 MB images, 5 MB audio, 50 MB video)
...
Building native extensions on top of a Rork app is unfamiliar territory at first, but it's also where Rork stops feeling like a no-code tool and starts feeling like a real iOS development environment — one where you can call into any Apple framework you need. If you want to go past notifications and into full native call UI, [Rork with CallKit and PushKit for VoIP apps](/articles/rork-dev/rork-callkit-pushkit-voip-implementation-guide) picks up the same thread — receiving a dedicated push on a separate route is the same "finish the work on the device" idea, one layer deeper.
