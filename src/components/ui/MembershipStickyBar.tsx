 ...
 * 判定の対象になりません。モバイルでもファーストビューを潰さない高さに抑えています。
 *
 * 表示条件（すべて満たしたときのみ表示）:
...
function getCopy(locale: string, siteName: string) {
  if (locale === "en") {
    return {
      ...
      leadShort: `${siteName} is ad-free and member-supported.`,
...
export function MembershipStickyBar({ locale, siteName }: Props) {
  ...
  useEffect(() => {
    ...
    if (visible) {
      root.style.setProperty("--floating-bottom", isMobile ? "76px" : "96px");
  ...
  // 表示資格の判定（非購入者・7日以内に閉じていない・このタブで閉じていない）
  useEffect(() => {
    setMounted(true);
    ...
    const evaluate = () => {
      ...
      eligibleRef.current =
        !hasPurchaseCookie() && !dismissedRecently() && !closedThisSession();
    ...
    >
      <div
        style={{
          maxWidth: 1000,
          margin: "0 auto",
          padding: isMobile ? "10px 12px 10px 16px" : "14px 96px 14px 24px",
          display: "flex",
          alignItems: "center",
          gap: isMobile ? 10 : 20,
        }}
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {isMobile ? copy.leadShort : copy.lead}
          </p>
          {!isMobile && (
            <p
              style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 4 : 10, flexShrink: 0 }}>
          ...
          >
            {isMobile ? copy.membershipShort : copy.membership}
          </Link>
          {!isMobile && (
            <Link
